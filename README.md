...
| `fabrik:revalidate` | Force re-entry of the Validate stage — the engine clears `stage:Validate:complete`, `fabrik:auto-merge-enabled`, `fabrik:awaiting-ci`, `fabrik:awaiting-review`, and itself, then re-runs Validate on the current HEAD SHA. Use as a one-action recovery for stuck-Validate, or as the manual fallback for items that completed Validate before SHA-change auto-revalidation was deployed. |
