...
type PRSettleResult struct {
	...
	// RequiredContextsStatus, RequiredMissing/Pending/Failed (ADR-072): the
...
func (e *Engine) settlePRMergeState(item gh.ProjectItem, _ *stages.Stage) PRSettleResult {
	...
	if len(checkRuns) == 0 {
		// ADR-072: a confirmed required-context failure must block regardless
		...
		// ADR-072: before declaring "no CI configured" ready, confirm any
	...
	default:
		// ADR-072: the present check runs alone read as "ready" — but that must
...
// required-context classification (ADR-072): a confirmed failure blocks with
