...
type PRSettleResult struct {
	...
	// RequiredContextsStatus, RequiredMissing/Pending/Failed (ADR-071): the
...
func (e *Engine) settlePRMergeState(item gh.ProjectItem, _ *stages.Stage) PRSettleResult {
	...
	if len(checkRuns) == 0 {
		...
		// ADR-071: before declaring "no CI configured" ready, confirm any
	...
	default:
		// ADR-071: the present check runs alone read as "ready" — but that must
...
// required-context classification (ADR-071): a confirmed failure blocks with
