...
func TestInterpretClaudeResult_APIErrorExit_ReturnsSentinelError(t *testing.T) {
	...
	text, completed, usage, err := interpretClaudeResult(context.Background(), 1, raw, errors.New("exit status 1"), false, t.TempDir()+"/sess", t.TempDir(), "", 2, -1)
...
func TestInterpretClaudeResult_APIErrorWithRealProgress_NotClassified(t *testing.T) {
	...
	_, completed, usage, err := interpretClaudeResult(context.Background(), 1, raw, errors.New("exit status 1"), false, t.TempDir()+"/sess", t.TempDir(), "", 2, -1)
