...
| `queued_comment_eject_test.go` | Sim-only | `queued_comment_eject_test.go` (sim) — #1863/ADR-1863's Queued comment eject: `TestQueuedCommentEject_WorkerInFlight_BatchContinues` (pending signal applied at the worker's checkpoint; the rest of the batch lands), `TestQueuedCommentEject_NotInLiveBatch_EjectedDirectly` (batch-cap overflow member), `TestQueuedCommentEject_NonActionableComments_NeverEject` and `TestQueuedCommentEject_FailedReroute_RetriesNextPoll`. `tests/sim` never wires in `boardcache.CacheImpl` or the reactive observers, so the scenarios call `Engine.RegisterObservers` and the `SimulateCacheStatusWriteThroughForTest` seam to stand in for the cache's status write-through (otherwise the rerouted member is never re-admitted), start the sim `Clock` at real now (`mergeTrainEnvOptions.StartTime`) so the awaiting-ci backstop does not fire, and hold the CI verdict back 100ms so the scan's pending signal is always set before the worker's checkpoint. |
...
- `settleQueuedReviewFindings` (ADR-1208) — merge-train-specific; #1452's own
  ...
  ADR-1208) remains untested at the sim layer, left as a follow-up. (The
  same scan's *comment* cause, #1863/ADR-1863, is covered —
  `queued_comment_eject_test.go`; see its row below.)
