...
Instead: Pruefer's Go code (`github.Client.SubmitPRReview`, `pruefer/review.go`) submits the review directly. As of #1251, `SubmitPRReview` accepts a Go-computed `event` (see §8, amended by [adrs/1251-pruefer-severity-gated-request-changes.md](1251-pruefer-severity-gated-request-changes.md)) — but Claude never supplies or influences it; the value is derived entirely from parsed finding severities, never from prompt output. Claude's tool allowlist (`pruefer/claude.go`'s `reviewAllowedTools`) is narrowed to `Read`, `Grep`, `Glob`, and specific read-only git subcommands (`Bash(git diff:*)`, `Bash(git log:*)`, `Bash(git show:*)`, `Bash(git blame:*)`, `Bash(git grep:*)`, `Bash(git status:*)`) rather than blanket `Bash(git:*)` or any `gh` access at all. The App installation token is never placed in the Claude subprocess's environment — Claude has no GitHub-calling capability, so there is nothing to leak even if the allowlist were somehow bypassed. This makes "never approve" a code-level invariant, not a prompt-level one — `REQUEST_CHANGES` is no longer categorically unreachable, but `APPROVE` remains so, permanently (see §8).
...
> **Amended by #1251** ([adrs/1251-pruefer-severity-gated-request-changes.md](1251-pruefer-severity-gated-request-changes.md)): `REQUEST_CHANGES` is no longer categorically unreachable — see that ADR for the current decision. The original V1 reasoning is kept below for history; `APPROVE` remains permanently out of scope, unchanged from what's recorded here.
