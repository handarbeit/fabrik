...
- Partial spawn failures leave the parent in `fabrik:paused` with an error comment; v1 requires manual cleanup of orphaned children. Automatic rollback is out of scope. **Amended by [ADR 1583](1583-spawn-children-resume-safe-retry.md):** the "manual cleanup of orphaned children" half of this consequence no longer holds — a retried spawn (the operator's own "remove `fabrik:paused`, re-advance" recovery path) now recognizes and resumes an already-created child instead of duplicating it. Automatic rollback remains out of scope; ADR 1583 makes retry resume-safe, not automatically-reversible.
