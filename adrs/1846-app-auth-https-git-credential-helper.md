...
- `fabrik init --github-app` still requests the base permission set, so a manifest-created App needs `contents:write` granted by hand to run over HTTPS. Follow-up: request it by default.
- The token file sits on disk for the daemon's lifetime (0600, gitignored `.fabrik/state/`). It is removed on clean shutdown. Workers can already read the same token from `GH_TOKEN`, so the file adds no new exposure to them.
