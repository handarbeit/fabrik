...
- `tests/sim/review_authority_test.go` (#1450) — deterministic sim-bed port of this ADR's four live scenarios (`TestReviewAuthorityReinvokesOnChangesRequested` superseding the retired `TestReviewAuthorityBlocksAndPausesOnChangesRequested` per ADR-1375, `CycleLimitPauses`, `ClearsOnApproval`, `YoloDoesNotBypassBlock`, `AdvisoryRegressionGuard`), zero token cost, seconds instead of minutes. Inherits the same landing-gate scope limitation this ADR documents (advance gate only, `reviewGateBlocksLanding` unreached). Does not replace the live suite named above — see `tests/sim/README.md`'s "permanently blind to" section (GraphQL/REST wire correctness) — but is the pre-gate regression coverage for this ADR's state-machine behavior going forward.
