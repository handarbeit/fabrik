...
- **The unsafe `NoBrowser` zero value bit again, in `cmd` (#1781).** This ADR named the hazard precisely — `Options{}` built without a deliberate `NoBrowser: true` means "attempt to open a browser" — and fixed it for the engine, but the seven `runGitHubAppSetup` call sites in `cmd/init_github_app_test.go` were never audited. One of them (`TestRunGitHubAppSetup_Discovery_NoInstallationFound`) reaches `guideMissingInstallations`, so `go test ./cmd/...` launched a real browser at `https://github.com/apps/fabrik/installations/new` on every run — including inside every Fabrik stage worker's own test run, which is how it surfaced. Naming a hazard in a doc comment does not retire it: the remaining call sites have to be swept. #1781 sets `NoBrowser: true` at all seven and adds a guard test that asserts through this ADR's `OpenBrowser` seam rather than by observing the absence of a window.
