...
**Update (2026-08-09, #1462):** #1279 was ultimately abandoned rather than merged, and its draft `adrs/1274-...` never landed on `main`. The "diff obtained but exceeds `max_diff_bytes`" path this ADR flagged as a coordination point was instead re-filed and implemented as #1462 — see [adrs/1462-pruefer-per-file-diff-exclusion-and-trim.md](1462-pruefer-per-file-diff-exclusion-and-trim.md), which reuses this ADR's `diffTooLargeMarker`/`alreadyNoticedTooLarge` in place (same file, not a merge conflict) exactly as anticipated above. This ADR's own decisions — the 406 classification, the files-API fallback, "fallback is path-list-only" — are unaffected and remain as-built; only the "reconcile at merge time" expectation is now resolved.
