...
**Positive:**
- Pruefer is *intended* to be genuinely self-hostable by anyone with zero shared credential: each user's first run creates and owns their own dedicated App, with no private key ever leaving their machine and no Pruefer-hosted service involved. **Correction (#1711, 2026-09-13):** this was never proven in production. `buildManifest` emitted a `hook_attributes` object without the `url` sub-key GitHub requires whenever `hook_attributes` is present at all, so GitHub rejected every real submission of the manifest outright (with a misleading `"url" wasn't supplied` error that pointed at the wrong field). Every test in `internal/githubauth` validates only against this package's own `httptest` mocks, never against GitHub's actual manifest validation, so nothing in the suite could have caught it. `handarbeit-pruefer`, the App this ADR's compat path depends on, was therefore created by hand rather than through `RunManifestFlow` — corroborated by its live permission set (`GET /app`: `issues: read`) differing from `buildManifest`'s requested set exactly on `issues` (the code requested `write`). Fixed in #1711 by omitting `hook_attributes` entirely, which Pruefer V1's polling-only design (ADR-1113 §1, ADR-032) makes correct as well as accepted.
