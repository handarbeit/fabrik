...
- **App deleted externally** (`AppID` and PEM both load fine, but `FetchAppSlug` fails against GitHub) → `Reconcile` logs why, then re-enters the manifest flow itself, preserving whatever non-secret local config existed (the manifest flow's own all-or-nothing persistence means the old state is only replaced once the new App is fully created) rather than silently creating a duplicate App or simply giving up.
