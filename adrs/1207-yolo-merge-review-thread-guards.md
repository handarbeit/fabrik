...
- Guard 1's primary call site (`handleStageComplete`, synchronous with the just-completed Claude invocation) reuses the pre-invocation item snapshot — the same staleness `handleStageComplete` already documents for its `wait_for_reviews` handling — so it can be tens of minutes stale on a long Validate run, not just a few seconds. A thread posted during that run can slip through guard 1 at this call site. This was deliberately not closed by re-fetching inside `attemptMergeOnValidate` (that would reintroduce the per-completion GraphQL round-trip Path 1 exists to avoid); it collapses into the same accepted `PollSeconds`-bounded residual race above, since guard 2 catches it on the very next poll using deep-fetched, fresh data. Flagged by Pruefer during review; see `docs/state-machine.md` §2.9 for the full explanation.
