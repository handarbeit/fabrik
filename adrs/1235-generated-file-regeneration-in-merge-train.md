...
- **Unattended execution of the declared regeneration command (review question, accepted tradeoff):** `regenerateAndCommit` runs the declared command against the trial worktree with no reasoning step in between, unlike the Claude-dispatch path — a PR that modifies `scripts/generate-llms-full.sh` cleanly (no conflict on the script itself) means that modified script could be executed unattended the next time any batch member's conflict is confined to `docs/llms-full.txt`, before `docs-drift.yml` gets a chance to catch a bad *artefact*. This is accepted as consistent with, not additional to, the merge train's existing trust model: every batch member reaches `Queued` only after its own `Validate` stage has already run `docs-drift.yml` unconditionally on that PR (ADR-1234) — meaning any modification to the regen script has already been executed once, by that exact same command, before the PR could land in the batch at all. `regenerateAndCommit` re-running it against the assembled multi-PR tree is the same class of unattended execution the merge train already performs for every batch (`go build`, `go test`, `go vet`, and `docs-drift.yml` itself all run against the assembled trial branch during combined Validate with no additional human gate). Sandboxing or gating this one command specifically would be inconsistent with — and disproportionate to — how the rest of the pipeline already trusts repo-tracked commands from already-reviewed PRs.
