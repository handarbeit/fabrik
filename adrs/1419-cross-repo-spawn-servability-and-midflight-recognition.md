...
On success, a present-tense receipt note (`formatMidflightSpawnReceiptNote`) is prepended to `output` — a sibling of Plan's deferred-tense `formatSpawnReceiptNote`, since these children already exist by the time the note is rendered, unlike a Plan declaration that defers creation to Implement. On failure, `spawnChildren` has already paused the parent and posted its own comment; `finalizeStageOutcome` releases the lock and returns without posting this dispatch's own output, mirroring the `FABRIK_PR_CREATE` failure precedent exactly.

**A same-turn hazard at Validate, not fixed by this ADR.** `attemptMergeOnValidate` (`engine/stages.go`) does not consult `item.BlockedBy`/`fabrik:blocked` before enabling auto-merge or performing a direct merge on a `fabrik:yolo` issue, and runs *before* `checkDependencies` is ever reached for that dispatch (`checkDependencies` only gates the `shouldAdvance` branch, which a successful auto-merge/direct-merge always short-circuits to `false`). A Validate output that both spawns a genuine blocker and signals `FABRIK_STAGE_COMPLETE` in the same turn could therefore have its PR merged before the freshly-created dependency edge is ever consulted. This was deliberately left as a prompt-level mitigation (`fabrik-validate/SKILL.md` instructs the agent not to complete alongside a genuine spawn — consistent with Validate's own "unmet requirement → BLOCKED" completion criteria) rather than an engine-side hard stop, matching the issue's own scope boundary against a new engine-side reconciliation backstop. See `docs/state-machine.md` §6.7.2 for the full mechanics and the reasoning for why Review carries no equivalent hazard (its next-stage dispatch is gated by `checkDependencies`).
...
- The Validate same-turn merge hazard (`attemptMergeOnValidate` not consulting `BlockedBy` before merging) remains architecturally present after this ADR — mitigated at the prompt level only. A future engine-side hard stop (e.g., re-checking `BlockedBy`/`fabrik:blocked` immediately before `attemptMergeOnValidate` acts) is a candidate follow-up if prompt-level steering proves insufficient in practice.
