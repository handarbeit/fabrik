...
3. **Check C — CI has already run against the current base.** Compares the base branch's own commit time (`git log -1 --format=%ct origin/$base_branch`) against the *earliest* start time among all of the PR's currently-successful *required*-check runs — queried via `gh api graphql`'s `isRequired(pullRequestNumber:)` field on the PR's last commit's `statusCheckRollup` contexts (not `gh pr view --json statusCheckRollup`, which has no required/non-required distinction — see "Check C's required-check filter" below), filtered to `isRequired==true` and every required check currently successful, minimum `startedAt` (not `completedAt` — see "Check C: completion time vs. start time" below; not the most recent `startedAt` either — see "Check C: one fresh required check does not mean all required checks are fresh" below), converted to epoch seconds. If that earliest start time is after the base commit, skip. On read failure, an empty result, a required check that isn't (yet) successful, or an earliest start time at or before the current base, **do not skip** — fall through to the rebase.
4. **Immediately before rebasing, re-verify Check A.** Checks B and C each cost real wall-clock time, during which the PR could newly enter the merge queue — see "Re-verifying Check A immediately before the push" below.
5. **Otherwise, rebase** — same command as before.
...
### Check C: bounding the `statusCheckRollup` page

`contexts(first:100)` caps the GraphQL read at 100 status/check contexts per PR. A fifth round of pre-merge review flagged this as a silent-truncation risk: if a PR's rollup ever exceeds 100 contexts (plausible on a monorepo with many workflows plus bot/status checks), a required check on the dropped tail simply isn't in `$required` at all — and because the count-equality test (`$required | length` vs `$ok | length`) only ever compares what the query actually saw, a truncated read can pass that test and report a false "fresh," the exact class of bug every prior Check C revision in this ADR exists to close.

The query now also reads `contexts.pageInfo.hasNextPage`, and the jq filter emits nothing at all when it's `true` — a rollup that doesn't fit on one page is treated as an incomplete read, matching every other read-failure case in this check, rather than silently evaluated against whatever fit on the first page. A full paginated read (following `endCursor` until `hasNextPage` is `false`) was considered and rejected for this pass: 100 contexts already exceeds any real PR on this repository by a wide margin, and the single-page-plus-truncation-guard version gets the same safety property (never compute freshness from a partial view) with one query instead of an unbounded number.
...
### Re-verifying Check A immediately before the push

A fifth round of pre-merge review pointed out a TOCTOU gap: Check A's `isInMergeQueue` read happens once, at the top of Step 1, before Checks B and C run — and each of those makes its own `gh api`/`git` call, which takes real wall-clock time. A PR could enter the merge queue in that window, after Check A already returned `false` and before the rebase (and the push it implies) actually happens. Given the ADR frames queued-PR ejection as the one outcome in this whole design that nothing downstream can undo, a stale Check A read defeats the purpose of having Check A at all in exactly the case it exists to prevent.

The fix re-runs Check A's identical query one more time, immediately before the `git rebase` call, applying the same fail-toward-skip default as the original read. This is deliberately not folded into a loop or a single shared check earlier in Step 1: Checks B and C both need to run regardless (their own outcomes are still meaningful even if Check A would now skip), and the point of the second read is specifically to close the gap between "last confirmed not-queued" and "about to push," which by construction has to be the last thing checked before that push. The added cost is one more cheap, PR-scoped GraphQL call — the same call already made once per invocation, now made twice in the (common) case where neither Check B nor Check C skips.
...
- Up to three additional `gh api graphql` calls per Validate invocation (Check A's `isInMergeQueue` query, Check C's `statusCheckRollup`/`isRequired` query, and Check A's pre-rebase re-verification of the same `isInMergeQueue` query — only reached when neither Check A, B, nor C already skipped) on top of the existing `gh pr view`/`git fetch`. All are cheap, read-only, PR-scoped (not repo-admin-scoped), and bounded by the same rate limits the worker already operates under.
