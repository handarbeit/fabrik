...
- **Resolved by #1829:** the age-bounded half of point 2's discriminator left a narrower window of the same underlying bug — a real `github-actions` suite whose first job has not yet been scheduled onto a runner is indistinguishable from an inert App suite except by age, and the incident this ADR was written from itself saw a 17-minute wait for a runner, well past any plausible fixed `PostPushDwell`. Once such a suite aged past the dwell with still-zero runs, it was misclassified as inert and stopped holding the gate — premature green, bounded to the post-dwell-but-pre-first-job window instead of the whole run. Resolved by special-casing `AppSlug == "github-actions"` as never inert while non-`completed` (`github.OutstandingCheckSuites`, #1829): GitHub creates exactly one check suite per workflow run, so a non-completed, zero-run `github-actions` suite always means a job hasn't been scheduled yet, never an idle installation, no matter how long that takes. Every other App slug keeps the original age-bounded rule from point 2 unchanged, so the no-deadlock guarantee for genuinely inert App installs (`cursor`, `claude`, `coderabbitai`, …) is untouched. The companion no-deadlock concern — could a `github-actions` suite itself sit non-`completed` with zero runs forever, e.g. an all-skipped-jobs workflow? — was investigated but not resolved by a live-captured fixture: a real recording requires an open PR to trigger this repo's `pull_request`-scoped CI, which does not exist at Implement time, so the closest verified analog was used instead (a workflow that fails at parse/startup time settles `completed`/`failure` with zero check runs promptly, rather than hanging non-`completed` — the same "zero jobs ever ran" shape). Even in the unverified worst case, the pre-existing `CIBackstopTimeout` backstop (point 6, unchanged) bounds the exposure to a pause requiring human unblock rather than an unbounded hang.
