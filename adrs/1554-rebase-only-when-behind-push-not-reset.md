...
- **The "Before You Start" behind-check and rebase in both skills are hardcoded to `origin/main`, not the resolved `base:<branch>` target.** This predates this fix — the original unconditional `git rebase origin/main` was already hardcoded this way — and is unrelated to R1/R2/R5's actual bug (the reset-on-divergence loop reproduces identically regardless of which branch is the rebase target). `base:<branch>` resolution is explicitly out of scope for this issue (see Scope). It is also inconsistent with this same file's later Pre-Completion Gate, which resolves `$base_branch` dynamically via `gh pr view --json baseRefName --jq .baseRefName` — flagged by Pruefer's review of #1554 itself. Fixing it properly means threading dynamic base-branch resolution through the earlier rebase point in both skills, which is new scope, not a fix to this issue's bug; left as a candidate follow-up rather than fixed here.
