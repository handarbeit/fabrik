...
> **Amended by [ADR-1642](1642-pruefer-repo-resident-config-base-ref.md):** the "no per-repo override" statement above is superseded for this field specifically. A repo-resident `.pruefer/config.yaml`, resolved at the PR's base ref, may now narrow (never widen — see ADR-1642's `narrowingRank`, deliberately distinct from this ADR's `severityRank`) the threshold this Config field ultimately holds for a given review. Every other setting this ADR and `Config` define remains exclusively operator-scoped.
