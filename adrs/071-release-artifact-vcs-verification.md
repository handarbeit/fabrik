...
- The Plan for this issue also called for a short cross-reference note in `.claude/skills/cut-release/SKILL.md`. That edit was skipped during Implement — a harness permission policy denied modification of files under `.claude/skills/` — so `docs/USER_GUIDE.md` alone carries the guard's documentation; it is the canonical doc the issue requires.
