...
**Concurrently-running invocations share one process-table scan, not one each.** `trackWorkerDescendants`'s per-tick discovery goes through `sharedProcessTableScan` (`sessionScopedDescendants`, the uncached variant, is reserved for this mechanism's own tests), a package-level cache keyed on `descendantScanInterval` as its TTL: the first goroutine to tick after the cache goes stale performs the real `ps -eo pid=,args=` scan and repopulates it, every other goroutine ticking within the same window reuses that result. Without this, one goroutine per in-flight invocation meant `MaxConcurrent` redundant full-table scans every tick — a review finding (`handarbeit-pruefer`) noting this was the same class of `ps` cost R4's investigation found responsible for real CPU spikes elsewhere in this mechanism (the `lsof` finding filed as #1805). Mirrors `dispatchCandidates`'s existing "fetch once, reuse across many checks" shape (`poll.go`, via `listProcessArgvFn`) but as a time-based cache rather than a single-call memoization, since the sharing here spans concurrently-running, independently-ticking goroutines over the life of several invocations rather than one function call.
...
The same principle recurred a fifth time, on the write side rather than a `ps`-read side: `trackWorkerDescendants` used to mark a newly-discovered descendant's PID `seen` unconditionally, discarding `upsertTrackedDescendant`'s own error. A transient registry write failure (e.g. `.fabrik/state/` momentarily unwritable under the same host contention this reaper exists to handle) meant the descendant was never actually persisted, yet `seen` permanently blocked it from ever being reprocessed on a later tick — invisible to both R2 and R3 for the rest of the invocation. Found in review (`handarbeit-pruefer`) during Validate; fixed by only marking a PID `seen` after a successful upsert, leaving it unmarked (for a later-tick retry) otherwise.
