...
- [ADR-1072: Holding Stages Are Reachable Only Via Dedicated Code, Not Positional Advance](1072-holding-stage-terminal-advance.md) — later revised the blanket `stage.HoldingStage` exclusion decided here into a conditional one (skip only while a merge-train worker is actively in flight for the item's repo), after 17 closed items were found permanently stranded in the `Queued` holding stage with no drain. The blanket exclusion was a reasonable call given what this ADR excluded and why at the time (gate-checked-stage disjointness, not holding-stage semantics); the exclusion of holding stages themselves had no documented rationale and turned out to be an oversight, not a constraint.
