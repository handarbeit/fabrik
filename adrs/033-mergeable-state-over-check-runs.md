...
This was observed in production: issues #716 and #717 on a managed repo were blocked by a failing `Cleanup artifacts` check_run for hours even though all required checks were green and GitHub's own merge button was enabled. `mergeable_state` was `clean` throughout. The only way to resolve the block without Fabrik support was to manually remove `fabrik:awaiting-ci` and trigger the merge.
