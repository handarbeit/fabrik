...
This was observed in production: develop issues #716 and #717 were blocked by a failing `Cleanup artifacts` check_run for hours even though all required checks were green and GitHub's own merge button was enabled. `mergeable_state` was `CLEAN` throughout. The only way to resolve the block without Fabrik support was to manually remove `fabrik:awaiting-ci` and trigger the merge.
...
The `unstable` state is intentionally included in the shortcut: GitHub defines `unstable` as "non-required checks have failed, but all branch-protection-required checks are satisfied and GitHub allows the merge." Blocking on `unstable` would replicate the same over-aggressive gate that caused develop#716/717.
