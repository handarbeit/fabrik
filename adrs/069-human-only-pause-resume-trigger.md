...
- `github.IsBotLogin` was originally a reviewer-classification fallback, where a false positive (misclassifying a human as a bot) is harmless. It is now also load-bearing for this resume gate, where a false positive is not harmless: a human login that happens to match one of its suffix/prefix heuristics (e.g. `*-bot`, `copilot-*`) would be silently unable to resume a pause by commenting. Future additions to `IsBotLogin`'s pattern list should weigh this cost, not just the original fallback use (noted at the function's definition in `github/types.go`).
- **Recovery sequence during a runaway-loop condition is "pause, fix the underlying condition, then remove `fabrik:paused`" — not "pause, then reply."** Replying with a human comment authorizes `processComments` to run once with the full accumulated comment set (see Decision) *and* removes the pause label. If the condition that caused the bot ping-pong is still active, the very next bot comment dispatches normally through the unfiltered non-paused path (`engine/item.go`, `newComments > 0 ⇒ dispatch`) — re-arming the same loop this ADR was written to stop. This fix restores the operator's ability to *hold* a pause against bot chatter; it does not, by itself, make "reply to resume" a safe way to clear a still-active runaway condition. That is a circuit-breaker concern, explicitly out of scope here (see "Why is this only fix 1 of 4?" and #1089).
