...
- **Labels are state**: `fabrik:locked:<user>`, `fabrik:editing`, `fabrik:paused`, `fabrik:awaiting-input`, `fabrik:awaiting-review`, `fabrik:awaiting-ci`, `stage:<name>:in_progress`, `stage:<name>:complete`, `stage:<name>:failed`, `model:<name>`, `effort:<level>`, `fabrik:yolo`, `fabrik:cruise`, `fabrik:unrestricted`, `fabrik:extend-turns`, `base:<branch>`
  ...
  - `fabrik:bot-reprompted:<login>` — set by engine during Phase 1 of the bot-reviewer escalation ladder (fired at 1× `ReviewWaitTimeout` when all outstanding reviewers are bots); indicates a DELETE+POST re-request and `@mention` comment were sent to the bot; cleared when the gate cycle ends (bot responds, Phase 2 fires, or mixed/human timeout path fires). Never persists beyond the gate cycle.
