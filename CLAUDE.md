...
- `apiKeyHelper` (a `settings.json` key, not an env var) is refused outright: hard-fail at startup if present anywhere in the resolved settings chain, and per-invocation if present in a worktree's own `.claude/settings.json`
