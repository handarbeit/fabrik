...
- **Labels are state**: `fabrik:locked:<user>`, `fabrik:editing`, `fabrik:paused`, `fabrik:awaiting-input`, `fabrik:awaiting-review`, `fabrik:awaiting-ci`, `fabrik:awaiting-done`, `fabrik:awaiting-member-close`, `fabrik:awaiting-close`, `fabrik:awaiting-advance`, `fabrik:awaiting-runaway-alert`, `fabrik:awaiting-landing-verification`, `fabrik:credited-pr:<N>`, `fabrik:landing-verification-failed`, `fabrik:awaiting-placement`, `fabrik:blocked`, `fabrik:rebase-needed`, `fabrik:bot-reprompted`, `fabrik:children-spawned`, `fabrik:sub-issue`, `fabrik:claude-limit`, `fabrik:clear-claude-limit`, `fabrik:api-key-helper-detected`, `fabrik:tools-denied`, `fabrik:nondefault-base-pr-noted`, `fabrik:non-default-base-excluded`, `stage:<name>:in_progress`, `stage:<name>:complete`, `stage:<name>:failed`, `model:<name>`, `effort:<level>`, `review-authority:<mode>`, `expected-reviewers:<mode>`, `fabrik:yolo`, `fabrik:cruise`, `fabrik:unrestricted`, `fabrik:extend-turns`, `base:<branch>`, `fabrik:revalidate`
  ...
  - `fabrik:non-default-base-excluded` — set by engine (`routeQueuedGroup`/`filterNonDefaultBaseMembers`) on a Queued merge-train member whose `base:<branch>` label resolves (via `baseBranchForItem`) to something other than the repository default, gated on the label's own absence. Applied *before* the batch cap and before `dispatchMergeTrainWorker` snapshots `batchNumbers`, so the excluded member never occupies a batch-cap slot and never appears in any worker's batch — the merge train has no way to correctly land a member targeting a different base than the rest of the batch (single-base-per-batch design), so it is excluded from batching entirely rather than silently mis-based against the default (the bug reported in #1646). A `base:` label naming a nonexistent branch resolves to the default via `baseBranchForItem`'s own fallback and is correctly **not** excluded. Fail-closed (excluded, no comment yet) when the repo's `WorktreeManager` isn't registered yet. A one-time explanatory comment is posted, gated on the label's own absence — the `fabrik:awaiting-ci` non-spamming convention; Queued items are shallow-fetched with no reliable `item.Comments`, so the label itself (reliably present even shallowly) is the idempotency signal, not a comment-content scan. Never applies `fabrik:paused` or `stage:*:failed` — the member stays untouched in `Queued` for a manual merge (R4). Self-clears on the next poll the exclusion no longer applies (`routeQueuedGroup` re-evaluates every Queued item every poll) — no settle scan needed. See ADR-1647, #1647.
