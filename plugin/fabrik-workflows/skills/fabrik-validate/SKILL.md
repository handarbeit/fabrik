...
**Report observed values, never a verdict.** The same discipline that applies to narrating a rebase/reset outcome (see the "describe only what you actually did … describe the observed state without attributing one" rule earlier in this document) applies here too: this summary must state what was actually observed — `mergeable`, `mergeStateStatus`, and each required check's own reported conclusion — never paraphrase those observations into a verdict phrase like "All checks pass" or "Validation passed." A required check with no reported conclusion is a fact to state, not a gap to paper over with an optimistic summary. **Never assert that checks pass while any required check has not reported a conclusion** — say plainly that it hasn't, and that the engine's own CI gate (`wait_for_ci` / `fabrik:awaiting-ci`) is what decides next, not this prose.
