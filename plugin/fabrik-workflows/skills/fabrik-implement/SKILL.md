...
- **Run `.fabrik/verify.yaml` if present.** When the project declares a behavioral gate (see the `fabrik-setup` skill for the contract), run its default `command`. A non-zero exit is a failing build — treat it like a failing test, not a warning.
