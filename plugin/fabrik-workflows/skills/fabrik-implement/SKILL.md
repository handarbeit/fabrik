...
Passing CI is necessary but **not sufficient**. **CI-green ≠ behavior-correct** — code that compiles and passes unit tests can still leave the feature completely broken. The classic failure: a fix ships green CI with zero effect on the running system because every test bypassed the one line that was actually broken. Guard against it:

- **Drive the real production entry point.** At least one test must exercise the actual seam the feature flows through — the reader, route handler, loader, schema validator, or CLI entry point — not a hand-built stand-in. Tests that construct the inputs a parser/reader/schema *would have produced* and feed them straight to the next layer have bypassed the code that can break. Feed the real artifact (the real sidecar, the real request body, the real file) through the real boundary.
- **Distrust silent-success.** Any `if (!ok) return` / `continue` / `break` (or equivalent) that drops data or skips work *without logging* is a candidate bug — it makes a failure invisible. Add a log line so the drop is observable, then confirm the data you expected actually survives the boundary.
- **Run `.fabrik/verify.yaml` if present.** When the project declares a behavioral gate (see the `fabrik-setup` skill for the contract), first check that all the env vars in its `needs:` are set. If they are, run its default `command`: a non-zero exit is a failing build — treat it like a failing test, not a warning. If any `needs:` var is missing, the command would fail spuriously — don't run it; take the honest-unverified path below instead.
- **Be honest about what you couldn't verify.** If the behavioral check can't run because a required `needs:` secret or tool is unavailable, say so explicitly in your Implement summary — state what was *not* verified and why. Never imply a behavioral check passed when it didn't run.

**Scope:** this applies to **behavioral changes** — anything that alters runtime behavior. Pure documentation changes and refactors already covered by existing tests are exempt; don't manufacture a behavioral check where nothing behavioral changed.
