...
func Execute() error {
	...
	flag.StringVar(&cfg.HookdeckAPIKeyEnv, "hookdeck-api-key-env", "", "Name of the environment variable holding the Hookdeck API key (only used when --event-source=hookdeck; default FABRIK_HOOKDECK_API_KEY; also FABRIK_HOOKDECK_API_KEY_ENV)")
	flag.StringVar(&cfg.HookdeckWebhookSecretEnv, "hookdeck-webhook-secret-env", "", "Name of the environment variable holding the GitHub App's webhook secret (only used when --event-source=hookdeck; default FABRIK_HOOKDECK_WEBHOOK_SECRET; also FABRIK_HOOKDECK_WEBHOOK_SECRET_ENV)")
