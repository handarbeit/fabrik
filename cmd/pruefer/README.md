...
**Rollback position, stated once up front:** every step below, up to and including the step immediately before deletion, is safely reversible or pausable — the old App keeps its existing installations and keeps reviewing PRs on every account it's still installed on, right up until the final destroy-last step actually deletes it. If a new daemon misbehaves mid-migration, you can stop, leave both identities dual-listed, and re-install the old App on any account you backed out of. Nothing is irreversible until the last step.
