...
1. Any path matching `excluded_paths` is filtered out of the diff *before* anything is measured against `max_diff_bytes` — an excluded file can never consume the size budget or suppress review of the rest, no matter how large it is on its own. If every touched file is excluded and what's left (the diff's unattributed preamble, normally empty) still fits under `max_diff_bytes`, the PR is skipped (nothing left to review) — silently on an ordinary poll, but with a one-time explanatory PR comment when the skip is the direct result of a forced `/pruefer review`. If the leftover preamble alone is still over `max_diff_bytes`, this falls through to the too-large notice in step 3 instead — an all-excluded diff is never a way to dodge that notice.
...
**Known limitation:** the per-file size handling above only recognizes git's unquoted `diff --git a/<path> b/<path>` header form. Under the default `core.quotepath=true`, a touched file whose path contains a space, double quote, backslash, or non-ASCII byte makes git emit a C-quoted header instead, which Pruefer doesn't currently parse — that file's whole diff block is counted toward `max_diff_bytes` but can't be matched by `excluded_paths` or dropped by the best-effort trim in step 2. Rename such a path (or avoid the affected characters) if you need it excludable. Tracked in [adrs/1274-pruefer-per-file-diff-exclusion-and-too-large-notice.md](../../adrs/1274-pruefer-per-file-diff-exclusion-and-too-large-notice.md).
