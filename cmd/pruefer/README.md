...
**Since #1631**, that GitHub-derived check is backed by a second, additive, in-memory record of Pruefer's own successful review submissions, checked first and independently of GitHub. It exists because a GitHub outage can make `FetchPRReviews` return a *successful but incomplete* response that omits the bot's own latest review at the current head — indistinguishable, from the GitHub-derived check alone, from "no prior review exists" — which drove an observed 14-review loop on a single unchanged commit during a real incident. The local record can't be fooled by that, because it doesn't ask GitHub at all once it already knows. This is strictly additive to the guarantee above: it is process-lifetime only (a restart clears it, and Pruefer falls back to exactly the pre-#1631, GitHub-derived-only behavior — never worse than before, only better within one process's uptime), and `/pruefer review` bypasses it exactly as it bypasses the GitHub-derived check. See [adrs/1631-pruefer-local-review-tracker-backstop.md](../../adrs/1631-pruefer-local-review-tracker-backstop.md).
...
1. Pruefer starts a temporary local HTTP listener and opens your browser to a page that hands GitHub a pre-filled manifest — scoped to exactly the four permissions below, with webhook delivery disabled (Pruefer polls; see ADR-1113/ADR-032) and `hook_attributes` omitted entirely, not merely left at a default. GitHub requires `hook_attributes.url` whenever `hook_attributes` is present at all, rejecting the whole manifest otherwise, and reports the rejection misleadingly as `"url" wasn't supplied` rather than naming the missing field (see [#1711](https://github.com/handarbeit/fabrik/issues/1711)) — so this flow has no way to carry a webhook URL through it at all. If you already know you want `event_source: hookdeck` (see [Event-driven mode (Hookdeck)](#event-driven-mode-hookdeck) below), this flow's inability to carry a webhook URL means you'll need either [Manual setup (compat mode)](#manual-setup-compat-mode) or a follow-up trip to the App's Settings page — its webhook step covers both, and notes that unlike a permission change, timing doesn't affect cost: a webhook's config takes effect immediately for every installation whenever it's set. If a browser can't be opened (e.g. an SSH/headless session), or you'd rather it never tried, Pruefer prints the URL to open manually instead — pass `-no-browser`, set `PRUEFER_NO_BROWSER=1`, or `no_browser: true` in config to always skip the browser-open attempt.
...
2. Fill in:
   ...
   - **Webhook**: depends on whether you already know you'll use `event_source: hookdeck` (see [Event-driven mode (Hookdeck)](#event-driven-mode-hookdeck) below).
     - **Not planning to use Hookdeck (the default)**: **uncheck "Active"**. Pruefer polls by default; it does not receive webhooks (see ADR-1113 for why this doesn't conflict with ADR-032's webhook-delivery ruling).
     - **Already know you want Hookdeck**: you can declare the webhook **now, at creation**, or leave it unchecked and come back once a Hookdeck source is provisioned — both are equally fine. Unlike a *permission* change (see "Verifying granted permissions" below), GitHub applies a webhook's URL, content type, secret, and "Active" state to every installation **immediately, with no per-installation approval step**, whether you set it now or edit it later — there is no cost difference to timing. Declaring it now just saves a return trip to Settings: check **"Active"** and fill in the Webhook URL, content type, and secret exactly as described in [Provisioning a Hookdeck source](#provisioning-a-hookdeck-source) below — provisioning the Hookdeck source first, then returning to finish this form, is fine either way; App creation isn't time-limited the way the manifest flow's callback is. GitHub does reject "Active" checked with no URL supplied — the same `hook_attributes.url` requirement the App-Manifest flow runs into above, whose rejection there is confusingly worded as `"url" wasn't supplied` rather than naming the missing field (see [#1711](https://github.com/handarbeit/fabrik/issues/1711)) — so don't check Active until you have a real URL to give it.
...
3. **Register a new, private App per additional account (one App per account, per [ADR-1722](../../adrs/1722-app-installation-trust-boundary.md)).** The owning account (`handarbeit`, for the shared App's own history) keeps the original App; every other account (e.g. `verveguy`, `liminisapp`, `shadoworg`) gets its own. For each: run [First run: automatic setup](#first-run-automatic-setup-recommended) in its own working directory, giving it a distinct, globally-unique `github_app_name` (e.g. `verveguy-pruefer`) and a matching `github_app_homepage_url` (see [Configuration reference](#configuration-reference) — `github_app_name` defaults to `pruefer`, which collides across deployments if left unset). That flow's own last action is to print an install link and have you follow it — going ahead and installing the new App on the target account at this point is fine; installing early creates no hazard on its own (see step 5's note below on what the actual hazard is).
   ...
   **If this account is going to use `event_source: hookdeck`:** [First run: automatic setup](#first-run-automatic-setup-recommended) above, which this step uses, structurally cannot carry a webhook URL through it: its manifest always omits `hook_attributes` entirely, whether the App is installed yet or not. Unlike a permission change, a webhook's URL, content type, secret, and "Active" state take effect immediately for every installation with no approval step — so there's no need to rush this before installing; do whichever of the following is convenient, now or later:
   - Use [Manual setup (compat mode)](#manual-setup-compat-mode) instead of the automatic flow for this one App, and declare the webhook on GitHub's creation form per that section's webhook step; or
   - Let the automatic flow create the App as usual, then go to the new App's settings page on GitHub at any time and configure the webhook there (same fields: Active, Webhook URL, content type, secret — see [Provisioning a Hookdeck source](#provisioning-a-hookdeck-source) below).

   If this account isn't using Hookdeck, or you haven't decided yet, there's nothing to do here — proceed as above.
...
3. Set the API key in your environment under whatever variable name you configure via `hookdeck.api_key_env` (default `HOOKDECK_API_KEY`) — e.g. in `.env`:
   ...
   ```
4. **If you already declared the webhook at App creation** (via [Manual setup (compat mode)](#manual-setup-compat-mode)'s webhook step, or the [migration runbook](#migration-runbook-public-multi-account-app--private-per-account-apps)'s equivalent step), this is likely already done — open the App's settings and confirm the values below match rather than re-entering them, then skip to step 5. The rest of this step is for an App that didn't declare a webhook at creation — an existing App predating this guidance, or one where Hookdeck wasn't decided yet at creation time — and needs one added now. That's no more costly than declaring it fresh would have been: unlike a *permission* change, GitHub applies a webhook's URL, content type, secret, and "Active" state to every installation immediately, with no per-installation approval step, however many accounts the App is already installed on. Go back to your GitHub App's settings (**Settings → Developer settings → GitHub Apps → your app → General**) and:
   ...
   Declaring the webhook and setting `event_source: hookdeck` (step 5 below) are deliberately independent: you can do the former at App-creation time (or, per above, later) and the latter — flipping this one config value — whenever the Hookdeck source is actually ready, not necessarily in the same sitting. Coupling an identity change (a new or updated App) with an ingestion-mode flip risks masking one problem with the other; Hookdeck's own historical failure mode here was silent degradation (see [ADR-1254](../../adrs/1254-event-driven-hookdeck-ingestion.md)'s consequences, fixed by [#1563](https://github.com/handarbeit/fabrik/issues/1563)) — keeping the two changes out of the same sitting avoids reproducing that.
...
When run with a real terminal attached (both stdin and stdout), Pruefer launches an interactive TUI by default — the same `bubbletea`/`bubbles`/`lipgloss` stack and model/update/view structure as Fabrik's own `tui/` package, so the two feel like the same family of tool. The layout fills the terminal — panes stretch or shrink to the current window height instead of rendering a fixed-size block — and is ordered so the panes that change sit above the one that doesn't: In-Flight Reviews, then Completed Reviews, then Watched Repos, then the footer. It shows:

- PRs currently under review, with elapsed time since the review started. This pane is never truncated by the layout.
- Recently completed reviews (last 200, in-memory — not persisted across restarts): repo, PR, outcome (reviewed or errored), turns, cost, and duration, aligned into columns. **Skipped PRs are filtered out of this list by default** — at a typical repo count, "already reviewed at this head SHA" and the other skip categories (draft, self-authored, excluded author/label/path, diff too large) are the overwhelming steady-state outcome, and listing them evicted the reviews that actually carry information. The pane's title discloses the filtered count instead (`Completed Reviews (7) · 22 skipped`); a review that errored is never filtered, even if it was also a skip. When the terminal is too short to show every completed review, the list windows around your selection with a "… N more" line rather than overflowing.
- Watched repositories, each with its last poll time, PR count found, last error (if any), and the installation that granted it — plus, when relevant, a warning if the last derivation hit a pagination ceiling or the `max_derived_repos` cap, and a count of `watched_repos` entries not covered by any installation (see [Installation-derived repo discovery](#installation-derived-repo-discovery)). As the panel lowest in priority, this is the one that shrinks first on a short terminal, windowing with "… N more" once space runs out.
...
**The base-ref rule.** Pruefer resolves a repo's config by reading `.pruefer/config.yaml` from the PR's **base ref** — via the GitHub Contents API, not from the local clone — never from the PR's head. This is the same doctrine behind Pruefer's `--setting-sources` handling: the reviewer loads **no** settings layer at all, so neither the reviewed repo's `.claude/settings.json` nor the operator's own profile can influence a review (see [How it works](#how-it-works) and [adrs/1113-pruefer-v1-architecture.md](../../adrs/1113-pruefer-v1-architecture.md)): a repo-resident file under review is untrusted input from the artifact being reviewed, and the base ref is the one input a PR cannot itself edit.
