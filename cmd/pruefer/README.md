...
2. Fill in:
   ...
   - **Webhook**: depends on whether you already know you'll use `event_source: hookdeck` (see [Event-driven mode (Hookdeck)](#event-driven-mode-hookdeck) below).
     ...
     - **Already know you want Hookdeck**: declare the webhook **now, at creation**, rather than coming back to it later. Check **"Active"** and fill in the Webhook URL, content type, and secret exactly as described in [Provisioning a Hookdeck source](#provisioning-a-hookdeck-source) below — provisioning the Hookdeck source first, then returning to finish this form, is fine; App creation isn't time-limited the way the manifest flow's callback is. Declaring it now is free: GitHub applies a freshly created App's webhook configuration to every installation with no approval step. Coming back later and pointing an *existing* App's webhook at Hookdeck, after installations already exist, is not free — changing an App's webhook configuration or its `default_events` requires each installation's owner to separately approve the change, one installation at a time. That's the same per-installation-approval mechanism "Verifying granted permissions" below documents for permission grants, and just as easy to leave silently unapproved on some installations. GitHub also rejects "Active" checked with no URL supplied — the same `hook_attributes.url` requirement the App-Manifest flow runs into above, whose rejection there is confusingly worded as `"url" wasn't supplied` rather than naming the missing field (see [#1711](https://github.com/handarbeit/fabrik/issues/1711)) — so don't check Active until you have a real URL to give it.
...
3. **Register a new, private App per additional account (one App per account, per [ADR-1722](../../adrs/1722-app-installation-trust-boundary.md)).** The owning account (`handarbeit`, for the shared App's own history) keeps the original App; every other account (e.g. `verveguy`, `liminisapp`, `shadoworg`) gets its own. For each: run [First run: automatic setup](#first-run-automatic-setup-recommended) in its own working directory, giving it a distinct, globally-unique `github_app_name` (e.g. `verveguy-pruefer`) and a matching `github_app_homepage_url` (see [Configuration reference](#configuration-reference) — `github_app_name` defaults to `pruefer`, which collides across deployments if left unset). That flow's own last action is to print an install link and have you follow it — going ahead and installing the new App on the target account at this point is fine; installing early creates no hazard on its own (see step 5's note below on what the actual hazard is).
   ...
   **If this account is going to use `event_source: hookdeck`, declare the webhook now — before following that install link.** [First run: automatic setup](#first-run-automatic-setup-recommended) above, which this step uses, structurally cannot carry a webhook URL through it: its manifest always omits `hook_attributes` entirely. Two ways to still get the free, at-creation declaration, both still free because they happen before this new App is installed anywhere:
   ...
   - Let the automatic flow create the App as usual, then — before following the printed install link — go to the new App's settings page on GitHub and configure the webhook there (same fields: Active, Webhook URL, content type, secret — see [Provisioning a Hookdeck source](#provisioning-a-hookdeck-source) below).

   Either way, finish this before installing the App on the account. If this account isn't using Hookdeck, or you haven't decided yet, there's nothing to do here — proceed as above.
