...
|---|---|---|---|
| https://handarbeit.io | `handarbeit/handarbeit.io` (private) | `/Users/bpja/dev/handarbeit.io` | `main /` |
| https://fabrik.handarbeit.io | `handarbeit/fabrik` | `/Users/bpja/dev/fabrik` | `main /docs` |
...
- **Emblem:** `/Users/bpja/dev/handarbeit.io/assets/handarbeit.png` — transparent background, must keep transparent. The handarbeit.io landing page emblem links to `https://fabrik.handarbeit.io`.
...
Always commit as `arbeithand`, never as the default `gh` CLI user (`verveguy`):
...
- Token: `FABRIK_TOKEN` in `/Users/bpja/dev/fabrik/.env` (a classic PAT for the `arbeithand` account, scopes: `admin:org, project, repo, workflow, write:discussion`)
- For `gh` CLI: `GH_TOKEN=$(grep '^FABRIK_TOKEN=' /Users/bpja/dev/fabrik/.env | cut -d= -f2-) gh ...`
...
3. Verify:
   ```bash
   GH_TOKEN=$(grep '^FABRIK_TOKEN=' /Users/bpja/dev/fabrik/.env | cut -d= -f2-) \
...
git -C /Users/bpja/dev/handarbeit.io log --oneline -5
git -C /Users/bpja/dev/fabrik log --oneline -- docs/ | head -5
```
