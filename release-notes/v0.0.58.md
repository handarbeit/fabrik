...
Cold-start cache population is now cheap (probe-driven, no deep-fetch on warm-up). TUI gains a rate-limit-exhaustion alert with countdown, `ctrl+r` force-refresh, and a non-blocking `u` key for plugin-skills upgrade.
