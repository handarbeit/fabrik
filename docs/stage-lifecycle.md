...
For `read_only: true` stages (Research, Plan by default — Specify is write-capable as of #1655, since it commits `specs/<issue>-<slug>/spec.md` on the worktree branch): dirty state is auto-stashed before Claude runs and restored after. Claude sees a clean worktree.
...
**Scope:** Enforced for all stages where `read_only: false` — including Specify by default (as of #1655), since it commits the spec-file projection to the worktree branch. Skipped for read-only stages (Research, Plan by default) — they do not write files and receive bare `Edit`/`Write` entries (or none, if `allowed_tools:` is overridden in stage YAML).
