...
`FetchLinkedPR` matches by branch name (`fabrik/issue-N`) alone, which is not sufficient confirmation on a `base:<branch>` repo: `closingIssuesReferences` is structurally empty there for *every* PR, not just the linked one, so a stale or repurposed `fabrik/issue-N` branch carrying its own unrelated open PR would otherwise be trusted as this issue's PR. For an item carrying a `base:` label, `resolveFabrikEnvOpts` therefore performs the same confirmation `handleBrokenReviewLinkage` does: it calls `FetchPRClosingIssues` on the candidate PR and only trusts the match when the PR body actually closes this issue. A transient `FetchPRClosingIssues` error fails open (the branch-name match is still trusted, mirroring `handleBrokenReviewLinkage`'s own fail-open behavior) — only a *successful* fetch that doesn't list this issue withholds the PR number.
