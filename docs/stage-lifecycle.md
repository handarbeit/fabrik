...
Under App auth, the installation token is not granted `contents`, so it authorizes worker `gh` CLI calls but not worker `git fetch`/`push` over an HTTPS remote whose credential helper resolves from `GH_TOKEN`/`GITHUB_TOKEN`. `Run()`'s startup preflight (`RefuseHTTPSWorkerGitUnderAppAuth`, `engine/github_app_auth.go`) refuses to start in that combination — see ADR-1756 and `docs/USER_GUIDE.md`'s "Worker git under App auth" — so a running worker never actually hits this env's git path un-refused.
