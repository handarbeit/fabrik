...
Worker git inherits its credentials from the same base environment. Under App auth with HTTPS git, `Run()`'s startup (`setUpAppGitCredential`, `engine/app_git_credential.go`) injects a `credential.https://github.com.helper` into the engine's own process environment via `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_<n>`/`GIT_CONFIG_VALUE_<n>` — an empty reset entry followed by a helper reading `.fabrik/state/github-app-git-token` — so engine and worker git both authenticate as the installation regardless of host helpers, and pick up token rotations mid-invocation. `mergeEnv` passes these entries through unchanged. Under `git_ssh`/an SSH rewrite, or in PAT mode, no helper is injected and any left by a previous exec is stripped. See ADR-1846.
