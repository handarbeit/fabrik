...
**All release artifacts MUST be authored as `@arbeithand` (the bot).** The operator's default `gh auth` identity is `@arbeithand`, which is wrong for releases. The script enforces this; do not bypass.
...
- **Tag push**: PAT-in-URL with credential helpers nuked — `git -c credential.helper= -c credential.https://github.com.helper= push https://x-access-token:${FABRIK_TOKEN}@github.com/handarbeit/fabrik.git <tag>`. Without nuking the helpers, the default `gh auth` identity wins and the push is attributed to arbeithand.
...
- **Use `gh` without `GH_TOKEN=$FABRIK_TOKEN` for release-related operations.** The default `gh auth` token is arbeithand and will produce wrong-identity artifacts.
...
- **arbeithand**: the operator's personal GitHub identity. Default `gh auth` user. **Must not** appear on any release artifact.
