...
3. **Spec-mandated trust-posture declaration (§15).** `adopt` — Fabrik already has a posture (default allowlist + `fabrik:unrestricted`), but it is documented in `CLAUDE.md` rather than treated as a contract. Promoting it to a `docs/trust-posture.md` (or extending `state-machine.md` §X) would close the discipline gap. This is documentation work, not code work.
