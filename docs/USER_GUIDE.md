...
> **Startup requirement.** Whenever a `holding_stage: true` stage (e.g. `Queued`) is configured in `.fabrik/stages/`, its board column is **mandatory** — Fabrik fails startup if it is missing, the same board-validation that guards every non-cleanup stage. This holds **independent of `merge_train`'s value**: the column is required because the stage is configured, not because the train happens to be on at this particular startup. `merge_train` is a runtime-togglable setting that does not regenerate board columns, so a board validated while the train was off can strand merged, closed issues the instant it's turned on — the least recoverable point in the pipeline, since native dependency edges clear on *close*, not on *merge* (see #1082, ADR-1421). The train ships **off by default** because it's an opt-in change to landing behavior, not because of any startup-safety concern — complete step 1 before enabling it either way. The `Queued` column itself is **terminal-only**: it is never a valid column to move items into manually, and a Status of `Todo`/`Backlog`/null means Fabrik ignores the item entirely.
...
Leave an **inline review thread comment** on the specific line(s) you want changed, or write your feedback in the **top-level review body** — start a review on the PR (either commenting on specific diff lines, or just writing a summary, or both) and submit it. Fabrik picks up unresolved inline thread comments and unaddressed review bodies (for `CHANGES_REQUESTED` reviews only — an `APPROVED`, `DISMISSED`, or `COMMENTED` review's body is not treated as feedback to act on; `COMMENTED` is excluded even though it also carries a guaranteed body, since automated reviewers routinely submit a generic summary as one) and re-invokes the stage agent to address them, commit, and push.
...
If a submitted-review batch leaves nothing unresolved to process — no inline PR review thread comments **and** no unaddressed review body — Fabrik skips re-invocation entirely and advances the issue normally. An `APPROVED`, `DISMISSED`, or `COMMENTED` review's body is not treated as actionable regardless of its text (a body on an approval is typically "LGTM," not feedback to act on; a `COMMENTED` review's body is excluded too, since automated reviewers such as Copilot routinely submit a generic summary as one), so none of these trigger re-invocation on their own. A `CHANGES_REQUESTED` review's body, however, is always treated as actionable when non-empty — which, per GitHub's own requirement that this verdict carry a body, means such a review always triggers re-invocation even with zero inline thread comments.
