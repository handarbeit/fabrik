...
> [!NOTE]
> **Orphan webhook cleanup:** At startup, Fabrik automatically deletes any previous-session webhook subscriptions it created (identified by a Fabrik-specific description prefix). This prevents the `Hook already exists` HTTP 422 error that occurs when the `gh` subprocess crashed mid-session and the old subscription was never torn down. Operators do not need to manually delete stale webhooks between restarts.
...
| Reconcile interval | `--reconcile-interval=<N>` | `FABRIK_RECONCILE_INTERVAL` | — | 0 (180 seconds) |
...
|-----------|-------|---------|
| `● webhook (N)` (green) | Healthy | Last light reconcile found no drift. Full-board idle cap: 60 min; status sweep: 10 min. |
| `○ webhook (N)` (blue) | Starting Up | Subprocess launched; persists until first reconcile tick (default ~3 minutes). Full-board idle cap: 60 min; status sweep: 10 min. |
...
**Per-repo failure isolation.** When Fabrik is in per-repo subscription mode, a subscription failure for one repo (missing `admin:repo_hook` scope, deleted repo, etc.) quarantines only that repo for the session — the remaining repos continue receiving real-time webhook events normally. The quarantined repo falls back to the safety-net poll (same cadence as polling-only mode). To recover: fix the underlying issue (re-add token scopes, restore repo access) and restart Fabrik. The next startup re-attempts subscription for all repos, including any previously quarantined ones.
