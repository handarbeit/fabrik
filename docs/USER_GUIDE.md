...
3. **Phase 3 (re-invocation):** When the gate clears with submitted reviews present, Fabrik re-invokes the stage agent via the comment-processing skill (`comment_skill`) with the unresolved inline review thread comments **and** actionable top-level review bodies as input. Each inline thread comment is enriched with its **file path** and, when available, **line number** and **raw diff-hunk context** (line number and hunk may be absent for file-level or outdated comments) so the agent understands where in the code the reviewer's feedback applies. A review body has no file/line anchor, so it's tagged with the review's overall verdict and a `[Review: ...]` identifier instead. A review's body and its own inline comments are commonly both present — the agent is instructed to treat overlap between them as expected (a body finding that restates an inline one is the same finding, addressed once) rather than duplicate work, while a body finding with no inline counterpart (e.g. one the reviewer couldn't anchor to a diff line) must still be addressed. A pure verdict-only body ("LGTM", empty) is filtered out before this step and never triggers re-invocation on its own. The agent addresses the feedback, commits, and signals `FABRIK_STAGE_COMPLETE`. This re-applies `fabrik:awaiting-review`, and the cycle returns to Phase 2 until the gate clears again under the same dual condition — no requested reviewers outstanding **and** at least one review submitted — or, if that does not happen before the wait limit, Fabrik falls back to `fabrik:awaiting-input`. **As of v0.0.39, re-invocation is unconditional** — it fires for any issue with `wait_for_reviews: true` and submitted inline feedback, regardless of whether auto-advance is active.
...
If a submitted-review batch leaves no unresolved inline PR review thread comments **and** no actionable review-body text to process, Fabrik skips re-invocation entirely and advances the issue normally. A review body containing only a bare verdict (e.g. "APPROVED", "LGTM", or an empty body) does not by itself trigger re-invocation — it's filtered out mechanically before this decision, the same way a plain "no findings" comment would be. A review body carrying real feedback text — even with no accompanying inline thread comments, as happens when a reviewer's finding can't be anchored to a specific diff line — does trigger re-invocation on its own.

This prevents spurious re-invocation cycles when reviews contain no actionable feedback for the agent to address, while still surfacing feedback a reviewer could only express in the body.
