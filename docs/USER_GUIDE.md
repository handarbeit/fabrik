...
read_only: true           # Optional. Stashes the dirty worktree before Claude runs and
                          ...
                          #   modify files (e.g., Research). Note the shipped Specify
                          #   default is `read_only: false` — it commits a spec file.
...
allowed_tools:            # Optional. REPLACES the default tool set — not additive. When set,
  ...
  - Glob                  #   SDLC tools: Read, Edit, Write, Glob, Grep, TodoWrite, Skill, Task,
                          ...
                          #   Use this to restrict read-only stages (e.g. Research, Plan)
...
- Rewrites the issue body with a structured spec, in **GitHub Spec Kit format**
- Commits that spec to `specs/<issue-number>-<slug>/spec.md` on the issue branch
...
**The issue body is canonical; the committed file is its projection.** The file is written
on every round that changes the body — not only the round that completes the stage — so
the two never drift apart. `## Open Questions` appears in the body during clarification
and is stripped from the file; the file's `**Status**:` field reads `Draft` while any
question remains and `Specified` once none do. The directory slug is locked at first
commit, so a title changing mid-clarification does not orphan it.

Downstream stages continue to read `.fabrik-context/issue.md`. The committed spec is for
human readers and post-merge tracking, not a new input to Research or Plan.

**This requires a write-capable Specify stage, and upgrading will not give you one.** The
shipped default (`stages/examples/specify.yaml`) now sets `read_only: false` and allows
`Write`, `Edit`, and the narrow Bash verbs the procedure uses — `Bash(git rev-parse:*)`,
`Bash(git add:*)`, `Bash(git commit:*)`, `Bash(git diff:*)`, `Bash(gh issue view:*)`,
`Bash(find:*)`. Note the `:*` suffixes: an entry without one is an exact-string match, so
`Bash(git add)` would permit only the literal `git add` with no arguments. Your own
`.fabrik/stages/specify.yaml` is tracked in your repository and is never overwritten, and
`fabrik refresh-stages` only adds *missing* keys — it will not change a `read_only: true` you
...
| Section | Source |
|---------|--------|
| `## Summary` | Extracted from the issue spec (`.fabrik-context/issue.md`) — `## Summary` or `## Overview` |
| `## Problem` | Extracted from the issue spec — `## Problem`, `## Background` or `## Motivation`, so both the original and Spec Kit issue-body shapes work |
...
**`allowed_tools` replaces the defaults — it is not additive.** When a stage sets `allowed_tools`, only those tools are permitted; the default list above is not merged in. This is intentional: analysis stages such as Research and Plan set `allowed_tools` to a read-only subset to prevent Claude from writing files during those stages. Specify is **not** one of them — it ships `read_only: false` with `Write`, `Edit` and narrow Bash verbs so it can commit the spec file, as described in [The Specify Stage](#the-specify-stage).
