...
`handleRedBatch`, `bisect`, and `landOneAtATime` are otherwise unchanged; every batch with two or more members bisects exactly as described in the rest of this section. `landOneAtATime`'s own red-singleton-ejection branch (below, FR-5) is also unchanged — it is reached only after a genuinely multi-member batch degrades, which is real train churn, not the self-inflicted case this short-circuit addresses.
