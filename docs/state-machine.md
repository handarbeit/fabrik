...
**Comment text is the existing generic no-work-needed skip comment** (`_Skipped: no work needed (FABRIK_NO_WORK_NEEDED emitted by Implement)._`) — no bespoke "coordinator complete" message. The wording is not perfectly precise about *why* (the engine decided this, not a Claude-emitted marker), but it does not misstate the outcome, and reusing it avoids a second, less-tested completion path.
