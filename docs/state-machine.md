...
Unlike the other two paths, this one is **guarded on `fabrik:awaiting-landing-verification`'s absence** rather than posted unconditionally. `landMergeTrainBatch`/`landSingleton` close the member's PR immediately after commenting, so they can never be re-entered for the same member; this path leaves the PR to GitHub's own merge-close and can legitimately be re-entered while the member is still Queued (the `pr.Merged` restart branch above, and a retry after a failed `recordAdvanceOutcome`). The label is applied on every path that records a completed landing, so its presence means the comment has already posted. As with the other paths, the comment gates nothing: `addLandedCommentWithRetry` retries transient failures and then warns and continues — a landing is never blocked or delayed on it.
