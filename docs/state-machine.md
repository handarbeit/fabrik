...
| PR state change | `CommentBreakerObserver` (`engine/observers.go`), subscribed to `itemstate.Store`, reacting to any `LinkedPRChanged` flag | `Change.Fields` is diff-based — fires only on a genuine field change, never merely because the field is present. Deliberately broader than the literal "merged/closed/draft↔ready" (also catches review-request/check-run churn on the PR) — for a last-resort backstop, erring toward fewer false trips is the accepted trade-off, the same one ADR-070 made explicitly for bot-noise filtering. |
