...
**On success:** `formatMidflightSpawnReceiptNote(spawned)` — a present-tense sibling of Plan's `formatSpawnReceiptNote` (§6.7's on-thread spawn receipt) — is prepended to `output` before posting. Unlike Plan's note ("N sub-issues declared above... will be created when this issue advances to Implement"), this note describes work already done: the children already exist, are boarded, assigned, and linked as blockers, by the time the note is rendered. The stage's own narrative output (and, when present, `FABRIK_STAGE_COMPLETE`) is otherwise untouched and posted normally — a mid-flight spawn does not, by itself, suppress or force the stage's completion decision.
...
**Validate-specific hazard: do not complete and spawn in the same turn.** `attemptMergeOnValidate` (§6.6.6-adjacent merge logic, `engine/stages.go`) does not consult `item.BlockedBy`/`fabrik:blocked` before enabling auto-merge or performing a direct merge on a `fabrik:yolo` issue — it is reached from `handleStageComplete` before `checkDependencies` is ever called for that dispatch (`checkDependencies` only gates the `shouldAdvance` branch, and `autoMergeEnabled`/a synchronous direct merge both short-circuit `shouldAdvance` to `false`, skipping it entirely). This means a Validate output that both declares a genuine blocker **and** emits `FABRIK_STAGE_COMPLETE` in the same turn could have its PR merged before the freshly-created `blocked_by` edge is ever consulted — the engine applies no hard stop for this specific interaction. The mitigation is prompt-level (`plugin/fabrik-workflows/skills/fabrik-validate/SKILL.md`'s "If You Discover a Blocking Issue" section instructs the agent not to emit `FABRIK_STAGE_COMPLETE` alongside a genuine spawn), consistent with Validate's own "Decision: Complete or Block" criteria — a just-declared hard dependency is definitionally an unmet requirement, so the correct verdict is BLOCKED, not READY TO MERGE. Review carries no equivalent hazard: `checkDependencies` runs (and correctly gates) if `shouldAdvance` is true off a non-Validate stage, so completing Review while also spawning a blocker is safe — the next stage's dispatch will not proceed past `fabrik:blocked`.
