...
2. **FR-2 — Poll and merge**: `pollForMergeable` polls the integration PR's `mergeable_state` using `FetchPRMergeableFields` with the same 30s interval as `pollTrainCI`. When `mergeable_state ∈ {"clean", "unstable"}` (`gh.MergeableStateAccepted` — corrected from an earlier draft of this doc that said `"clean"` alone; `unstable` is accepted because it means only a non-required check is red/pending, not a required one), `MergePR` is called (no admin bypass). `MergePR` itself independently re-checks `mergeable_state` against the same allowlist before merging (see "`MergePR`'s own CI precondition (ADR-933)" in §5, after §5.3) — normally a no-op here since `pollForMergeable` already judged the PR acceptable, but it closes a narrow TOCTOU window if the state flips between the two checks. On timeout, a warning comment is posted on the first batch member and the function returns without advancing members (they remain in Queued for the next train cycle). On merge API failure — including `gh.ErrNotMergeableCI` from that TOCTOU window — an error comment is posted and landing aborts, leaving members in Queued to retry on the next train cycle; this is not escalated to a pause and does not touch `fabrik:rebase-needed`.
...
### 2.4 PR Review Threads and Review Bodies with Feedback

**Trigger (inline threads):** Reviewers leave inline code comments on the linked PR in unresolved review threads. These are real GitHub comments with `DatabaseID`s.

**Code path (inline threads):** Detected by `buildReviewThreadComments()` in the catch-up loop → `dispatchReviewReinvoke()` → async `processComments()` with synthetic comments

**Trigger (review bodies, #1205):** A reviewer submits (or edits) a PR review whose top-level body carries actionable text — not just its `State` verdict. This is the same `PullRequestReview` object §2.3 tracks for gate purposes, but here the body's *content* is the trigger, not its state transition. Every reviewer puts findings in the body at least some of the time (Gemini, CodeRabbit, `claude-review.yml`); Pruefer specifically demotes findings it cannot anchor to a diff line/file into the body under a `## Additional findings (could not anchor to diff)` heading (ADR-1189) — those are exactly the findings this trigger exists to surface, since before #1205 nothing read `LinkedPRReviews[].Body` for content, only for `.State` (the gate check, §2.3/§6.1).

**Code path (review bodies):** Detected by `buildReviewBodyComments()` in the catch-up loop (mirrors `buildReviewThreadComments()`, sourcing from `item.LinkedPRReviews` instead of `item.LinkedPRReviewThreadComments`) → `dispatchReviewReinvoke()` → async `processComments()` with synthetic comments. A review's body and its own inline thread comments can both be outstanding at once and are merged into the same reinvoke batch — see §6.2.

**Effect:** Triggers a review reinvocation cycle — the stage agent is re-invoked via `processComments()` with the review thread comments and/or review body as input, allowing it to address reviewer feedback. This is a **distinct event type** from regular comment processing (see §6.2). A pure-verdict review body (empty, or a bare approval like "LGTM") is filtered out mechanically before this trigger fires — see §4.1 signal 5.
...
- **`PRMergeReady` (`mergeable_state ∈ {clean, unstable}`):** gate clears immediately (`addCompleteLabelAndRemoveCI`); per-check classification is skipped. (The ADR-033 shortcut itself is untouched by ADR-933 — GitHub has already confirmed required checks passed in this case.)
- **A confirmed required-context failure (ADR-933):** `classifyCIFromRequiredContexts()` returns `(true, true, false)` — same `fabrik:awaiting-ci` application and `CIWaitTimeout` guard as a failed check run — evaluated **before** the check-run/mergeable_state classification below, since the failure may be visible only via `settle.RequiredContextsStatus`/`RequiredFailed`, not `settle.CheckRuns`.
...
**Signal 5 (#1205): mechanical non-actionable review-body filter.** `buildReviewBodyComments()` (§6.2), the review-body analogue of `buildReviewThreadComments()`, applies a fifth signal specific to synthetic review-body comments (`Comment.ReviewID != ""`) before admitting one into a review-reinvoke batch: `isNonActionableReviewBody(body)` (`engine/comments.go`) skips a review whose body is empty/whitespace-only, or whose trimmed, lower-cased body is an **exact** match against a short curated approval-phrase list (`nonActionableReviewBodyPhrases`: "lgtm", "lgtm!", "looks good", "looks good!", "looks good to me", "looks good to me!", "approved", "👍"). This is deliberately **exact-match, not substring** — a substring scan risks dropping a real finding in a body that happens to open with "LGTM" before diving into actual feedback, which is precisely the failure mode #1205 exists to fix. `buildReviewBodyComments()` also reuses `isBotServiceNotice` (signal 4) unmodified — it already operates on any `gh.Comment`, and a synthetic review-body comment is wrapped into that shape before the check runs. Like signal 4, this is a content classification, not a watermark: a "LGTM" review body never reaches `processComments`, so it never spawns a worker or produces a reply. `DISMISSED` reviews and reviews with an empty `PRReview.ID` (no GraphQL node ID to watermark against — e.g. the REST-only `base:<branch>` gate path, which never populates `LinkedPRReviews` at all) are excluded upstream of this signal, alongside the existing ROCKET-reaction and `CommentProcessed` checks mirrored from `buildReviewThreadComments()`.
...
| 1 | React with 👀 to all new comments | `AddCommentReaction("eyes")` / `AddPRReviewCommentReaction("eyes")` / `AddReviewReaction("eyes")` | Signals acknowledgment to the user |
...
| 10 | React with 🚀 to all processed comments + resolve review threads | `AddCommentReaction("rocket")` / `AddPRReviewCommentReaction("rocket")` / `AddReviewReaction("rocket")` + `ResolveReviewThread()` | Marks comments as processed (durable); resolves addressed review threads |
...
**Reaction-endpoint selection (steps 1 and 10, #1205):** `acknowledgeComments`/`finalizeComments` (`engine/comments.go`) pick the reaction call per comment via a three-way branch, checked in this order: (1) `c.ReviewID != ""` (a synthetic PR review-body comment) → `AddReviewReaction`, the sole GraphQL `addReaction` mutation call site in the codebase, since GitHub's REST API exposes no reactions endpoint for `PullRequestReview` itself (only issue comments, PR review *comments*, commit comments, issues, releases, and discussions) — keyed on the review's GraphQL node ID (`PRReview.ID`/`Comment.ReviewID`), not `DatabaseID`; this branch is checked before the `DatabaseID == 0` synthetic-comment guard below it, since a review body's `DatabaseID` (if any) is unrelated to its reactability via this endpoint. (2) `c.ReviewThreadID != ""` (an inline thread comment) → `AddPRReviewCommentReaction`, REST-keyed on `DatabaseID`. (3) else (a plain issue/PR conversation comment) → `AddCommentReaction`, also REST-keyed on `DatabaseID`. Step 10's thread-resolution side effect (`ResolveReviewThread`) is scoped to branch (2) only — a review body has no `ReviewThreadID` to resolve.
...
**Recording point.** `recordCommentBreakerInvocation()` is called inside `processComments` (`engine/comments.go`) immediately before Claude is actually invoked — after the step 2 `fabrik:editing` label and step 3 worktree setup (§4.2), but before step 4's `InvokeForComments()` call. Recording this late (not at the step 0 `JobStartedEvent` emission) means an early return before Claude runs — e.g. a `fabrik:editing` label API failure — does not count as a wasted cycle.
...
**Trip action.** `checkCommentBreaker()` is called at the tail of `processComments` — both on the non-completing-error early-return path and, when the invocation proceeds normally, after `publishCommentOutput`/`finalizeComments` (so any reset those steps applied takes effect first). When `commentBreakerCount() >= N`, `tripCommentBreaker()`:
...
2. Posts an explanatory `🏭 **Fabrik — comment-processing circuit breaker tripped**` comment naming the invocation count, the window, the last comment author, and how to resume (remove `fabrik:paused`).
...
- If gate cleared, `buildReviewThreadComments()` collects inline comments from unresolved review threads (no ROCKET reaction, not in `snap.CommentProcessed(c.ID)`), and `buildReviewBodyComments()` (#1205) collects synthetic comments from unresolved, actionable review bodies (§4.1 signal 5 filters out empty/pure-approval bodies mechanically before this point). The two sources are OR'd — either alone is sufficient. If both are empty: passes through (returns false).
...
- **Cycle limit check** (`snap.ReviewCycles(stage.Name)` vs `MaxReviewCycles`, default 5): if exceeded, `pauseForReviewCycleLimit()` adds `fabrik:paused` + `fabrik:awaiting-input` and claims. If not exceeded: increment count via `ReviewCycleIncremented`, dispatch `dispatchReviewReinvoke()` (applies `WorkerEntered`, acquires semaphore, calls `processComments()` asynchronously), set `advancedItems[key] = true`, claim. `dispatchReviewReinvoke`'s `build` callback concatenates `buildReviewThreadComments()` and `buildReviewBodyComments()` into a single batch — a review carrying both inline comments and additional body findings is addressed together, in one invocation, not split across two reinvoke cycles.
...
**processComments widening:** `processComments()` itself also merges any unresolved review-thread comments and review bodies at entry, before Step 1 — calling `buildReviewThreadComments()`/`buildReviewBodyComments()` directly (#1205) rather than duplicating their filter logic inline, so the two entry points (catch-up loop dispatch vs. this same-poll merge) can never drift out of sync. This closes the race where a user nudge arrives before the catch-up loop Phase 1 fires — outstanding review feedback on both surfaces is addressed in the same invocation as the nudge comment.

**Review thread resolution:** Step 10 of `processComments()` resolves addressed review threads via `ResolveReviewThread()` after adding ROCKET reactions. A review-body comment (`ReviewID != ""`) has no thread to resolve — this side effect is scoped to `ReviewThreadID != ""` comments only (§4.2).

**PR summary comment:** Step 8b of `processComments()` posts a Fabrik-marked `"<StageName> (review feedback addressed)"` comment on the linked PR when the invocation is a review-reinvoke and `output != ""`. `isReviewReinvoke()` (#1205) now returns true when every comment in the batch is review-originated — `ReviewThreadID != "" || ReviewID != ""` — not only an all-thread-comment batch as before, so a body-only or mixed thread+body batch still triggers this summary. The comment includes Claude's cleaned output, a machine-generated per-thread footer listing `path:line — resolved` for each unique `ReviewThreadID` in the input batch (deduped; line resolved from `Comment.Line` with `OriginalLine` fallback; review-body comments contribute no thread-footer entry, since they have no path/line), and a summary line reporting resolved-thread and review-body-finding counts separately (`formatReviewFeedbackComment`, `countReviewBodies`) — e.g. "Resolved 2 review thread(s) and 1 review body finding(s) across 3 comment(s)." This gives reviewers a visible record in the PR timeline that their feedback — inline and body alike — was addressed.

### 6.3 Review Reinvoke vs Regular Comment Processing

| Aspect | Regular Comments | Review Reinvoke |
|--------|-----------------|-----------------|
| Source | `item.Comments` (issue comments) | `item.LinkedPRReviewThreadComments` (PR inline comments) **and** `item.LinkedPRReviews` (PR review bodies, #1205) |
| Detection | `findNewComments()` | `buildReviewThreadComments()` **and** `buildReviewBodyComments()` (either non-empty is sufficient; a body's content is additionally filtered by §4.1 signal 5 before it counts) |
| Dispatch | Synchronous in `processItem()` | Async goroutine via `dispatchReviewReinvoke()`, batch = thread comments + review bodies concatenated |
| Cycle limits | None | `MaxReviewCycles` (default 5) — shared by both sources; a review with only a body now consumes one cycle per reinvoke, same as a thread-only review |
| Timeout | None | Integrated with `ReviewWaitTimeout` |
| Thread resolution | Yes — `processComments()` merges unresolved `LinkedPRReviewThreadComments`/`LinkedPRReviews` at entry, so a user nudge resolves both in the same invocation | Yes — resolves review threads after processing; review-body comments have no thread to resolve |
| PR summary posting | None | Posts `"<StageName> (review feedback addressed)"` on the linked PR with per-thread footer (one `path:line — resolved` bullet per unique `ReviewThreadID`) plus a review-body finding count reported separately in the summary line; skipped when `output == ""` or no linked PR |
| Worker guard | Uses dispatch loop's `snap.Worker() != nil` | Has its own `snap.Worker() != nil` check in catch-up loop |
| Reaction endpoint | `AddCommentReaction` (REST, by `DatabaseID`) | `AddPRReviewCommentReaction` (REST, inline threads) **or** `AddReviewReaction` (GraphQL `addReaction`, review bodies — no REST endpoint exists for `PullRequestReview` reactions) |
| `base:<branch>` support | Full | Inline threads and review bodies both structurally out of scope — `closedByPullRequestsReferences` (and everything nested in it, including `latestReviews`) is empty for non-default-branch PRs; see §6.1 |
...
- Three outcomes:
  ...
  - `(ciBlocked=true, ciFailure=true, ciTimedOut=false)` — failure confirmed (via `classifyCIFromCheckRuns` **or**, since ADR-933, `classifyCIFromRequiredContexts` for a confirmed required-context failure); `fabrik:awaiting-ci` applied idempotently; dispatch `dispatchCIFixReinvoke()` or pause on cycle limit
...
3. For each item: resolve its current stage via `stages.FindStage(e.cfg.Stages, item.Status)`. Skip if `!item.IsClosed`, or if a *resolved* stage is `stage.CleanupWorktree` or `stageIsGateChecked(stage)` (excludes Validate — left to `runValidatePRTerminalAdvance`, preventing any double-advance/race between the two settle-owners). If the resolved stage is `stage.HoldingStage`, skip only when `mergeTrainWorkerActive(repoKey)` is true for the item's `owner/repo` — otherwise fall through and advance it like any other eligible stage. A `stage == nil` column (Backlog, or any custom/extra column) is **not** skipped — it is treated as eligible, since it has no worktree and no stage bookkeeping to protect.
...
| Column `<X>`, closed (including Holding, if no train worker is active for its repo) | Settle pass succeeds | Column `<Done>`, closed | — (`stage:Done:complete` added separately by the existing `CleanupWorktree` dispatch branch on a later poll) | — |
| Column `<Done>` or gate-checked, closed | Settle pass evaluated | No change (out of scope for this scan) | — | — |
