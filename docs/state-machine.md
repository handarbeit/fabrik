...
| 8b | Restore or re-derive `stage:<Stage>:complete` (#1802) | `endStageRework()` | No-op unless step 2b actually cleared the label. On a non-completing exit (this step, or any of the earlier setup-failure/invocation-failure early returns): restores `stage:<Stage>:complete` directly. On a completing exit: leaves it to step 11's `handleStageComplete()` to re-derive from scratch (including its `wait_for_ci` deferral) — not re-added here, to avoid two sources of truth for "what does complete mean." `fabrik:reworking` is removed last, after this. See ADR-1802. |
...
If `fabrik:reworking` is left orphaned by a prior crash mid-rework (`beginStageRework` landed, but the owning `processComments` goroutine died before `endStageRework` could restore `stage:<Stage>:complete`), the item's completion claim stays incorrectly cleared until repaired. On restart, `runStartupCleanup()`'s third pass (mirroring the `fabrik:editing` pass immediately above it) restores `"stage:" + snap.Status() + ":complete"` and removes `fabrik:reworking` for any item with no active Worker. If `snap.Status()` cannot be resolved, the marker is left in place (logged loudly) rather than silently dropping the only remaining signal that a restore is owed — see ADR-1802.
