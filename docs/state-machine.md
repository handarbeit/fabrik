...
1. Clear any orphaned `fabrik:awaiting-input` label (same rationale as `handleStageComplete`).
2. Add `stage:<emitting>:complete` if not already present (`hasLabel` check).
