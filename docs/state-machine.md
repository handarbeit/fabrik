...
- Uses the same `reviewGateOutstanding` clearing condition as Path 2, so the two sites cannot disagree. Blocks conservatively on a fetch error (both slices discarded); does not block when no PR number resolves.
...
- **Conservative on error.** A failure from either fetch discards *both* slices and blocks — trusting whichever call succeeded could produce a false `len(outstanding) == 0` read that clears the gate on unknown state. Bounded by the `fabrik:awaiting-review` timeout, so a sustained API outage pauses for a human rather than hanging.
- **No PR ⇒ no block.** An unresolvable PR number means there are no reviewer requests; `handleBrokenReviewLinkage` owns the broken-linkage pause.
...
**References:** [ADR-1216: Review gate checked at the landing decision](../adrs/1216-review-gate-at-landing-decision.md)
