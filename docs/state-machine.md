...
† **Queued** is a *holding stage* (`holding_stage: true`) active only when `merge_train: on`. Items enter Queued when `attemptMergeOnValidate` routes to `advanceToQueued` instead of GitHub auto-merge. Per-item dispatch is suppressed (`itemMayNeedWork` / `itemNeedsWork` return false for holding stages); the batch handler `handleMergeTrainBatch` snapshots the Queued items once per poll cycle via `groupQueuedByRepoAndBase` (`engine/poll.go`, #1648), which **excludes closed and `fabrik:paused` members** (see the poison-well guard under [Merge-Train Red-Batch Bisection](#merge-train-red-batch-bisection-adr-059-d4)) and further **partitions the remaining items by resolved base branch** (`baseBranchForItem`) — one `queuedRepoGroup` per distinct (repo, base) pair actually present, not one group per repo. `routeQueuedGroup` runs once per partition and dispatches `runMergeTrainWorker` for each when its own batch is ready, so a repo with members on both the default branch and a `base:<branch>` label runs two fully independent trains simultaneously — each with its own pinned base SHA, trial branch, draft CI PR, and integration PR targeting its own base (ADR-1648, superseding ADR-1647's exclusion of such members from batching entirely). On a green trial-branch CI result, `landMergeTrainBatch` opens a landing integration PR, merges it, advances each member from Queued → Done via `advanceToNextStage`, and closes their individual PRs **and issues** (ADR-059 D3). The Queued column must exist on the GitHub Project board when `merge_train: on`; it is validated at startup.
...
**Selection algorithm.** `handleMergeTrainBatch` groups the current Queued snapshot by `owner/repo` **and resolved base branch** (`groupQueuedByRepoAndBase`, #1648/ADR-1648 — superseding the repo-only `groupQueuedByRepo` this function used before), preserving first-seen repo/partition order for the groups themselves. **Within each (repo, base) partition, member order is a deterministic function of the Queued set, not of whatever order the board-state source handed items back in** (#1833/ADR-1833): items are sorted ascending by `(StatusEnteredAt, Number)` — the member that has waited longest to enter Queued goes first, issue number breaking any tie — via `sortQueuedGroupsByEntry`, applied after partitioning so ordering never crosses a partition boundary. `StatusEnteredAt` is a record-on-write timestamp (`itemstate.ItemState.StatusEnteredAt`, sibling of `LabelAppliedAt`/ADR-1314) stamped the instant a Status mutation is applied, threaded into `gh.ProjectItem` only by the in-memory-cache path (`boardcache.CacheImpl`'s `snapshotToProjectItem`); the direct-GraphQL `GitHubAdapter` path never populates it, since no cheap durable "entered this Status" signal exists via REST/GraphQL for Project v2 items — every item there ties on the zero value and the sort degenerates to ascending Number order, still fully deterministic, just not wait-time-ordered. Before this fix, the in-memory cache's underlying map iteration (`itemstate.Store.All()`) was genuinely randomized per poll, so a Queued set exceeding `max_batch_size` could select a different arbitrary subset every cycle, abandoning the previous cycle's in-flight trial CI each time. `baseBranchForItem` is called only for items that actually carry a `base:` label — an item with no label buckets under the `defaultPartitionBase` sentinel without ever touching git, so partitioning costs nothing for the overwhelming common case. **A third fail-closed guard runs before this, ahead of the base-label check (#1772/ADR-1772):** an item whose board-cache entry has never been deep-fetched (`CacheImpl.IsItemDeepFetched`, checked per `(repo, number)` — never a global "cache is bootstrapped" flag) carries an empty `Labels` slice indistinguishable from a genuinely label-free item; rather than risk silently pinning it to `defaultPartitionBase`, it is excluded from batching for that poll (logged under the `merge-train` tag naming the item and reason) and left in `Queued`, picked up warm on a later poll once hydrated. This check fails open when no `CacheImpl` is wired, since a live-read engine has no unhydrated state to guard against. `routeQueuedGroup` then runs once per (repo, base) **partition**, routing its items by the **poll-native** `LinkedPRIsMergeQueueEnabled` signal (populated by the GraphQL board query — never a REST `pulls` fetch, which always reports `false` for the queue flag, and never a webhook payload):
...
- **Non-queue repo** → the **ADR-059 internal merge train**: the partition's remaining items form one batch, capped independently per partition (`max_batch_size`). The surviving items are dispatched to a single `dispatchMergeTrainWorker`, keyed on the composite `mergeTrainKey(owner/repo, base)` — not bare `owner/repo` — via `mergeTrainInFlight`, so a repo with members on two different bases runs two fully independent trains simultaneously, neither blocking, cancelling, or alerting on the other's guard state (ADR-1648, superseding ADR-1647's exclusion of `base:<branch>` members from batching entirely). Each partition's worker lands its own members `Queued → Done` via `landMergeTrainBatch`, forking from and targeting its own resolved base — a `base:maint/1.x` batch lands onto `maint/1.x`, never touching `main`.

**Batch-snapshot logging is deduped by composition (#1151), per partition.** `routeQueuedGroup` runs once per poll cycle for as long as any Queued items exist for a partition — including the full CI-wait duration of an in-flight train, which can span many poll cycles. Before dispatching, it computes an order-independent signature of the internal-train subset (`mergeTrainBatchSignature`: sorted item numbers, comma-joined) and compares it against `Engine.mergeTrainBatchSnapshotSeen` (a `sync.Map` keyed on the same composite `mergeTrainKey`). The `batch snapshot for <trainKey>: N item(s) — ...` line is only logged, and the stored signature updated, when the signature differs from the last-logged one for that partition — a membership or count change re-logs; a pure reordering of the same members (GraphQL item order is not guaranteed stable poll-to-poll) does not. Dispatch itself (`dispatchMergeTrainWorker`) always runs regardless of whether the log fired.

Both engines drain the same `Queued` column and advance their members to `Done` on land; only **who batches** differs (GitHub's queue vs Fabrik's trial branch). `max_batch_size` caps the internal-train subset **per (repo, base) partition**, not the repo's combined total across partitions — so a large repo A never starves repo B (the pre-D6 `batch[0]`-anchored latent multi-repo bug, hardened by grouping), and one of repo A's own bases never starves a sibling base in the same repo either.
...
**`mergeTrainInFlight` lifecycle**: set at `dispatchMergeTrainWorker` entry (LoadOrStore), keyed on the composite `mergeTrainKey(owner/repo, base)` — not bare `owner/repo` — since #1648, so two partitions of the same repo each get their own independent claim/clear lifecycle. Cleared by `finishTrain` — the single ADR-067-mandated clear point, called from `prepareTrainWorker`'s own-failure defer and `runMergeTrainWorker`'s top-level defer (both on success and failure). The goroutine's entry in `mergeTrainInFlight` is therefore gone by the time the goroutine exits, and the next `handleMergeTrainBatch` poll cycle can dispatch a fresh train for any still-Queued members in that same partition. `finishTrain` also clears the corresponding liveness registry inside `itemstate.Store` (`Store.ExitRepoWorker`, same composite key, set by `dispatchMergeTrainWorker` alongside the `LoadOrStore` claim) — see §9.2, issue #1222. The two registries share the exact same set/clear lifecycle, but serve different purposes: `mergeTrainInFlight` is the atomic duplicate-launch claim plus the `assembling`/`bisecting`/`CIResult`/`prNum`/`trialName` sub-state; the Store registry is the liveness answer the auto-upgrade idle guard reads (repo-wide, via `Store.RepoWorkerActiveForAnyBase`'s prefix scan) and `mergeTrainWorkerActiveForRepo` (`closed_item_advance_settle.go`) consults the same way, since that caller has no way to know which base a stranded closed item belongs to.
...
- **`max_batch_size` cap (FR-4):** `handleMergeTrainBatch` caps the Queued snapshot to the first `MaxBatchSize` items (default 5) via `capBatch`, logging any truncation. `capBatch` itself performs no ordering — it truncates whatever order it receives, which since #1833/ADR-1833 is always `groupQueuedByRepoAndBase`'s deterministic `(StatusEnteredAt, Number)` sort (see the "Selection algorithm" paragraph above), so "first `MaxBatchSize`" means the members that have waited longest to enter Queued. This bounds the worst-case bisection depth.
...
**Runaway guard (ADR-059 D8):** a composition-agnostic rate cap that fires when a (repo, base) partition creates ≥ `MaxTrainTrialsPerWindow` trial branches (default 20) with **zero successful landings** within a rolling `TrainTrialWindowDuration` window (default 60 min). The guard is distinct from the poison-well guard (which terminates the loop) and the bisect cost cap (which degrades to one-at-a-time within one goroutine run): it bounds the cross-poll-cycle burst that occurs when infra is broken and every trial fails. Since #1648, this fires **per (repo, base) partition, not per repo** — a `maint/1.x` partition's runaway trip pauses only that partition's own Queued members and never touches a healthy sibling `main` partition in the same repo.

**Counter:** `assembleAndValidate` is a thin wrapper around `assembleAndValidateInner` (the single site where all trial branches are created) that calls `recordTrial(trainKey)` only when the trial's result is **not** `TrainCIGreen` (#1528). A green result is, by construction, either the landing attempt itself or a bisection sub-trial that just proved a sub-batch clean — never a "zero successful lands" event, which is the guard's entire premise. Before this fix, every trial counted unconditionally, including the green survivor-validation trial that confirms a bisection isolated its poisoner and is about to land — so a **successful** bisection could trip the guard on its own green trial, pausing the clean survivors it had just vindicated one second before they would have landed (deterministically for any 3-member poisoned batch under the default `MaxTrainTrialsPerWindow=6` test configuration, since bisecting a 3-member batch down to a single poisoner takes exactly 6 raw trials). The exclusion is by-outcome, not by-origin: a **red** bisection sub-trial still counts, exactly as before — it represents genuine "no progress," the same as a red top-level trial. `TrainCIPending` results and assembly errors also still count unconditionally, unchanged. The counter itself is a rolling window (slice of `time.Time`) keyed on the composite `trainKey` (`mergeTrainKey(owner/repo, base)` — #1648, was bare `owner/repo`), protected by `mergeTrainTrialsMu`. `resetTrialCounter(trainKey)` deletes the entry; it is called from both `landMergeTrainBatch` and `landSingleton` after a successful merge, so any train where survivors do land never accumulates toward the cap — and a landing on one partition never resets a sibling partition's still-accumulating counter, since each has its own `trainKey`.

**Guard fires:** when `isRunawayTripped(trainKey)` returns `true` (count ≥ N within M minutes with zero resets), `fireRunawayGuard` is called. It:
- Logs the event at the `merge-train` tag (never silent): the (repo, base) train key, trial count, window duration, and member count.
...
**Atomicity and per-episode idempotency (ADR-1533):** `fireRunawayGuard` is called from three independent sites — twice inside `runMergeTrainWorker` (Hook 1, the worker goroutine) and once from `routeQueuedGroup` (Hook 2, the poll goroutine) — and nothing prevents Hook 1 and Hook 2 from running concurrently for the same `trainKey` once the shared trial counter trips: the poll loop does not check whether a worker is mid-firing. Each call site constructs its own, possibly-overlapping member slice from whatever local state it holds. Before #1533, this meant a member appearing in two racing calls' slices could receive a duplicate alert, and — independently — a member whose `AddComment` call happened to fail was left `fabrik:paused` with no comment and no retry path, since a paused member permanently drops out of `groupQueuedByRepoAndBase`'s Queued snapshot on every later poll.

The fix makes the whole pause+alert sequence for one `fireRunawayGuard` call a single critical section, serialized by `mergeTrainRunawayMu` (a single, unsharded, engine-wide mutex — since #1648 this also serializes concurrent per-base workers within the same repo, not just across repos; same accepted rare-event trade-off, just a larger set of legitimate concurrent callers), so two concurrent calls can never interleave their loops. Within that section, `mergeTrainRunawayAlerted` (an in-memory `map[string]int` keyed `"trainKey#N"` — #1648, was `"owner/repo#N"` — recording the trial count at which the member was last alerted) records which members have already been alerted **this episode**; a call re-encountering an already-alerted member at the same or a lower count skips it entirely (no duplicate comment, no redundant label calls) — a strictly *higher* count means genuinely new trials ran since the last alert (only possible after an operator manually resumes a paused member without the episode having ended via a successful land), and falls through to a fresh alert rather than being skipped. `resetTrialCounter` — the guard's only existing "this trip is over" signal, called on a successful land — also clears `mergeTrainRunawayAlerted` for the same `trainKey`, so the next trip starts a fresh episode without touching a sibling partition's own alert history. The runaway-alert settle scan (`runaway_alert_settle.go`), working from a bare already-paused board item with no partition context, reconstructs the identical `trainKey` via `mergeTrainKeyForItem` — the sentinel for a no-label item (matching `groupQueuedByRepoAndBase`'s own zero-cost bucketing exactly), or a re-resolution via `baseBranchForItem` for a labeled one — so it can never disagree with the key `fireRunawayGuard` originally alerted under.

A member whose `AddComment` call fails is **not** marked alerted. Instead it is left with a new durable marker label, `fabrik:awaiting-runaway-alert` (the `fabrik:paused` application is unaffected — pausing never depends on the comment succeeding). `settleRunawayGuardAlertScan` (`engine/runaway_alert_settle.go`), a per-poll settle scan sourced directly from `board.Items` (not `groupQueuedByRepoAndBase`, which the member's own `fabrik:paused` label already excludes it from), retries the alert every poll independent of any `fireRunawayGuard` call ever reaching that member again. After `MaxRetries` failed retries, `escalateRunawayAlertFailure` posts a fallback comment carrying the same explanation — but the marker is removed, and the member marked alerted, **only if that fallback comment itself is confirmed posted**; on a persistent `AddComment` outage that outlasts the fallback attempt too, the marker stays and the settle scan keeps retrying indefinitely, so the one remaining diagnostic signal is never erased out from under a member that still has zero delivered explanation (#1533 review, finding 1). See §6.18 and the Label Semantics Reference (§1.4) for `fabrik:awaiting-runaway-alert`'s full added-by/removed-by contract.
...
2. **Hook 2 (poll-side, `routeQueuedGroup`):** before `dispatchMergeTrainWorker` is called. If the partition's counter is already tripped (from a prior worker run), pauses the current partition's Queued snapshot (the uncapped partition `items` — every Queued member in that (repo, base) partition, not just the batch-capped subset that would have been dispatched) and skips dispatch for that partition only. This handles beyond-cap members that Hook 1 could not reach. Since #1648 partitions are formed by `groupQueuedByRepoAndBase` before this check ever runs, there is no longer a separate "exclude the excluded" step here (ADR-1647's exception, which existed only because non-default-base members used to be excluded from batching rather than routed to their own partition, no longer applies — see ADR-1648). Queue-enabled members and batch-cap overflow are unaffected: both are still paused as before, scoped to their own partition.

Hook 1 and Hook 2 **can** run concurrently for the same (repo, base) partition — the "beyond-cap members cannot form their own batch while the worker goroutine is still active" argument bounds only whether a *second train* can form for that same partition, not whether Hook 2's own guard check can race Hook 1's. Correctness for the alerting side comes from `mergeTrainRunawayMu` and `mergeTrainRunawayAlerted` (above), not from an assumption that the two hooks never overlap. A sibling partition's Hook 1/Hook 2 pair is entirely independent — its own `trainKey`, own counter, own critical section contention only through the shared `mergeTrainRunawayMu`.

**Operator resume:** once the runaway guard fires for a partition, all affected members have `fabrik:paused` and `fabrik:awaiting-input` applied and are therefore excluded from `groupQueuedByRepoAndBase` on every subsequent poll. No re-formation occurs for that partition until a human manually removes both `fabrik:paused` and `fabrik:awaiting-input` from each affected member — a sibling base partition in the same repo is entirely unaffected and continues normally. Before resuming, the operator should investigate the infra root cause (GitHub Actions billing, required-check configuration, base-branch health). The in-memory trial counter resets to zero on engine restart, but `fabrik:paused` labels persist on GitHub — so a restart alone does not re-enable the train; the labels must be cleared explicitly.

**`fireRunawayGuard` is deliberately exempted from the #1545 reroute fix** (R5 audit, below): unlike `ejectRedSingleton`'s pre-#1545 defect, a Queued member paused by the runaway guard is *not* stranded — `groupQueuedByRepoAndBase`/`routeQueuedGroup` re-admit an unpaused Queued member directly from `board.Items` on the very next poll, entirely independent of `itemMayNeedWork`/`processItem`'s `HoldingStage` exclusion (that exclusion only blocks *per-item stage dispatch*, not the batch handler's own board read). Rerouting every paused member to Validate instead would force each one through a full, redundant Validate re-run for a cause (infra-wide: billing, a broken base branch, all required checks erroring) no code stage can fix, and would make each member re-earn its way back into Queued via a fresh stage completion instead of simply resuming the batch it had already validated into.
...
| `fireRunawayGuard` (member-pausing path) | **Exempted** | Its recovery path is already reachable: `groupQueuedByRepoAndBase`/`routeQueuedGroup` re-admit an unpaused Queued member directly from `board.Items`, bypassing the `HoldingStage` dispatch exclusion entirely (see the Runaway guard section above). Rerouting the whole paused column to Validate would force every member through a redundant re-run for a cause (persistent infra failure) no code stage can address. |
...
| `fabrik:reworking:<Stage>` | `beginStageRework`, called immediately after the Step 2 `fabrik:editing` add succeeds | Only when the re-entered stage's own `stage:<Stage>:complete` is already present on `item.Labels` — the common mid-flight-rework case (nothing to clear yet) is a no-op. Added *before* `stage:<Stage>:complete` is removed (mark-first-then-clear) so the marker's presence is always a safe, idempotent restore trigger regardless of which of the two mutations actually landed before a crash (#1802) | `endStageRework`, called at every exit path (the two setup-failure early returns, the `err != nil && !completed` block, and `finalizeComments`) | On a non-completing exit: removed immediately, right after `stage:<Stage>:complete` is restored directly. On a completing exit: removal is deferred until *after* `finalizeComments`'s call to `handleStageComplete` returns (see the `stage:<X>:complete` row below) — not alongside it — so a crash anywhere during `handleStageComplete`'s own completion write still leaves the marker present as a durable "restore owed" signal. Stale copies with no active Worker are repaired at startup by `runStartupCleanup()`'s third pass, which reads the reworked stage's name directly out of the `fabrik:reworking:<Stage>` label suffix — never from the item's current board `Status` (a Pruefer review finding, #1802: a completing rework that also auto-advances moves `Status` to the *next* stage before the marker is removed, so a Status-keyed restore could stamp `stage:<NextStage>:complete` onto a stage that never ran). If `stage:<Stage>:complete` or `fabrik:awaiting-ci` is already present for that named stage (the rework settled correctly before the crash), only the marker is removed; otherwise `stage:<Stage>:complete` is restored first | Strictly nested inside the `fabrik:editing` bracket — `itemNeedsWork`'s `fabrik:editing` gate already makes the transient `stage:<Stage>:complete` clear unobservable to any dispatch decision, so this label changes no gate (R4); it exists purely so the board's label set reflects an in-progress rework instead of a stale completion claim. See ADR-1802 |
| `fabrik:paused` | `escalateFailedStage`, `blockOnInput`, `pauseForReviewTimeout`, `pauseForReviewCycleLimit`, `pauseForCITimeout`, `pauseForCIFixCycleLimit`, `pauseForRebaseCycleLimit`, `attemptMergeOnValidate` (on ErrNotMergeable rebase cycle limit reached, or CI wait timeout), `handleStopRequest` (TUI manual stop) and `runShutdownPause` (daemon clean-stop, ADR-1393) — both via the shared `pauseInterruptedIssue` primitive, `tripCommentBreaker` (comment-processing circuit breaker, #1089, §4.6) | After MaxRetries, FABRIK_BLOCKED_ON_INPUT, review/CI/rebase timeout or cycle limit, TUI `s`-key stop, a daemon-wide clean stop (SIGINT/SIGTERM, ADR-1393), or N non-advancing comment-processing invocations within the configured window (§4.6) | User (manual removal), or `processItem` (on new human comment that triggers unpause) | When user removes it manually, or a human comments on a paused issue | Blocks processing on active stages; a human comment is an implicit resume — bot-authored comments (`github.IsBotLogin`) and Fabrik's own output do not resume (`resumeAuthorised`, #1083), nor does a human comment that predates the pause (#1813). Cleanup stages ignore comments entirely while paused. A Validate-stage item with an externally merged PR can still advance via `runValidatePRTerminalAdvance` regardless of this label. `tripCommentBreaker` applies this label through the same `pauseIssue` helper as every other pause source, so the human-only resume guarantee applies uniformly (§4.6). Unchanged semantics for the daemon clean-stop writer (ADR-1393 R6) — as of #1379 also suppresses deep-fetch admission. |
...
| `fabrik:tools-denied` | Inline in `finalizeStageOutcome`'s final `else` branch (the `toolsDenied` case) — the stage-dispatch path; **and**, identically (#1704), a mirroring `errors.As(err, &toolsDeniedErr)` branch in `processComments`'s error handling — the shared funnel for ordinary comment review and all three reinvoke dispatchers (`dispatchCIFixReinvoke`, `dispatchReviewReinvoke`, `dispatchRebaseReinvoke`). Both call the same `recordToolsDeniedDetection` helper, so the label/comment/counter bookkeeping cannot drift between the two consumers of `interpretClaudeResult`'s shared classification | On a Claude invocation whose tool call(s) were denied by the CLI's own permission layer (`claudeToolsDeniedError`, detected structurally in `interpretClaudeResult`'s clean-exit path from a non-empty `resp.PermissionDenials` array, gated on `!completed` — never from output prose), gated on the label's own absence (idempotent — comment posted only on the absent→present transition) | The non-denial path in `finalizeStageOutcome`, gated on `!toolsDenied` (unlike `fabrik:claude-limit`/`fabrik:api-key-helper-detected`, this invocation itself may BE the condition — the classification does not short-circuit, so the clear must not fire on the very detection it would erase); `cleanupClosedIssueTransientLabels` (defensive sweep, via `transientLifecycleLabels`) | On the next invocation that is not itself classified as a tools-denied exit; or when issue is closed (defensive sweep) | Structurally unlike its two siblings: the CLI exits cleanly (`is_error: false`, `terminal_reason: "completed"`) and real work may have happened before the denial, so this does NOT short-circuit `finalizeStageOutcome` — `commitWIP`, the branch push, and `markCommentsSeenByStage` all still run, mirroring `claudeTurnLimitError`/`claudeResumeFailureError`'s continue-processing shape rather than the did-not-run family's early return. `StageAttempted` is recorded (normal dispatch cooldown applies) but `StageRetryIncremented` is deliberately never called — does not count against `max_retries`. Bounded instead by its own `ToolsDeniedRetries`/`MaxToolsDeniedRetries` counter (default 3) — shared across both consumers, so a denial on `ci-fix-reinvoke` and a denial on the stage path accumulate against the same per-stage counter; at the bound, `pauseForToolsDeniedLimit` applies `fabrik:paused` + `fabrik:awaiting-input` (never `stage:<name>:failed` — the condition is never treated as a stage failure, even at the bound), reused unforked from either call site. Because classification is expressed as a non-nil `error` from `interpretClaudeResult`, `blockedOnInput := err == nil && ...` is structurally unreachable for a detected denial — the outcome is identical whether or not the worker also emits `FABRIK_BLOCKED_ON_INPUT`. On the comment/reinvoke path, the new branch sits ahead of `checkNoOpCommentCycle`/`checkCommentBreaker` and returns before either runs, so a mode denial is bounded exclusively by `MaxToolsDeniedRetries`, never by the comment breaker's frequency-based thresholds (which stay unchanged). See §7.3b, §7.3c, and ADR-1523/#1704. |
| `fabrik:toolchain-stale` | `checkToolchainDrift` (`engine/toolchain.go`) — called from both `runInvocationWithExtension` (stage dispatch, immediately after the `apiKeyHelper` check) and `processComments` (comment review, right after context files are written) against the same shared implementation, so the two invocation paths cannot drift (#1786) | On a comparable declared-vs-resolved toolchain mismatch (`.nvmrc`, `package.json` `engines.node`, `go.mod` `toolchain`/`go` directive, or `.tool-versions`, compared against the binary version actually resolved on the daemon's own inherited `PATH` via `node --version`/`go version`), gated on the label's own absence (idempotent — comment posted only on the absent→present transition). Detection never returns an error and is never routed through `finalizeStageOutcome` — unlike `fabrik:claude-limit`/`fabrik:api-key-helper-detected`, it never skips the invocation; the stage/comment-review call proceeds exactly as it would have regardless of what it finds | `checkToolchainDrift` itself, the next time it runs for that issue and finds no comparable mismatch (declaration file fixed, or the daemon restarted with a corrected `PATH`); `cleanupClosedIssueTransientLabels` (defensive sweep, via `transientLifecycleLabels`) | On the next invocation (stage dispatch or comment review) that observes no comparable mismatch; or when issue is closed (defensive sweep) | Warn-and-continue, not a gate: `StageAttempted`/`StageRetryIncremented`/`max_retries` bookkeeping is entirely untouched, and neither `stage:<name>:failed` nor `fabrik:paused` is applied. Comparison is deliberately narrow and fails closed on ambiguity (an `.nvmrc` alias, a complex `engines` range, an unparseable `.tool-versions` line, or a binary missing from `PATH` are all "not comparable" — silence, never a false-positive warning); `.nvmrc`/`.tool-versions`/`engines.node` compare at major-version granularity, `go.mod`'s `toolchain`/`go` directive at major.minor using minimum-version semantics. A successful resolution is memoized process-wide per tool for the daemon's own lifetime (`resolveToolVersion`) — sound specifically because the bug this label exists to catch is that this `PATH` cannot change without a restart; a resolution *failure* (timeout, transient exec error) is deliberately never cached, since a failure carries no such invariance guarantee — caching it would permanently and silently disable detection for that tool after a single bad moment. No dedicated settle scan: this is a fire-once informational label, structurally identical to `fabrik:api-key-helper-detected`. See ADR-1786. |
...
| `stage:<X>:complete` | `handleStageComplete` (for non-`wait_for_ci` stages), `checkCIGate` (for `wait_for_ci: true` stages — added only after CI passes), `handleNoWorkNeeded` (emitting stage + all skipped stages), cleanup stage handler, `endStageRework` (restore on a non-completing comment-re-entry exit, #1802) | `handleStageComplete`: after Claude signals FABRIK_STAGE_COMPLETE on stages without `wait_for_ci: true`. `checkCIGate`: when all CI checks pass (R5) — this is the conjunctive gate (ADR 032): `stage:X:complete` is deferred until the CI gate actually clears, not applied on FABRIK_STAGE_COMPLETE. After FABRIK_NO_WORK_NEEDED (emitting stage + all subsequent non-cleanup stages) or worktree cleanup. For `stage:Validate:complete` specifically, `handleStageComplete` and `checkCIGate` also apply `ValidateCompletedAtSHA` carrying the current HEAD SHA so the SHA-invalidation scan (§2.16) can detect future SHA changes. `endStageRework`: immediately, on a comment re-entry's non-completing exit that had cleared it via `beginStageRework` — a direct re-add, not routed through `handleStageComplete`. | Never removed in the ordinary case, with two exceptions: `stage:Validate:complete` is removed by the SHA-invalidation scan (§2.16) when the linked PR's HEAD SHA changes after completion; and any stage's `:complete` is cleared by `beginStageRework` for the duration of a comment re-entry that reworks it (#1802) — restored directly by `endStageRework` on a non-completing exit, or left to `handleStageComplete`'s own re-derivation (including its `wait_for_ci` deferral) when the re-entry itself re-signals FABRIK_STAGE_COMPLETE | Permanent, with the same two transient exceptions: `stage:Validate:complete` when the linked PR SHA changes after completion (§2.16); and any stage during a comment-re-entry rework window, bracketed by `fabrik:reworking:<Stage>` (#1802, ADR-1802) — never observable by any dispatch-admission gate, since the window is strictly nested inside `fabrik:editing`'s own gate | Prevents re-invocation of the stage; triggers catch-up advancement |
...
| `fabrik:children-spawned` | `spawnChildren` (engine-side, shared by `preImplement` and the Review/Validate mid-flight hook, §6.7.2 — ADR-1419) | After all `FABRIK_SPAWN_CHILD_*` children in one batch are successfully created and linked as blockedBy of parent, whichever stage originated the spawn | User (manual, to re-trigger spawn) | If user wants a fresh spawn (children created under the prior spawn must be closed manually first — `fabrik:spawned-child:<i>:<n>` markers, ADR-1583, are only consulted while this label is absent) | Idempotency guard against `preImplement` re-spawning the same Plan-declared batch on retry — while present, `preImplement` is a no-op. The Review/Validate mid-flight hook needs no equivalent guard (its own idempotency comes from parsing each dispatch's fresh `output` exactly once, §6.7.2), but still sets this label on a successful spawn — it remains true that this parent has spawned children at least once. |
| `fabrik:spawned-child:<blockIndex>:<childNumber>` | `spawnChildren` (ADR-1583) | Immediately after each spawn block's child issue is successfully created, on the **parent** | `spawnChildren`, once the full batch (including sibling `DEPENDS_ON` wiring) succeeds | N/A (best-effort cleanup on success) | Durable, restart-surviving marker recording that this Plan-declared block's child already exists — see §6.7's "Resume-safe retry" note. Transient only: present during an interrupted/in-progress spawn, absent once `fabrik:children-spawned` is set. Removing one manually forces re-creation of that specific block on the next retry (e.g. if the recorded child was genuinely deleted). |
...
1. **Unpause:** On a paused issue, a **human** comment (`resumeAuthorised()` — excludes bot logins via `github.IsBotLogin`; Fabrik's own output is separately excluded upstream by `findNewComments`' `🏭 **Fabrik` body-prefix check) **created at or after the latest `fabrik:paused` `labeled` event** removes `fabrik:paused` (and clears failed state if present) and falls through to comment processing. A bot-authored comment does not unpause (#1083), and neither does a human comment that strictly predates the pause (#1813, §4.4).
2. **Unblock awaiting-input:** On an awaiting-input issue, a **human** comment removes both `fabrik:paused` and `fabrik:awaiting-input`, then routes to `processComments()`. A bot-authored comment, or a human comment that strictly predates the pause, does not unblock.
...
- Adding **or removing** `fabrik:yolo` or `fabrik:cruise` → takes effect at the next advance decision, even mid-run, on both advance paths (`handleStageComplete()` and the catch-up loop's `runCatchUpPhase2()`) — both call the shared `refreshAutonomyLabels()` helper, which performs a live, item-scoped `FetchLabels()` immediately before either reads `hasYoloLabel`/`hasCruiseLabel` (#1769; see §6.23 / Appendix A for the full rationale, including why *removal* previously went unobserved on both paths)
...
**Suite-aware green (#1822).** `pollTrainCI`'s two green returns (all-green runs + required contexts satisfied; zero runs + accepted `mergeable_state`) and `classifyLandingCI`'s two green returns are each gated on `ciSuiteHold(e.client, …)`: an outstanding suite keeps `pollTrainCI` polling (bounded by `CIBackstopTimeout`) and makes `classifyLandingCI` return `TrainCIPending`; a confirmed red still returns `TrainCIRed` immediately. See §6.4 *Suite-aware completeness*, ADR-1822.
...
**Relationship to the comment-rework clear/restore (#1802, ADR-1802):** this scan is the direct precedent for #1802's `fabrik:reworking:<Stage>`/`beginStageRework`/`endStageRework` mechanism — both clear a stage's `stage:<Stage>:complete` when it has gone stale and let the normal completion flow re-populate it. The two are independent and never coordinate directly: this scan only ever targets `stage:Validate:complete`, triggered by a SHA change, outside any comment re-entry; #1802's mechanism targets whichever stage is being re-entered, triggered by `processComments`, bracketed by `fabrik:editing`. Both removal and (re-)addition of `stage:Validate:complete` are idempotent GitHub label mutations, so the two mechanisms running in the same window — e.g. a SHA change arriving while a comment re-entry is reworking Validate — cannot double-apply or conflict; each just mutates the label set to match its own view of truth.
...
**Catch-up loop `shouldAdvance` resolution (Path 2):** `runCatchUpPhase2` calls `refreshAutonomyLabels(&item)` first (#1769, §6.23) — mirroring Path 1's step 1 — so this decision also reads live label state, not whatever the item snapshot happened to carry into the catch-up loop. It then checks `cfg.Yolo || hasYoloLabel(item) || hasCruiseLabel(item)` — items without any of these are skipped entirely. Then: if neither yolo nor cruise LABEL is present and `stage.AutoAdvance` is explicitly false, the item is skipped. This produces the same behavior as Path 1.
...
| 2b | Clear stale `stage:<Stage>:complete` if present (#1802) | `beginStageRework()` | No-op unless the re-entered stage's own `stage:<Stage>:complete` is already on the item. When present: adds `fabrik:reworking:<Stage>`, then removes `stage:<Stage>:complete` — mark-first-then-clear, strictly inside the step-2 `fabrik:editing` bracket so no dispatch gate ever observes the transient clear (R4). See ADR-1802. |
...
| 8b | Restore or re-derive `stage:<Stage>:complete` (#1802) | `endStageRework()` | No-op unless step 2b actually cleared the label. On a non-completing exit (this step, or any of the earlier setup-failure/invocation-failure early returns): restores `stage:<Stage>:complete` directly, then removes `fabrik:reworking:<Stage>` immediately — nothing later in the flow will restore it. On a completing exit: this step is deliberately skipped here — `stage:<Stage>:complete` is left to step 11's `handleStageComplete()` to re-derive from scratch (including its `wait_for_ci` deferral), and `fabrik:reworking:<Stage>` itself is *not* removed until after step 11 completes (not here at step 8b), so a crash anywhere between this point and step 11's completion-label write still leaves the marker as a durable signal that a restore is owed. See ADR-1802. |
...
| 11 | If FABRIK_STAGE_COMPLETE was detected: handle completion | `handleStageComplete()` | Same completion flow as a normal stage invocation (advance, PR ops, etc.). On a completing exit that began a rework (step 2b), `fabrik:reworking:<Stage>` is removed immediately after this step returns — not before — per step 8b's note (#1802, ADR-1802). |
...
1. **Awaiting-input unblock:** `isAwaitingInput(item)` is true + new **human** comments created at/after the pause (`resumeAuthorised()`) → `unblockAwaitingInput()` → `processComments()`
2. **Paused unpause:** `fabrik:paused` present + new **human** comments created at/after the pause (`resumeAuthorised()`) → remove `fabrik:paused`, `clearFailedStage()` → fall through → `processComments()`
3. **Normal comment processing:** Item is not paused → `findNewComments()` finds comments (human or bot) → `processComments()`

**Resume anchor (#1813, ADR-1813).** `resumeAuthorised()` is the single predicate behind all four resume gates (`itemNeedsWork`'s awaiting-input and paused branches, `processItem`'s awaiting-input block and paused loop), so admission and resume cannot disagree. It authorises a resume only from an *event* — a human comment whose `CreatedAt` is at or after the latest `labeled` event for `fabrik:paused` (read live via `FetchLabelAppliedAt`, never the record-on-write cache, so a label removed in the UI cannot leave a stale early anchor; `isAwaitingInput` implies paused, so this one label covers both) — not from the *state* "an unprocessed human comment exists". A comment that can never be processed therefore lifts a pause at most once: after the resume any re-pause writes a newer `labeled` event, and the comment predates it. Consequently the breaker counters (`CommentBreakerReset`, `EngineCyclesCleared`, `clearFailedStage`) are only zeroed by a genuine resume. **Tie-break (R6): every indeterminate input resumes** — anchor fetch error, no `labeled` event found, a zero `CreatedAt`, or a comment timestamp equal to the anchor (one-second resolution); only "every human comment strictly predates the pause" refuses. Trade-off: a human comment posted *while* an invocation runs, which then pauses (`blockOnInput`, a breaker trip), predates that pause and does not resume it — the human comments again.

In paths 1 and 2, `resumeAuthorised()` only gates *whether* to resume — once a human comment authorizes it, the raw (unfiltered) comment set from `findNewComments()` is what's actually handed to `processComments()`, in the same `processItem` invocation. Any bot comments that accumulated while paused/awaiting-input are processed together with the resuming human comment, not deferred to a later poll (#1083) — except bot service-notice comments (§4.1 signal 4), which `findNewComments()` excludes from that raw set entirely regardless of pause state, since they are non-actionable either way.

All three paths ultimately source their candidate comments from `findNewComments()` (paths 1–2 via `resumeAuthorised()`'s wrapping, path 3 directly). But `processItem()`'s three paths are not the only route into `processComments()` — the three catch-up-loop reinvoke dispatchers (`dispatchReviewReinvoke`, `dispatchCIFixReinvoke`, `dispatchRebaseReinvoke`, §6.2) call it directly with comments built by `buildReviewThreadComments()` or a synthetic single-comment builder, neither of which goes through `findNewComments()`. A quota/rate-limit bot notice can therefore still reach `processComments()` as an entry in its working `comments` slice via one of these non-`findNewComments()` paths — the claim that it "never reaches `processComments()` via any path" was false, and as of #1221 that gap is closed differently: `processComments()` itself (§6.2) applies `isBotServiceNotice` as a single chokepoint to its fully-assembled working `comments` slice, immediately after the slice is assembled — covering the `findNewComments()`-sourced set (idempotently) as well as the `LinkedPRReviewThreadComments` merge and all three reinvoke dispatchers' input — before any reaction, label, worktree, or invocation side effect. `findNewComments()` is the enforcement point for whether a worker is dispatched at all (§4.1 signal 4); `processComments()` is the enforcement point for whether a comment — however it arrived — is ever handed to Claude or otherwise acted upon.

All six of these entry points — the three `processItem()` paths above and the three reinvoke dispatchers — funnel through the same `processComments()` function body, so all six inherit the `fabrik:reworking:<Stage>`/`stage:<Stage>:complete` clear-and-restore behavior (§4.2 steps 2b/8b, ADR-1802) uniformly, with no per-site special-casing. There is exactly one place that decides whether a re-entered stage's completion claim needs clearing (`beginStageRework`, called once, immediately after step 2's `fabrik:editing` add succeeds) and exactly one place that restores or defers to the normal completion flow (`endStageRework`, called from every exit path of `processComments()`/`finalizeComments()`) — regardless of which of the six triggers brought the item here.
...
**Trip action.** `checkCommentBreaker(item, reason string)` is called from five points in `processComments`: each of the three setup-failure early-return paths (editing-label add, base-branch resolution, worktree setup — `reason` names the failing step and its error), the non-completing invocation-failure early-return path (`reason == ""`), and, when the invocation proceeds normally, after `publishCommentOutput`/`finalizeComments` (`reason == ""`, so any reset those steps applied takes effect first). A **sixth**, distinct early-return sits just above the non-completing-failure point (mirroring the pre-existing usage-limit exclusion there): a tools-denied classification (`*claudeToolsDeniedError`, #1704, §7.3c) returns before either `checkNoOpCommentCycle` or `checkCommentBreaker` is ever called, so a mode denial is bounded exclusively by its own `MaxToolsDeniedRetries` counter, never by this breaker's frequency-based thresholds. When `commentBreakerCount() >= N`, `tripCommentBreaker(item, count, window, reason)`:
...
4. Suppresses further dispatch: once `fabrik:paused` + `fabrik:awaiting-input` are present, the item is gated by the same paused/awaiting-input machinery as every other pause (§1.4, §4.4) — only a **human** comment created after the pause (`resumeAuthorised()`) can resume it, and per §4.4 a resuming human comment is processed together with any bot chatter that accumulated during the pause, which itself records a fresh invocation and is subject to the breaker again.
...
- **Reinvoke-before-block (R1, #1375, amending ADR-1250):** `buildReviewFeedbackComments()` — unresolved inline review-thread comments (`buildReviewThreadComments()`, unmodified) **plus** unaddressed review-body comments (`buildReviewBodyComments()`, new) — is computed unconditionally next, *before* `blocked`/`timedOut` are consumed. If that combined set is non-empty, the reinvoke below dispatches regardless of whether the gate is currently `blocked` — this is what makes the reinvoke reachable at all under `review_authority: authoritative`, where an unresolved `CHANGES_REQUESTED` would otherwise keep `checkReviewGate` returning `blocked=true` indefinitely (Finding 1). Only when this set is empty does the handler fall through to the pre-existing tail: if `blocked`, records `CooldownRecorded{Reason: "review-blocked"}` so `itemMayNeedWork`'s expiry path re-evaluates the item every 10 × PollSeconds, then claims; if `timedOut`, calls `pauseForReviewTimeout()` and claims.
  ...
  - **Did-not-run cycle refund (#1812, ADR-1812).** A reinvoke whose Claude invocation provably never ran is not an attempt to converge and must not spend the cycle budget. `dispatchReinvoke` (`engine/reinvoke.go`, the shared scaffold for review, CI-fix and rebase — and the only place that also covers `checkAutoMergeConvergence`'s inline `RebaseCycleIncremented`/`CIFixCycleIncremented` charges) calls `processCommentsClassified`, which returns a `didNotRunKind` alongside the unchanged error: `account-suspended` (the ADR-1120 suspension gate, which returns `nil` before any side effect), `usage-limit` (`claudeUsageLimitError`, also a `nil` return, excluded from the comment breakers), `api-error` (`claudeAPIErrorExit`, whose classifier already refuses a run that consumed turns *and* incurred cost — R4), and `api-key-helper` (`apiKeyHelperDetectedError`; produced only by the stage-dispatch path today, so recognised here for forward-compatibility only). Any other error — turn limit, resume failure, tools denied, a mid-run crash, an output-publication failure — yields `""` and stays charged exactly as before (R3); ambiguity is always charged. On a non-empty kind the scaffold applies the dispatcher's compensating mutations (`ReviewCycleDecremented`, plus `ReviewBlockedCycleDecremented` when `handleReviewGate` charged it, i.e. `blocked || timedOut`; `CIFixCycleDecremented`; `RebaseCycleDecremented` — each floored at zero and a no-op with no `Change` on an already-zero counter), records `DidNotRunReinvokeRecorded`, logs `invocation did not run (<kind>) — refunding <counter> cycle counter for stage "<S>" (#1812)`, and **skips the dispatcher's `after` hook** — so the #1045 HEAD-unchanged refund, CI-fix's no-op-SHA debounce and rebase's auto-merge re-enable, all meaningless when nothing ran, never fire. The two refund mechanisms are therefore disjoint and cannot double-refund. `ReviewBlockedCycles` keeps ADR-1518's never-refunded guarantee for a run that *did* execute and no-op'd; only a did-not-run exit compensates it.
  - **`DidNotRunReinvokes` — evidence for the pause message (R7, #1812).** Because refunded cycles vanish from the counters, `StageState.DidNotRunReinvokes` is a tally of did-not-run reinvokes that is never refunded by a cycle refund, is cleared by `EngineCyclesCleared`, and is **reset by `DidNotRunReinvokesReset` whenever a reinvoke stays charged** (`dispatchReinvoke` applies it on every non-did-not-run exit, including ambiguous ones) — so it is the streak of never-ran reinvokes since the last genuine cycle, never an outage from days earlier that could flip the message for a later real non-converging loop. `pauseForReviewCycleLimit`, `pauseForCIFixCycleLimit` and `pauseForRebaseCycleLimit` consult it via `didNotRunPauseNote`: when that streak is at least the cycle count, the "reviewer is repeatedly requesting changes" (resp. "CI still failing" / "not mergeable") sentence is replaced with a statement that the invocations never ran, naming the likely cause (a Claude API error such as a 429 session limit, or a usage limit), the elapsed-time discriminator (real cycles take minutes; a burst within seconds means nothing ran) and where to look (`.fabrik/logs/<repo>/issue-N/`); a smaller nonzero tally adds a one-line note; a zero tally leaves the message byte-identical. The stable dedup fragments (`reviewCyclePauseFragment` etc.) are unchanged, so #1460's `hasPauseComment` still matches. **Bound after the refund:** a did-not-run storm no longer trips `MaxReviewCycles`; it is bounded by the comment circuit breaker (§4.6, default 10 per 30 min) and the #1555 no-op breaker (default 10), both of which still count these exits — the false pause moves from cycle 5 to roughly cycle 10 with the breaker's own message. Backoff on this path is out of scope; detecting the 429 upstream is what prevents the storm.
...
9. `mergeable_state == "clean"` (ADR-033, narrowed to `clean`-only by ADR-1441/#1441 — previously `mergeable_state ∈ {clean, unstable}` per the `MergeableStateAccepted` allowlist): return `PRMergeReady` — GitHub reports no *required* check failing or pending. **This is not proof every job ran** (#1822): a job still queued for a runner has no check run, so GitHub reports `clean` for a green prefix. `FetchCheckRuns` is not called, but since #1822 the shortcut first consults the suite hold (see *Suite-aware completeness* below); a held suite returns `PRMergeUnsettled` instead. An empty `HeadSHA` skips the shortcut and falls to rule 10. `mergeable_state == "unstable"` **no longer matches this rule** — it falls through to rules 10–19 below like any other non-`clean` value (e.g. `"blocked"`), so a confirmed failure on a non-required check is caught by rule 19 instead of being silently accepted here.
...
18. Check runs empty + `mergeable_state ∈ {"", "unknown"}` + required contexts satisfied (or unconfigured): return `PRMergeReady` (no CI configured) — but only if the suite hold (below) is clear: a suite reporting runs, or a run-less suite younger than the post-push dwell, contradicts "no CI" and returns `PRMergeUnsettled` instead (#1822).
19. Check runs non-empty: classify via `github.ClassifyCheckRuns(checkRuns)` (#958) — reduces to the latest run per check *name* (highest ID wins), then: any pending (`queued`/`in_progress`) at any name → return `PRMergeUnsettled` (`CheckRuns` set) **regardless of any failed run**, whether that failed run is a different check name or a stale (lower-ID) entry for the same name superseded by the pending rerun; else any failed (`failure`/`timed_out`/`action_required`) → return `PRMergeBlocked` (`CheckRuns` set); else all green: **before returning `PRMergeReady` (ADR-933)**, `classifyRequiredContexts()` is consulted a third time (fetching classic commit statuses via `FetchCombinedStatus` only when at least one required name isn't already resolvable from the present check runs) — a non-`Satisfied` result returns `PRMergeBlocked`/`PRMergeUnsettled` instead (same Failed→Blocked, Pending→Unsettled mapping as rules 13/17), closing the exact hole #933 was filed for: an all-`skipped`/`neutral` check-run set on the head must never resolve to `PRMergeReady` when a required context hasn't confirmed success. Only once required contexts are satisfied (or unconfigured) **and the suite hold (below) is clear** (#1822) does this rule return `PRMergeReady` (`CheckRuns` set); a held suite returns `PRMergeUnsettled` with `CheckRuns` set and `MergeableState` omitted. Pending-over-failed (within `ClassifyCheckRuns` itself) is still evaluated **first**, reversing the pre-#958 order where a failed run short-circuited to `PRMergeBlocked` before pending was even checked.
...
**Suite-aware completeness (#1822, ADR-1822).** A check run exists only once its job has been scheduled onto a runner, so a `needs:`-dependent job (or one queued for a busy runner) is invisible to `FetchCheckRuns`, and an all-green *prefix* of a workflow reads as a complete pass — while GitHub itself reports `mergeable_state == "clean"` whenever no *required* check is outstanding. The check **suite** is created at queue time and stays non-`completed` until every job has run, so it is the signal missing from the run set. `settlePRMergeState` therefore consults `ciSuiteHold` (`engine/ci_suites.go`) at every point that would otherwise return `PRMergeReady`: rule 9, rule 18 and rule 19. It is deliberately consulted **last** and only on a would-be-green path, so a confirmed failure (`PRMergeBlocked`), a pending run, or an unsatisfied required context (ADR-933) is never masked, and a red or pending SHA does not pay for the extra call.

- **Live read, never cached.** `FetchCheckSuites` (`GET /repos/{o}/{r}/commits/{sha}/check-suites`, paginated and `total_count`-verified like `FetchCheckRuns`, #1539) is a pass-through on `GitHubClient`, `boardcache.ReadClient`, `GitHubAdapter` and `CacheImpl`: the `check_suite` webhook is a no-op (§ webhook table), so there is no cached suite state to serve. It needs only `checks: read`, already held under App auth — no permission or manifest change.
- **Discriminator (`gh.OutstandingCheckSuites`).** A suite is outstanding when `status != "completed"` **and** either `latest_check_runs_count > 0` (real work in progress — the incident case) **or** it has zero runs but is younger than the post-push dwell, measured from the suite's own `created_at` — **or** it belongs to the `github-actions` App (#1829), in which case age and run count are irrelevant: any non-`completed` `github-actions` suite is outstanding, full stop. GitHub creates exactly one check suite per workflow run, so a run-less `github-actions` suite past the dwell always means a job hasn't been scheduled onto a runner yet, never an idle installation — the incident's own `E2E (Playwright)` job waited 17 minutes for a runner, well past any plausible fixed dwell, which is the narrower window #1829 closed (the original #1822 incident happened to have `runs > 0` on its `github-actions` suite by the time the gate was read, so it never hit this path). An **old** run-less non-completed suite from any **other** App is *inert* — an installed App (`cursor`, `claude`, `coderabbitai`, `github-pages`…) whose suite sits `queued` forever — and is ignored. "Every suite must be completed" is explicitly rejected: it deadlocks on inert Apps. The rule is "any" suite, never "the" suite (GitHub Actions creates one suite per workflow run). A `github-actions` suite that has genuinely settled to `completed` with zero runs (e.g. an all-skipped-jobs workflow, or a startup/parse failure) still clears normally — the `completed` filter is checked before the app-slug branch, so this carve-out cannot itself deadlock the gate.
- **Post-push mitigation.** The window in which a real suite exists with zero runs before its first job registers is covered by the age bound above (a variant of option (b), reusing `PostPushDwell`, default 90s, anchored on GitHub's `created_at` so — unlike the in-memory `LastHeadSHAUpdate` of rule 15 — it survives a restart), plus a scoped form of option (a): a suite reporting runs contradicts an empty run set, so rule 18 holds. A repo with genuinely no CI has no outstanding suite and still clears. Option (c) (the Actions workflow-run read) was rejected: it needs `actions: read`, which the App is deliberately not granted (ADR-1756), and does not generalize beyond GitHub Actions. A missing or unparseable `created_at` on a run-less suite counts as young (hold).
- **Fail-safe.** A suite read error holds (`PRMergeUnsettled`; the reason carries the error). The hold is `PRMergeUnsettled` with `MergeableState` **omitted** — it is a wait for workflow work, not an R3 case, so the invariant above is preserved. `checkMergeabilityGate` claims it before `checkCIGate`, so it is bounded by `settleAwaitingCIScan`'s unconditional `CIBackstopTimeout` (ADR-1410, §6.14.5): a suite stuck `in_progress` forever escalates through `pauseForCITimeout` like a stuck check run. That comment names the outstanding suite and app (`suiteTimeoutNote`, one best-effort live read at escalation time).
- **One primitive, every surface.** The same `ciSuiteHold` gates `classifyLandingCI` (and through it `singletonFastPathEligible`, §6.20) and `pollTrainCI` (both green returns), so the advance gate, the landing poll, the trial poll and the singleton fast path cannot disagree on one SHA. Each is a `TrainCIPending` (never green) on a hold or a read error.
- **Residual limits.** No suite read can see a workflow that has not yet been *triggered* (`workflow_run` chains, later events). The ordinary non-train yolo auto-merge path (`attemptMergeOnValidate`) has no CI classification of its own and is protected transitively — `stage:Validate:complete`, which precedes landing, is conjunctive on this gate; `MergePR`'s `unstable`-accepting self-gate is unchanged, and a Validate stage with `wait_for_ci: false` is out of scope. A member that goes red *after* admission to the merge train is #1821's concern.
...
- Embedded directly in the auto-merge path for Validate+yolo items. It performs **no CI classification of its own** (an earlier version fetched check runs with pending/timeout bookkeeping; that was removed and this section had drifted — corrected in #1822).
- For a non-train yolo item it calls `EnablePullRequestAutoMerge` (GitHub enforces *required* checks natively), falling back to `MergePR` on failure, whose only CI protection is its own `mergeable_state ∈ {clean, unstable}` self-gate (ADR-072).
- CI protection is therefore **transitive**: landing follows `stage:Validate:complete`, which for a `wait_for_ci: true` stage is itself conjunctive on Path 2 below — the suite-aware gate (§6.4, #1822). No separate landing-time CI gate exists here (it would need its own escalation owner to be bounded); `MergePR`'s self-gate and `wait_for_ci: false` stages remain unprotected against a partial check set.
...
**Idempotency guard:** If the parent issue has `fabrik:children-spawned`, `preImplement` returns immediately (no-op). Manual removal of this label (and closing of children created under the completed spawn) is required to trigger a fresh spawn. This guard runs before the recovery path below and must remain first — recovery must never bypass it, or repeated deferred-retry cycles risk double-spawning.
...
4. For each `FABRIK_SPAWN_CHILD_BEGIN/END` block in document order (creation pass — unchanged by `DEPENDS_ON`, except that the block-index → child node-ID mapping is now retained for the sibling-wiring pass below):
   - **Resume-safe retry (ADR-1583).** Before the loop, `parseSpawnChildLabels(item.Labels)` recovers which 1-based block indices already have a `fabrik:spawned-child:<blockIndex>:<childNumber>` marker from a previous, interrupted attempt at this same batch; if any exist, `item.BlockedBy` is refreshed live first (`refreshForSpawnResume`, mirroring the missing-Plan-comment recovery's cooldown-gated live-read pattern below — a live-read failure or active cooldown returns the same `errPreImplementDeferred` outcome, **not** a pause). For a block with an existing marker: `CreateIssue` is **skipped** — the child's node ID is instead resolved via `FetchProjectItem(childOwner, childRepo, childNumber)`, pausing loud (naming the specific marker label to remove) if that recorded child can no longer be found. For a block with no marker, the sequence below runs exactly as it always has.
   - `CreateIssue(owner, repo, title, body+footer, assignees)` via REST — returns `(number, nodeID)`. **`assignees` is always `[cfg.User]` (ADR-1419, requirement 4)** — every spawned child, same-repo or cross-repo, is assigned to the user of the instance meant to process it; folded into the same POST as creation rather than a separate mutation, so a bad/misconfigured `user:` value fails loud through this single already-fail-loud call rather than opening a second, silent failure point. Immediately after a successful `CreateIssue`, the durable `fabrik:spawned-child:<blockIndex>:<childNumber>` marker is written on the **parent** (`addLabelChecked`, best-effort — a write failure is logged, not fatal, and only regresses that one child to pre-ADR-1583 behavior if a *later* step in the same attempt also fails).
   - `AddProjectV2ItemById(board.ProjectID, nodeID)` — adds child to the same project board; returns `childItemID`. **When resuming a marked block**, this is skipped in favor of a `LookupIssueProjectItem` check first — if the child is already on the board, its existing item ID (and current Status) is reused instead of calling `AddProjectV2ItemById` again (unconfirmed against real GitHub whether a repeat call is safe — ADR-1583 checks rather than assumes). A fresh (unmarked) block always takes the direct-call path, unchanged.
   - `AddBlockedByIssue(parent.NodeID, nodeID)` — links child as a `blockedBy` dependency of the parent. **When resuming**, the (freshly refreshed) `item.BlockedBy` is checked first; if the edge already exists, this call is skipped for the same unconfirmed-idempotency reason as the board-add above.
   - `AddLabelToIssue` for `fabrik:sub-issue` on the child (informational; idempotent add, always attempted)
   - `UpdateProjectItemStatus(board.ProjectID, childItemID, sf.FieldID, specifyOptionID)` — moves child to the `Specify` column (or first non-Backlog, non-terminal column as fallback). **Non-fatal**: if the call fails, `e.statusField` is nil (startup fetch failed), or no viable column exists, the child lands in Backlog, a warning is logged, spawn continues — and `recordChildPlacementFailure` writes the durable `fabrik:awaiting-placement` marker on the child so the board-placement settle scan retries it (§6.9, ADR-062). **When resuming a block whose `LookupIssueProjectItem` check (above) found a non-empty Status**, this call is skipped entirely rather than re-run — the child may have already progressed under its own pipeline while the parent sat paused, and blindly re-placing it into `Specify` would clobber that progress (ADR-1583).
   ...
   - On any failure in the fatal steps (CreateIssue, AddProjectV2ItemById, AddBlockedByIssue, or resolving a marked-but-now-missing child): post error comment naming completed and failed children, add `fabrik:paused` to parent, stop without adding `fabrik:children-spawned`. The comment no longer instructs closing orphaned children (ADR-1583) — it states that already-created children are tracked and will be resumed, not recreated, on the next retry.
5. **Sibling-wiring pass** (second pass, after all children in step 4 exist): for each block that declared `DEPENDS_ON`, call `AddBlockedByIssue(childNodeIDs[ownIndex], childNodeIDs[dependsOnIndex])` — the same primitive used for the parent edge in step 4, now linking two children. Immediately on success, apply `BlockedByEdgeAdded` to the Store too — keyed on the **dependent child's own** `(repo, number)`, not the parent's, mirroring step 4's parent-edge write (#1783 follow-up) so a dependent child is not exposed to the same stale-`BlockedBy` race this ADR closes for the parent. A block may depend on any earlier sibling regardless of where in step 4 its own creation happened to land. On failure: post an error comment (all children created so far are listed), add `fabrik:paused` to parent, stop without adding `fabrik:children-spawned` — the same partial-creation-on-failure shape step 4 already uses, not a new failure mode.
6. After all children are created, linked to the parent, **and** all declared sibling edges are wired: best-effort remove every `fabrik:spawned-child:<blockIndex>:<childNumber>` marker this call knows about (ADR-1583 — steady state carries none of them, they exist only transiently during an interrupted/in-progress spawn), then add `fabrik:children-spawned` to the parent. The idempotency guard is applied for the full two-phase operation (steps 4+5) — after both, not immediately-after-step-4 — specifically so a sibling-wiring failure is retried on the next attempt rather than silently accepted as "done."

**After spawn:** `spawnChildren` returns `(spawned, true, nil)`, where `spawned` lists each child as `owner/repo#N`. From `preImplement`'s call site, `processItem` returns without invoking Claude — decomposition is a terminal outcome for that dispatch (R1, #1783).

**The dispatch gate is a synchronous Store write, not a race against deep-fetch/cycleSet timing (#1783, ADR-1783).** Before #1783, the only thing that could stop a *later* dispatch attempt from running Claude on the full parent was `fabrik:blocked`, and `fabrik:blocked` depended on `checkDependencies` seeing a freshly deep-fetched `BlockedBy` — which required a `LabelsChanged` change (from `fabrik:children-spawned`) to populate `cycleSet`, which required a subsequent poll's `selectDeepFetchCandidates` to admit the item for a real `FetchItemDetails` call. Under GraphQL rate-limit pressure or a transient deep-fetch failure, that chain could take several poll cycles to complete — or, if the item was admitted to dispatch by some other route in the interim (e.g. a comment) while `BlockedBy` was still stale-empty, `checkDependencies` would report "not blocked" and Implement would build the whole parent as a monolith minutes after the children were filed (the #1659 report). Immediately after each child is linked via `AddBlockedByIssue`, `spawnChildren` now also applies a `BlockedByEdgeAdded` mutation (`internal/itemstate`) directly to the engine's own `Store`, appending that one dependency edge with no other deep field touched. Because `boardcache.CacheImpl.FetchProjectBoard` reconstructs every item's `BlockedBy` straight from the Store on every call (`snapshotToProjectItem`), the very next board fetch — this poll's own continuation or the next poll, whichever comes first — already carries the correct edges, with zero dependency on `cycleSet` wake timing, deep-fetch admission, or a live re-fetch succeeding. `checkDependencies` then sees the new `blockedBy` edges — both parent edges and any sibling edges — on that board and adds `fabrik:blocked`, gating the parent until all children close. `checkDependencies` and `PushUnblockObserver` require **no changes** for sibling edges: both already operate generically over any `item.BlockedBy` entry regardless of whether it originated from the parent-wiring step or the sibling-wiring step. The `BlockedByEdgeAdded` write is unconditional per child — it fires whether the edge was freshly linked or found already-linked on an ADR-1583 resume — so a partial (paused) spawn's already-linked children are reflected in the Store even though the batch as a whole did not complete (R5 non-regression).

**Partial spawn failure:** `fabrik:children-spawned` is NOT applied if any step fails — including a `DEPENDS_ON` validation failure (step 1, before any child exists), a board-servability refusal (step 2, before any child exists), or a sibling-wiring failure (step 5, after all children exist but some sibling edges may be unwired). On retry (after user removes `fabrik:paused`), the caller re-runs `spawnChildren` from the beginning — but **resume-safe as of ADR-1583**: any block already recorded via a `fabrik:spawned-child:<blockIndex>:<childNumber>` marker (step 4, above) is recognized and resumed rather than recreated. Users no longer need to manually close orphaned children before re-advancing for the three per-child steps this covers (`CreateIssue`, `AddProjectV2ItemById`, `AddBlockedByIssue`); the sibling `DEPENDS_ON` wiring pass (step 5) itself is unchanged — a raw re-call on every retry, deliberately out of this ADR's scope (its own `AddBlockedByIssue` calls are expected, but not confirmed, to no-op on a repeat).
...
**References:** [ADR-048: Engine-Side Pre-Implement Spawn](../adrs/048-spawn-child-engine-side.md), [ADR-1583: Spawn-Children Resume-Safe Retry via Durable Per-Child Markers](../adrs/1583-spawn-children-resume-safe-retry.md), [ADR-1783: Deterministic Post-Spawn Dependency Gate](../adrs/1783-deterministic-post-spawn-dependency-gate.md)
...
**Same shared `spawnChildren`, same wiring — except ADR-1583's resume marker.** The block list parses via the same `ParseSpawnBlocks`/`spawnBeginRepo`/`isSpawnEndLine` own-line-marker discipline documented in §6.7 (the #1263 hardening applies identically — a Review/Validate output that merely mentions the marker in prose cannot misfire) and is passed to `e.spawnChildren(ctx, board, item, owner, repo, blocks, false)` — the exact function §6.7's Flow documents, including the board-servability scope-check, mandatory assignment, and `blocked_by` linking. Nothing about board registration, assignment, or dependency-edge creation is reimplemented for this call site. The final argument (`resumable`) is `false` here, unlike `preImplement`'s and `recoverMissingPlanComment`'s `true`: ADR-1583's `fabrik:spawned-child:<blockIndex>:<childNumber>` marker assumes `blocks` reparses identically on every retry, which holds for Plan's immutable, already-posted comment but not for this call site's `output` — a fresh Claude generation on every dispatch, including a redispatch after a failed mid-flight spawn's `fabrik:paused` is removed. With `resumable=false`, this origin never reads, writes, or leaves behind that marker; a retried mid-flight spawn always takes the fresh-`CreateIssue` path for every block, exactly as it did before ADR-1583 — carrying forward that ADR's original duplicate-on-retry exposure for this origin specifically, rather than risking the marker resuming an unrelated, already-created child under a same-numbered but different block on a non-deterministic redispatch (caught in review on PR #1708).
...
**No change to `spawnChildren`'s own idempotency guard *from this section's fix*.** `fabrik:children-spawned` continues to be added to the **parent** unconditionally once all children are created, added to the board, and linked — regardless of each individual child's board-placement outcome. This section only makes the child's own placement durable/recoverable; it does not make the parent's guard conditional on it (per the issue's explicit scope). (A separate mechanism, ADR-1583/§6.7, does change what a *retried* spawn does before this guard is ever reached — see "Resume-safe retry" in §6.7's Flow step 4 — but that fix is orthogonal to board-placement recoverability, which this section covers.)
...
5a. **Suite-only holds are bounded the same way (#1822).** A hold caused by an outstanding check suite (§6.4 *Suite-aware completeness*) is `PRMergeUnsettled`, claimed by `checkMergeabilityGate` before `checkCIGate`, so it is bounded by `CIBackstopTimeout`, not the liveness dwell (no check run is pending, so `LastCIProgressAt` never advances for it). `pauseForCITimeout` names the outstanding suite and app. The train polls' suite holds are likewise bounded by their existing `CIBackstopTimeout` deadline.
...
**Scope: mid-flight ejection only, no admission-time filtering.** A member with an unresolved thread is still admitted into a *forming* batch; it is ejected once this scan (or a worker checkpoint) observes the finding. `groupQueuedByRepoAndBase`/`routeQueuedGroup`'s batch-composition logic is untouched.
...
| Merge-train singleton fast path (`trySingletonFastPath`, §6.20, ADR-1644) | The member's **own** PR — no separate PR is ever minted | `FetchLinkedPR(owner, repo, issueNumber)` — durably rediscoverable, same as the ordinary auto-merge path, no `fabrik:credited-pr:<N>` needed |
...
**Code path:** `landMergeTrainBatch`/`landSingleton`/`finishSingletonFastPathLanding` (`engine/merge_train.go`), `advanceConvergedPRToDone` (`engine/merge_gate.go`), `advanceValidateTerminalItem`'s merged-PR branch (`engine/pr_terminal_advance.go`) — each applies `fabrik:awaiting-landing-verification` (and, for `landMergeTrainBatch`/`landSingleton` only, `fabrik:credited-pr:<N>`) immediately after its own successful Done transition. `finishSingletonFastPathLanding` (§6.20, ADR-1644) follows `advanceConvergedPRToDone`'s labeling exactly — see the table above. Retry-owner: `settleLandingVerification` → `settleLandingVerificationItem` (`engine/landing_verification_settle.go`; `settleLandingVerification` called from `poll()` in `engine/poll.go`, immediately after `settleNonDefaultBaseCloses`).
...
### 6.20 Merge-Train Singleton Fast Path (ADR-1644)

**Trigger:** Issue #1644. A merge-train batch containing exactly one Queued member pays the full trial cycle — a disposable trial worktree, a pushed branch, a draft CI PR, a complete CI run, a landing integration PR — even when the member's own PR has already been validated against the exact tree the trial would produce. Fabrik's own Validate-stage convention (rebase onto base before signalling completion) means a member arriving fresh at Queued is typically already up to date with base, making the trial's re-validation redundant by construction on the common path. This is not a bug — ADR-1153's check-completeness gate and `pollTrainCI`'s backstop timeout both behave correctly throughout — it is avoidable, correctly-performed, duplicate work.

**Mechanism: skip the trial only on positive evidence, never on absence of evidence.** `trySingletonFastPath` (`engine/merge_train.go`), called from `runMergeTrainWorker`'s re-form loop on every iteration where `len(current) == 1`, immediately before a trial would otherwise be built. `singletonFastPathEligible` requires **all four** of the following to hold; any single one failing — including any API error — falls through to the unmodified trial path:

1. **No re-push since batch formation (TOCTOU guard).** The member's live PR head SHA (fetched fresh via `FetchPRDetails`) still equals the `trainMember.headSHA` snapshot captured once by `fetchTrainMembers` at worker start. A member re-pushed between batch formation and this check must never be landed on stale evidence.
2. **The pinned base SHA is an ancestor of the member's head.** `FetchCommitsBehind(owner, repo, p.baseSHA, m.headSHA) == 0` — checked against `trialParams.baseSHA`, the base pinned once at batch start (ADR-059 D-b), **never a live `origin/<base>` read** (R3), so a base that moves mid-flight cannot make a stale decision look current. `behind_by == 0` means the pinned base is a fast-forward ancestor of the member's head, so the trial's tree (`pinned_base + member`) would be byte-identical to the member's own PR head tree.
3. **The PR is mergeable.** `gh.MergeableStateAccepted(pr.MergeableState)` (ADR-072) — the same `{clean, unstable}` acceptance test `MergePR`'s own self-gate uses; a `dirty` state means an actual conflict.
4. **The member's own CI is green *and complete*.** `FetchCheckRuns` must return at least one check run — zero check runs is rejected outright, *before* `classifyLandingCI` is even consulted, regardless of `mergeable_state` (see below for why this is stricter than `classifyLandingCI`'s own zero-check-runs handling). Given at least one check run, `classifyLandingCI` (ADR-1153/ADR-1441, reused unmodified — the same check-run-based classifier `pollForMergeable` uses for the ordinary landing path) must return `TrainCIGreen`. Since #1822 `classifyLandingCI` additionally returns `TrainCIPending` while any check suite has outstanding work or the suite read fails (§6.4 *Suite-aware completeness*), so a member whose own all-green runs are only a prefix of a still-running workflow is **not** eligible and falls through to the trial path.

Every branch that cannot positively confirm a condition returns `(false, reason)` — there is no code path that returns eligible except by every check having been positively satisfied. This is the deliberate inverse of #1614's failure mode: there the train wrongly *skipped* a merge; here a wrong skip would mean *skipping validation*, so only positive evidence may authorize it (R2).

**Why zero check runs is rejected outright, not delegated to `classifyLandingCI`'s own zero-check-runs fallback.** `classifyLandingCI`'s zero-check-runs branch falls back to `mergeable_state` alone (ADR-933's "Actions disabled" case) — correct for its usual caller, `pollForMergeable`, which judges a landing PR whose tree a just-polled *trial* CI run already validated, so zero check runs there is a benign GitHub quirk, not an absence of validation. This fast path never builds a trial, so that precondition doesn't hold: zero check runs on the member's own PR head means nothing has actually validated that tree, not "green by absence." `singletonFastPathEligible` therefore rejects on zero check runs before `classifyLandingCI` is ever consulted — a fast-path-specific precondition layered in front of the reused classifier, not a fork of it.

**Why this is a loop-level guard, not a change to `assembleAndValidate`.** `assembleAndValidate` has four call sites: the main re-form loop (the only genuinely-fresh Queued-batch decision), `bisect`'s poisoner-isolation sub-trials (a member's own-PR CI passing says nothing about whether it's the poisoner in an already-red combination — fast-pathing here would be a correctness bug, not just scope creep), `landOneAtATime`'s one-at-a-time fallback (explicitly out of scope — `landSingleton`'s existing fallback role is unchanged), and `landGreenBatch`'s main-moved rebase loop (D5 recovery of an *already-green* trial after base moved — a different problem from "should a trial be built at all"). Placing the guard inside `assembleAndValidate` itself would silently reach all four; placing it in the loop, immediately before the call, reaches only the one call site this feature targets. Applied on every loop iteration, not just the first, so a batch that bisects down to a single clean survivor and re-forms is also eligible.

**Pending review-finding ejects (#1208) are applied before the fast path is even considered.** `applyPendingReviewEjects` (§6.16) existed at three checkpoints before this feature, all downstream of a call to `assembleAndValidate` — the re-form loop's own Hook 2, `landOneAtATime`'s per-singleton loop, `landGreenBatch`'s main-moved rebase loop. The fast path never calls `assembleAndValidate`, so without a fourth checkpoint specific to it, a member flagged mid-trial for unresolved review-thread findings could be landed directly, never reaching any of the three existing checkpoints. The guard therefore applies `applyPendingReviewEjects` to the length-1 `current` slice *before* calling `trySingletonFastPath`: an ejected member leaves `current` empty, and the loop `continue`s into the top-of-loop zero-survivors return; an unflagged member falls through to the fast path exactly as before. (Found in review — the original implementation had no such checkpoint.)

**Landing shape: `advanceConvergedPRToDone`, not `landSingleton`.** `landSingleton` (the bisection fallback's per-member landing helper) is built entirely around minting a *dedicated* landing PR off a validated trial branch — a shape this feature has no trial branch to produce. Instead, `finishSingletonFastPathLanding` follows the ordinary auto-merge path's own Done-transition template: `MergePRAtHeadSHA(owner, repo, m.prNum, m.headSHA)` merges the member's **own** PR directly, pinned to the exact head SHA `singletonFastPathEligible` validated (see below), `recordAdvanceOutcome` advances Queued → Done (with the same durable failure-retry/escalation every terminal advance gets, §6.17), `closeIssueIfNonDefaultBase` runs unconditionally on the confirmed merge (see below), and — only once the advance itself succeeded — `awaitingLandingVerificationLabel` is applied with **no** `fabrik:credited-pr:<N>` label, since the merged PR *is* the item's own linked PR and is durably rediscoverable via the ordinary `FetchLinkedPR` fallback (§6.19's table, row 1) — unlike merge-train's other two landing paths, whose credited PR is never the member's own. `resetEjectionCount`/`resetTrialCounter` still run, exactly as every other successful land performs.

**The landed comment, and why its shape differs from the other two paths.** Every merge-train landing path posts a `🏭 **Fabrik merge-train** — Landed …` comment on the member's own PR — the sole cross-landing-path, member-scoped record of which PR actually landed the change (#1275). `finishSingletonFastPathLanding` posts `Landed via singleton fast path PR #<n>.`, and here `<n>` is the **member's own PR number**, because that PR *is* the merge vehicle — there is no distinct integration or singleton PR to name, by construction. Under `landMergeTrainBatch` (`Landed via batch PR #<n>.`) and `landSingleton` (`Landed one-at-a-time via singleton PR #<n>.`) the cited PR is always distinct from the member's, and a comment citing the member's own PR there would indicate a direct merge that bypassed the train. Anything reading these comments must therefore key off the path named in the text, not off distinctness alone (`tests/e2e/mergetrain_helpers.go`'s `landedPRPattern`/`waitForLandingPRDetail` do exactly this).

Unlike the other two paths, this one is **guarded**, rather than posted unconditionally. `landMergeTrainBatch`/`landSingleton` close the member's PR immediately after commenting, so they can never be re-entered for the same member; this path leaves the PR to GitHub's own merge-close and can legitimately be re-entered while the member is still Queued (the `pr.Merged` restart branch above, and a retry after a failed `recordAdvanceOutcome`). The guard is two-part, and both halves are load-bearing. The comment posts only when *this* call actually recorded a completed landing — the same condition that applies `fabrik:awaiting-landing-verification`, i.e. a successful `recordAdvanceOutcome` — **and** only when that label is absent from the item's own snapshot, meaning no earlier call already did. The label's absence alone is insufficient: it is applied only *after* the advance succeeds, so on a failed advance (a missing board Status option, say — the case `fabrik:awaiting-advance` exists to retry, §6.17) the member stays Queued with its PR already merged and re-enters every poll with the guard still open, posting one duplicate per poll until `MaxRetries` escalates to `fabrik:paused`. Tying the comment to the successful-label condition closes that window without suppressing it permanently: no advance, no comment, and the retry that eventually advances posts exactly one. The `landed` half alone is equally insufficient, since the already-Done restart branch re-applies the label idempotently on every re-entry. As with the other paths, the comment gates nothing: `addLandedCommentWithRetry` retries transient failures and then warns and continues — a landing is never blocked or delayed on it.

**Why `MergePRAtHeadSHA`, not the plain `MergePR` every other merge-train landing path uses.** `MergePR`'s own self-gate re-reads `mergeable`/`mergeable_state` live immediately before merging, but never confirms the head is still the exact commit some earlier, separate check validated — immaterial for `landSingleton`/`landMergeTrainBatch`/the ordinary auto-merge path, since all three merge a Fabrik-owned trial, integration, or landing branch nobody else pushes to. This fast path is the first merge-train landing path to merge the member's own, externally-writable PR branch directly *after* a separate, earlier CI/ancestry classification — so a commit landing on that branch between the classification and the merge call would otherwise be merged having never been validated at all. `MergePRAtHeadSHA` closes this by passing the expected head SHA through to GitHub's merge endpoint's own `sha` field: a request whose live head no longer matches is refused (`gh.ErrConflict`, from GitHub's 409) rather than silently merging whatever is actually there. A refusal is handled identically to any other merge failure at this call site (see the state-transition table below) — the member stays in Queued for a fresh, correctly re-evaluated attempt next poll.

**`closeIssueIfNonDefaultBase` is required here for the first time in merge-train.** Both existing merge-train landing paths (`landMergeTrainBatch`, `landSingleton`) always call `CloseIssue` explicitly regardless of base, so neither ever needed this guard. This fast path relies on the merged PR's own `Closes #N` (R5's stated rationale for landing the member's own PR directly) — but GitHub only auto-honours that keyword into the repository's *default* branch (#1096). A `base:<branch>`-labeled Queued member would otherwise land but never close; `closeIssueIfNonDefaultBase` is the same best-effort, `fabrik:awaiting-close`-backed guard (§6.13) the ordinary auto-merge path already uses.

**Restart safety is close to free.** `MergePRAtHeadSHA` shares `MergePR`'s idempotent-on-already-merged behavior (returns immediately once the PR reports `merged`). `trySingletonFastPath` checks `pr.Merged` before running eligibility at all: if a prior run merged the member's own PR but died before the Done transition, the next poll's fast-path check resumes straight to `finishSingletonFastPathLanding` without re-evaluating eligibility (which could plausibly now read differently against an already-merged PR). `finishSingletonFastPathLanding` itself mirrors `landMergeTrainBatch`'s "already Done" restart branch: when `m.item.Status == "Done"`, it still re-applies `closeIssueIfNonDefaultBase` and the landing-verification label, both idempotent.

**No interaction with the runaway guard or trial counter.** `e.recordTrial` (the wrapper around `assembleAndValidate` that feeds `isRunawayTripped`) only fires for non-green results and is never invoked by the fast path, since the fast path never calls `assembleAndValidate` at all. A landed fast path is unambiguous progress, identical in kind to any other green land.

**Contrast with batch admission (§6.25).** The fast path is fail-*closed*: it skips validation, so only positive evidence of green authorises it. §6.25's admission gate at fresh batch formation is the deliberate inverse, fail-*open*: it removes a member from a batch, so only positive evidence of red (`classifyLandingCI` → `TrainCIRed`, reused unmodified) acts, and every ambiguity admits.

**Logging (R4).** Every disposition — eligible-and-landed, or the specific unmet condition that forced a trial — is logged from `trySingletonFastPath`/`singletonFastPathEligible`, so "why didn't it fast-path?" is answerable from logs without inference. On a completed landing `finishSingletonFastPathLanding` additionally emits the repo-scoped `landing complete for <trainKey> (singleton fast path, PR #<n>, 1 member)` line that `landMergeTrainBatch` emits for a batch — the train's terminal progress signal on its TUI job row, and the only log-visible statement that a train actually reached a landing. It is repo-scoped (`logfRepo`), not per-issue: a per-issue line would not reach the train's job row. Unlike the landed comment above it is emitted unconditionally rather than guarded, since a repeat on re-entry is accurate and is itself the signal that a re-entry happened.

**Code path:** `trySingletonFastPath`, `singletonFastPathEligible`, `finishSingletonFastPathLanding` (`engine/merge_train.go`), called from `runMergeTrainWorker`'s re-form loop. See §6.19's table for how this path's credited PR is discovered post-Done.

**State transitions:**

| Before | Trigger | After | Labels Added | Labels Removed |
|---|---|---|---|---|
| Queued, single member, eligibility unresolved | `trySingletonFastPath`: any condition not positively confirmed (TOCTOU mismatch, ancestry not confirmed, not mergeable, CI not green/complete, or an API error) | Queued (unchanged) — falls through to the ordinary trial path | — | — |
| Queued, single member, all conditions positively confirmed | `MergePRAtHeadSHA` succeeds; `recordAdvanceOutcome` succeeds | Done, awaiting landing verification | `fabrik:awaiting-landing-verification` | — |
| Queued, single member, all conditions positively confirmed | `MergePRAtHeadSHA` succeeds; `recordAdvanceOutcome` fails | Queued (unchanged); advance failure durably retried (§6.17) | — | — |
| Queued, single member, all conditions positively confirmed | `MergePRAtHeadSHA` fails (transient, or the pinned head SHA no longer matches — `gh.ErrConflict`) | Queued (unchanged); retried next poll | — | — |
| Queued, single member, PR already merged (restart) | `trySingletonFastPath` observes `pr.Merged == true` | Resumes straight to the Done transition, as above | (as above) | — |
| Queued, single member, pending review-finding eject flagged (#1208) | `applyPendingReviewEjects`, applied before the fast path is considered | Rerouted off Queued (§6.16) — never reaches `trySingletonFastPath` | — | — |

**References:** [ADR-1644: Merge-Train Singleton Fast Path](../adrs/1644-merge-train-singleton-fast-path.md), [ADR-1153: Train CI Completeness over Mergeable State](../adrs/1153-train-ci-completeness-over-mergeable-state.md), [ADR-1441: Unstable Requires Check-Run Classification](../adrs/1441-unstable-requires-check-run-classification.md), [ADR-072: MergePR Self-Gates on Mergeable State](../adrs/072-mergepr-self-gates-on-mergeable-state.md), [ADR-1208: Queued Review-Finding Ejection](../adrs/1208-queued-review-finding-ejection.md), §6.13 (`closeIssueIfNonDefaultBase`), §6.16 (`applyPendingReviewEjects`), §6.19 (post-Done landing verification), issue #1644

---

### 6.21 Non-Default-Base Linkage Notice (#1649)

**Trigger:** GitHub's `Closes #N` inertness on a non-default base branch (§2.8, ADR-1096) goes further than suppressing auto-close: per [GitHub's own docs](https://docs.github.com/en/issues/tracking-your-work-with-issues/using-issues/linking-a-pull-request-to-an-issue), closing keywords "are interpreted only when the pull request targets the repository's default branch… otherwise these keywords are ignored, no links are created" — so a `base:<branch>` item's issue shows **no PR at all** in its Development panel. Fabrik itself is unaffected (`FetchLinkedPR` resolves by the `fabrik/issue-N` head-branch convention, never GitHub's linkage graph), but the operator looking at the board has no visual cue and has to go hunting. Reported as the "adjacent, lower priority" half of #1646; ADR-1096/1097 already cover the *closing* half — this covers the purely human-visible half.

**Durable marker (`fabrik:nondefault-base-pr-noted`), written unconditionally after the post attempt.** Unlike `fabrik:awaiting-close` (§6.13), this label is not written only in a failure branch — it's added right after `postItemComment` is called, whether or not that call actually succeeded. There is no durable outstanding action to distinguish a "written on failure, retry later" state from: a missed comment (transient `AddComment` error) is simply gone, not queued. This mirrors `fabrik:api-key-helper-detected`'s accepted trade-off (CLAUDE.md), not ADR-1097's retry/escalation shape.

**Why no retry/escalation machinery, deliberately.** `fabrik:awaiting-close`'s retry loop exists because a stalled explicit close has a real downstream cost — dependent issues that unblock on close (not merge) stay blocked indefinitely. A missed one-time informational comment has no such cost: nothing gates on its delivery, no dependent issue waits on it, and the underlying fact it reports (this PR has no GitHub linkage) remains discoverable by other means (the issue's `base:<branch>` label itself, or a `cross-referenced` timeline event GitHub still creates). Adding `recordSettleRetry`/`escalateSettle` here would be retry machinery in search of a problem it doesn't have. R2's "must survive restarts and repeated polls without duplicating" is satisfied entirely by the label's own presence — it says nothing about guaranteeing eventual delivery.

**Code path:** `noteNonDefaultBasePRLinkage` (`engine/nondefault_base_linkage_notice.go`), reusing `itemHasBaseLabel`/`baseBranchForItem` (§6.13's `closeIssueIfNonDefaultBase` precedent) for the base-vs-default comparison. Retry-owner (detection only, not delivery): `settleNonDefaultBaseLinkageNotice`, called from `poll()` in `engine/poll.go`, immediately after `settleNonDefaultBaseCloses`.

**`settleNonDefaultBaseLinkageNotice` flow**, run unconditionally every poll over raw `board.Items` (ADR-1270 family, ninth-or-later instance — ordinal not load-bearing), independent of `itemMayNeedWork`/`itemNeedsWork` dispatch and of the item's board column (eligibility is "has a base: label and a discoverable PR," not "reached a particular stage or Done"):

1. `item.IsPR` → skip.
2. `!itemHasBaseLabel(item)` → skip (R3's fast path: per `baseBranchForItem`'s documented contract, an item with no `base:` label always resolves to exactly the default, so the overwhelming common case never touches a `WorktreeManager` or does a git shell-out).
3. `item.LinkedPRNumber == 0` → skip (R1: no PR discovered yet — `LinkedPRNumber` is already populated on the board snapshot via the deep-fetch cache's `FetchLinkedPR`, so this costs no new API call, R4).
4. `hasLabel(item.Labels, "fabrik:nondefault-base-pr-noted")` → skip (R2 idempotency).
5. Non-panicking `WorktreeManager` lookup (`e.mu.Lock(); wm, ok := e.worktreeManagers[key]; e.mu.Unlock()`), mirroring `closeIssueIfNonDefaultBase` — skip-and-log if unregistered (self-heals next poll once a dispatch registers it; same restart-window caveat as ADR-1096).
6. `base, err := e.baseBranchForItem(item, wm)` → skip-and-log on error.
7. `def, err := wm.DefaultBaseBranch()` → skip-and-log on error.
8. `base == def` → skip (R3: covers both a genuinely default-base item and the case where `baseBranchForItem`'s own remote-fallback already reduced the effective base to default — e.g. the labeled branch doesn't exist on the remote, in which case GitHub's ordinary linkage applies and no notice is warranted).
9. Otherwise: post the one-time comment naming the PR (`item.LinkedPRNumber`), the resolved base, and the repository default; then unconditionally add `fabrik:nondefault-base-pr-noted`.

**A distinct comment from `baseBranchForItem`'s own fallback notice.** Step 6 can itself post a *different* one-time comment ("base branch not found on remote, falling back to default"), gated by an in-memory-only `sync.Map` (not restart-durable, unlike this feature's own label-gated marker). When that fallback fires, step 8's `base == def` comparison correctly no-ops this feature's notice — but the two comments are independent and must not be conflated when reading an issue's thread or writing tests against it.

**Closed issues are not excluded.** Unlike `closeIssueIfNonDefaultBase` (which treats an already-closed issue as nothing-to-do, since *closing* an already-closed issue is a no-op action), this scan posts regardless of `item.IsClosed` — the information ("this PR was never GitHub-linked") remains true and useful for an issue closed by any means, and posting an informational comment is not the same kind of action as an idempotent-by-necessity state mutation.

**State transitions:**

| Before | Trigger | After | Labels Added | Labels Removed |
|---|---|---|---|---|
| `base:<branch>` item, PR discovered, base ≠ default | Settle pass reaches step 9 | Notice posted | `fabrik:nondefault-base-pr-noted` | — |
| `base:<branch>` item, no PR yet, or base == default (incl. remote-fallback) | Settle pass reaches step 3 or 8 | Unchanged | — | — |
...
### 6.23 Autonomy-Label Live Re-Read at Both Advance Decisions (#1769)

**Trigger:** An operator adds or removes `fabrik:yolo`/`fabrik:cruise` on an issue while a stage is in flight, intending to change the advance/auto-merge decision the engine is about to make for that issue.

**The problem this section fixes.** Two independent advance paths read `hasYoloLabel(item)`/`hasCruiseLabel(item)` off `item.Labels` at the moment they decide whether to advance the stage (and, at Validate, whether to enable auto-merge):

- **Path 1 — `handleStageComplete`** (`engine/stages.go`), which runs synchronously right after a stage's `FABRIK_STAGE_COMPLETE`. `item` here is the pre-invocation dispatch-time snapshot, which can be stale by tens of minutes on a long-running stage.
- **Path 2 — the catch-up loop's `runCatchUpPhase2`** (`engine/poll.go`), reached whenever `stage:<name>:complete` is already present — which is every `wait_for_ci: true` stage, since `handleStageComplete` defers to the catch-up loop there (§6.5) rather than reaching its own advance logic. On the default stage set this is **Validate**, where advancing under withdrawn authority means auto-merging the PR.

Before this fix, Path 1 *attempted* a mid-run label re-fetch but targeted `e.cfg.Owner`/`e.cfg.Repo` — empty in multi-repo mode (a 404, silently swallowed by an `err == nil &&` guard) and, on a single-repo instance whose config repo differs from the item's own, the wrong issue's labels entirely. Path 2 performed no re-fetch at all: `item.Labels` is not part of the deep-fetch cache contract (`copyDeepFieldsFromState` excludes `Labels`), so even an item that went through a "fresh" deep fetch earlier in the same poll pass could still carry stale autonomy labels at the point `runCatchUpPhase2` reads them. Both defects meant a label *removal* made mid-run was never observed by either path — the stage would complete and auto-advance (or auto-merge) under authority the operator had already withdrawn, only picking up the change on the next full board refresh.

**The fix: one shared helper, consulted by both paths.** `refreshAutonomyLabels(item *gh.ProjectItem)` (`engine/stages.go`) performs a live `FetchLabels` call scoped to the item's own resolved `owner, repo` (`itemOwnerRepo`, never `e.cfg.Owner`/`e.cfg.Repo`) via `e.client` (the raw, uncached `GitHubClient` — never `e.readClient`, since `Labels` is not part of that cache's deep-field contract regardless), and mutates `item.Labels` in place. Both `handleStageComplete` and `runCatchUpPhase2` call it as the very first step of their autonomy-decision logic, before any `hasYoloLabel`/`hasCruiseLabel` read — mirroring `effectiveReviewAuthority`/`effectiveExpectedReviewers`'s "one shared helper, every gate" precedent (ADR-1250, ADR-1283, `engine/reviews.go`), except — unlike those two, which only resolve a label already present on the snapshot — this helper performs the fetch itself.

On a fetch error, `item.Labels` is left untouched and a warning naming the item and the error is logged — silent fallback is exactly what hid this defect, and does not survive the fix. On a successful fetch, `item.Labels` is replaced unconditionally, including when the fresh result is an empty label set: a real fetch returning zero labels is data, not a failure, and must not be discarded the way the old `len(freshLabels) > 0` guard discarded it.

**One fetch covers every downstream autonomy-label read.** `attemptMergeOnValidate` (the single landing-decision function both paths call — ADR-1216) receives `item` by value and performs its own `hasCruiseLabel(item)` check. Because both callers invoke `refreshAutonomyLabels` upstream of every `attemptMergeOnValidate` call site, that check is fresh too, without a second REST call from this fix — bounding the *autonomy-label* fetch cost to exactly one live `FetchLabels` per `handleStageComplete`/`runCatchUpPhase2` invocation (i.e. at most once per item per poll, since the two paths are mutually exclusive admission for the same item in the same pass). This is narrower than "one fetch total," though: `attemptMergeOnValidate` already performs its own live re-read for an unrelated purpose — the pre-existing ADR-1419 `FetchItemDetails` dependency guard (for `BlockedBy`) — and that call's GraphQL query resets `item.Labels` as a side effect, overwriting the REST-fresh snapshot `refreshAutonomyLabels` just produced. On the Validate/yolo merge path, `item.Labels` is genuinely fetched twice per invocation, not once. This is harmless today (the dependency guard runs after both the `hasCruiseLabel` check and the `fabrik:auto-merge-enabled` idempotency check, and nothing later in the function reads `hasYoloLabel`/`hasCruiseLabel`), but a future label check added after the dependency guard must not assume it is reading the REST-fresh snapshot.

**Code path:** `refreshAutonomyLabels` (`engine/stages.go`); called from `handleStageComplete` (`engine/stages.go`) and `runCatchUpPhase2` (`engine/poll.go`).
...
capable of carrying a stale pinned base as a freshly-created one. **The check is skipped, however,
when the found `integrationPR` is already merged** (`integrationPR != nil && integrationPR.Merged`
— the FR-2 restart-safety state, e.g. an engine restart between merging the integration PR and
advancing its members to Done): nothing is being opened or reused in that state, so refusing there
would only strand an already-landed batch in Queued forever on a transient `FetchLabels` error or a
label edited after the merge, blocking FR-3's advance-to-Done/close-PR logic for no protective
benefit — found during PR review and fixed in the same PR (`TestLandMergeTrainBatch_AlreadyMergedPR_SkipsBaseCheck`).
`landSingleton` (the FR-5 one-at-a-time fallback, not a rare corner case — any bisection failure
routes through it) calls the check immediately before its own `CreatePR` call; it never searches for
or reuses an existing PR at all (by design — see the ADR-059 D4 data-loss note in its own doc
comment), so it has no equivalent already-merged state to guard against. The singleton *fast path*
...
| `landMergeTrainBatch` found an existing integration PR that is already merged (FR-2 restart-safety state) | `integrationPR != nil && integrationPR.Merged` — the check is skipped entirely, regardless of member labels | FR-3 (advance to Done, close member PR/issue) proceeds normally; no `FetchLabels` calls made, no refusal possible | — | — |
...
### 6.25 Merge-Train Own-PR CI Admission Gate (ADR-1821)

**Trigger:** Issue #1821. `fetchTrainMembers` validated exactly two things per Queued member — a linked PR exists and has a head SHA — and never consulted the member's CI. A member whose **own** PR checks were already red was therefore admitted, merged into a trial, and only discovered by that trial's CI (or by bisection): on `verveguy/concept-maps` two such members cost roughly 2 hours each (an ~54-minute assembly, a 24-minute CI cycle, then re-bisection) to rediscover a fact that was readable off the members' own check-runs in one API call. `classifyLandingCI` already answers the question and is already trusted for a landing decision (§6.20 step 4), but was only applied on the singleton fast path, never at admission.

**Mechanism: a fail-open admission gate at fresh batch formation.** `admitTrainMembers` (`engine/merge_train_admission.go`) is called from `prepareTrainWorker` immediately after `fetchTrainMembers`. For each member it reads `e.client.FetchCheckRuns(owner, repo, m.headSHA)` — the live REST client, at the head SHA `fetchTrainMembers` snapshotted, so the evidence corresponds to the exact commit that would enter the trial — and classifies it with `classifyLandingCI`, **unmodified**. The gate has no notion of red of its own: "confirmed red" is exactly `TrainCIRed`, which includes a required-context failure.

**Polarity is fail-open — the inverse of §6.20.** The singleton fast path skips validation, so it authorises only on positive evidence and every ambiguity blocks it. The gate *removes* a member from a batch, so it acts only on positive evidence of failure and every ambiguity admits:

| `classifyLandingCI` / read outcome | Gate action |
|---|---|
| `TrainCIRed` (failed check run, or required-context failure) | **Defer** (below) |
| `TrainCIPending` (incl. a rerun in progress masking an older failure) | Admit |
| `TrainCIGreen` | Admit |
| Zero check runs, no required-context failure | Admit (the fast path's fail-closed zero-run rejection is deliberately *not* copied) |
| `FetchCheckRuns` error | Admit, logged as `fetch-error` |
| Context cancelled mid-gate | Admit the remainder unchecked |

Admitting is byte-identical to pre-#1821 behavior. Deferring on ambiguity would silently drain the queue and strand members that have done nothing wrong; the cost of the fail-open choice is that a red member with a pending run, a zero-run head, or an unreadable read is discovered by the trial exactly as it always was.

**Scope: fresh formation only (R7).** The gate lives in `prepareTrainWorker`, *after* `reconstructTrainState` has returned `true` for both restart routes (`completeDeferredLanding` of an already-merged marker PR, and `resumeTrain` of an existing in-flight trial), and it is not inside `fetchTrainMembers` itself. Those routes re-resolve an already-decided batch and must not re-litigate admission. Bisection sub-trials, `landOneAtATime`, and `landGreenBatch` never pass through it either. The gate needs `state.projectID` for the reroute, which `fetchTrainMembers` does not have.

**Precondition: the reroute target must have `wait_for_ci` (R9).** A deferred member is rerouted to `stageBeforeHolding` (normally Validate) with its `stage:<X>:complete` label intact. What re-picks it up is Phase 1's `handleMergeAndCIGates`: `settlePRMergeState` classifies red checks as `PRMergeBlocked`, `checkCIGate` returns `ciFailure`, `fabrik:awaiting-ci` is applied and `dispatchCIFixReinvoke` runs (bounded by `MaxCiFixCycles`, with the no-op-SHA debounce). Phase 2 — the only place `attemptMergeOnValidate` → `advanceToQueued` runs — is never reached for a claimed item, so a red member is not advanced straight back to Queued, even under `fabrik:yolo`. But `checkCIGate` is a no-op unless the stage has `wait_for_ci: true`; without it nothing would claim the member (under yolo it would bounce back to Queued and be re-deferred every poll; under cruise it would strand). The gate therefore checks the target stage first and, if it lacks `wait_for_ci`, admits every member without reading anything, logging once per formation. The shipped and live `validate.yaml` both set `wait_for_ci: true`; a custom config that omits it simply gets no gate.

**Deferral (`deferRedMember`).** Modelled on `ejectRedSingleton` (#1545) minus the pause:

1. The member is dropped from the batch unconditionally — it is confirmed red.
2. `rerouteQueuedMemberOffHolding` runs **first** (the ADR-1208 reroute-before-side-effects primitive). On failure nothing is posted, consumed, or recorded; the next poll's fresh formation retries the whole operation. The member stays out of the current batch regardless.
3. On success, any pending review-finding eject signal for the member (`takePendingReviewEject`, §6.16) is consumed and logged, so a member flagged both ways is never double-routed or double-commented and a stale signal cannot later fire on a re-queued member that already addressed the finding.
4. One comment is posted: header `🏭 **Fabrik merge-train — deferred (own CI failing)**`, the failing check(s) via `renderFailedChecks` (falling back to the classifier's `detail` for a required-context red, which has no failing runs to render), a statement that the failure is the PR's own and not a train interaction, and that the member has **not** been paused — the CI gate will dispatch a fix from the target stage and the issue re-queues once CI is green and that stage completes again. It is composed locally: `ejectMember(stayInQueue=false)` hard-codes review-finding wording, and `renderDiagnosticBlock` / `renderBatchContext` / `reentryInstruction` carry trial/integration-PR wording and a pause-assuming recovery instruction that are wrong for this cause.

**Never paused, never counted (R5).** A deferral does not touch `mergeTrainEjectionCounts` and never calls `pauseMergeTrainMember`, so it cannot accumulate toward `MaxMergeTrainEjections`. This is the opposite of `ejectRedSingleton`'s reasoning: there the failure was only ever observed on a synthetic trial branch, so nothing external would re-detect it and a pause was the only human gate. Here the failure is observable on the member's own PR check-runs, so the ordinary CI gate has an external signal to act on.

**Ping-pong backstop (R9).** `mergeTrainCIDeferred` (`owner/repo#N` → head SHA, in memory, `mergeTrainCIDeferredMu`) records the SHA each member was last deferred at. A repeat deferral of the *same* SHA — reachable only if the live read shows red while Phase 1's cache-backed read does not — still reroutes but posts no second comment. The entry is cleared when the member is next admitted non-red. It resets on restart, costing at most one duplicate comment.

**Cost.** One `FetchCheckRuns` per member per fresh batch formation, sequentially, bounded by `MaxBatchSize`. A pending or timed-out trial re-forms the batch on the next poll, so the read repeats per member per poll until a landing or eject. `classifyLandingCI` additionally calls `FetchCombinedStatus` (cache-backed) only when `required_status_contexts` is configured and a required name is absent from the check runs. A one-member batch reads check runs again in `singletonFastPathEligible`. The small per-poll cost is set against ~2 hours avoided per red member. Every disposition — defer, or admit with its reason (`pending`, `green`, `fetch-error`, `no-wait-for-ci`) — is logged with the classifier's `detail`.

**A new sustained label state.** A deferred member carries both `stage:<X>:complete` and `fabrik:awaiting-ci` for the whole CI-fix cycle (`classifyCIFromCheckRuns` adds the label but never removes the completion label). Both the main Phase 1 loop and `settleAwaitingCIScan` (ADR-1270) run the same handler chain on it; duplicate dispatch is prevented by the `snap.Worker() != nil` guard and duplicate pause comments by `hasCIGatePauseComment`. `MaxCiFixCycles` and the `CIBackstopTimeout` can still pause a slow fix — an ordinary Validate pause, not a deferral pause.

**Crash window.** The reroute and the comment are two calls; a crash between them leaves a rerouted member with no comment, matching ADR-1208 and ADR-1545.

**Code path:** `admitTrainMembers`, `deferRedMember`, `composeCIDeferralBody` (`engine/merge_train_admission.go`), called from `prepareTrainWorker` (`engine/merge_train.go`).

**State transitions:**

| Before | Trigger | After | Labels Added | Labels Removed |
|---|---|---|---|---|
| Queued member, fresh batch formation, reroute target has `wait_for_ci` | `classifyLandingCI` → `TrainCIRed` on the member's own head; reroute succeeds | Rerouted to the stage before Queued; excluded from the batch; one comment; ordinary CI gate takes over | — | — |
| (as above) | `TrainCIRed`; reroute fails | Queued (unchanged), excluded from this batch; no comment, no dedupe record; whole operation retried next poll | — | — |
| (as above) | `TrainCIRed` again at the same head SHA (e.g. stale-cache bounce) | Rerouted again; no repeat comment | — | — |
| (as above) | Pending, green, zero check runs, or `FetchCheckRuns` error | Admitted to the batch, unchanged from pre-#1821 | — | — |
| Queued member, reroute target lacks `wait_for_ci` | Gate skipped before any read | Admitted, unchanged | — | — |
| Queued member, restart reconstruction (Route 1 / `resumeTrain`), bisection, `landOneAtATime`, `landGreenBatch` | Never reaches the gate | Unchanged | — | — |
| Deferred member at the reroute target, red checks | Phase 1 `handleMergeAndCIGates` → `ciFailure` | CI-fix reinvoke dispatched; not advanced (Phase 2 unreached) | `fabrik:awaiting-ci` | — |

**References:** [ADR-1821: Merge-Train Own-PR CI Admission Gate](../adrs/1821-merge-train-red-member-admission-gate.md), [ADR-1644: Merge-Train Singleton Fast Path](../adrs/1644-merge-train-singleton-fast-path.md) (the inverse fail-closed polarity), [ADR-1208: Queued Review-Finding Ejection](../adrs/1208-queued-review-finding-ejection.md) (reroute primitive), [ADR-1545: Red-Singleton Reroute Before Pause](../adrs/1545-red-singleton-reroute-before-pause.md), [ADR-1420: Merge-Train Ejection Diagnostics](../adrs/1420-merge-train-ejection-diagnostics.md), §6.16 (`applyPendingReviewEjects`), §6.20 (the singleton fast path), issue #1821

---

### 6.26 Merge-Train Conflict Resolution Replay via `git rerere` (ADR-1834)

**Trigger:** Issue #1834. A merge-train conflict resolution lives only on its trial branch — `cleanupTrialArtifacts` deletes the trial worktree and branch after every trial, including bisection sub-trials, win or lose. When a trial is abandoned (bisected, re-formed after an eject, rebuilt after the base moves), the resolution is deleted with it, and the next assembly pays a fresh Claude invocation for the byte-identical conflict. On one operator's repo, the same file's conflict was resolved twice in 80 minutes across two trials for the same members.

**Mechanism: `git rerere` enabled repo-wide, replay detected via the merge's own output.** `enableRerere` (`engine/worktree.go`) sets `rerere.enabled`/`rerere.autoupdate` on the bare clone, called from `ensureBareClone` alongside the existing `setCommitterIdentity` — every worktree derived from that bare clone (regular per-issue worktrees and merge-train trial worktrees alike) shares the enablement and the `rr-cache` it writes to. For a bare repo, `bareDir` *is* `$GIT_DIR`, so `<bareDir>/rr-cache` is already scoped correctly and survives `CleanupTrainWorktree`'s `git worktree remove`/`git branch -D` untouched — no new cache-location plumbing was needed.

**Scoping is repo-wide, not worktree-scoped — a correction made during implementation.** The originally planned narrower approach — enabling rerere only for merge-train trial worktrees via Git's per-worktree config extension (`extensions.worktreeConfig` + `git config --worktree`) — was implemented, then reverted after direct reproduction showed it is unsafe: turning on `extensions.worktreeConfig` for a bare clone makes `core.bare` (`true`, a bare repo's own top-level value) the extension's shared/common config value, and any linked worktree that doesn't explicitly override it inherits that value — breaking every ordinary git operation in that worktree with `fatal: this operation must be run in a work tree`. Since Fabrik's every worktree for a given repo — every open issue's worktree, not just merge-train trials — shares one bare clone (`NewWorktreeManagerForRepo`), this would have broken every currently-open worktree for the repo the instant the extension was first turned on, not merely the newly-created trial worktree. Repo-wide enablement carries no such risk: a rerere lookup with no recorded resolution is a pure no-op, so a regular per-issue worktree's Validate-stage rebase conflict is, at worst, itself replayed for free if it happens to exactly match a merge-train-recorded resolution — a strict improvement, never a regression.

**`git merge` always exits non-zero on a conflict, even when rerere fully replays it.** Git's merge machinery invokes rerere synchronously inside `git merge` itself: it hashes each conflict hunk and, given a recorded resolution, rewrites the file in place. With `rerere.autoupdate=true`, a file whose every hunk was replayed is also automatically staged — but `git merge` still reports a conflict and a non-zero exit regardless of how much rerere already resolved, so `mergeErr != nil` in `assembleTrialBranch` is uninformative on its own about whether anything is actually left to do.

**Three-way dispatch in `resolveTrainConflict`, keyed on the post-merge unmerged-path set:**

| `unmergedPaths` after the failed merge | Dispatch |
|---|---|
| Empty, and no declared generated path was named in the merge's own `CONFLICT (...)` output | `commitRerereReplayedMerge` finalizes directly (`git add -A` + `git commit`) — **no Claude invocation** |
| Empty, but a declared generated path *was* named in the merge's `CONFLICT (...)` output | `regenerateAndCommit` runs the declared regeneration command fresh — **no Claude invocation**, and the replayed content is never trusted (see the ADR-1235 interaction below) |
| Non-empty | Unchanged from before this issue: `classifyConflictedPaths` runs against whatever remains, dispatching Claude for the non-generated remainder (if any) and/or regeneration for generated paths, exactly as ADR-1235 already specified. Claude's own `git status` naturally shows only what rerere left behind — `buildTrainConflictComment` was never handed a precomputed path list for the plain case, so no change was needed here at all |

The empty-`unmergedPaths` cases above are the ones this issue adds: previously, an empty result fell through to the same "no generated paths in the conflict" branch as an ordinary clean-Claude-dispatch case, invoking Claude even when there was nothing left to resolve.

**The ADR-1235 interaction: rerere has no awareness that a path is a declared generated file.** If a conflict confined to a declared generated path (e.g. `docs/llms-full.txt`) was already resolved via `regenerateAndCommit` once, and the identical conflict recurs, `rerere.autoupdate` would — left unchecked — silently replay and re-stage that old committed content before `resolveTrainConflict`'s `unmergedPaths`-based classification ever ran, so `matched` would come back empty and the regeneration command would never re-run, landing stale content instead of ADR-1235's guaranteed fresh regeneration of the trial's actual current sources. The fix is `conflictedGeneratedSpecsFromMergeOutput` (`engine/generated_files.go`): git always prints a `CONFLICT (...)` line naming every originally-conflicted path, independent of whether rerere then silently resolved it, so a substring check of each declared generated path against those lines recovers the conflict's *original* membership regardless of what `unmergedPaths` shows now. `resolveTrainConflict` unions that recovered set with whatever `classifyConflictedPaths` finds in the current (post-rerere) unmerged set via `unionGeneratedSpecsByPath`, before any dispatch decision — so a declared generated path is always force-regenerated, whether it's still genuinely unmerged or was already silently replayed.

**Trust boundary is unchanged (Requirement 4).** A rerere replay is committed and pushed exactly like a fresh Claude resolution, then validated by the trial's own combined CI — the same gate, with no weakening. Nothing about being "already resolved before" skips or shortens downstream validation.

**Red-trial resolution hygiene: `forgetPoisonerResolutions` (Requirement 5).** Bisection sub-trial worktrees are destroyed (`cleanupTrialArtifacts`) immediately after each sub-trial's own CI result is known — before the poisoner is identified, since bisection only learns the poisoner once every sub-trial has run. This means the porcelain `git rerere forget <path>` command (which re-derives the current conflict's hash from a *live* conflicted working tree — it cannot be pointed at an arbitrary historical hash from outside a live conflict) can't be run against the sub-trial that actually observed the poisoner's conflict; that worktree is already gone. Once `handleRedBatch` isolates the poisoner, `forgetPoisonerResolutions` reconstructs a fresh live conflict cheaply instead: a disposable trial worktree forked off the same pinned `p.baseSHA` every trial in the episode used, a solo `git merge --no-ff --no-edit <poisoner.headSHA>`, and — if that conflicts — `git rerere forget` for every path named in *that* merge's own output (`conflictedPathsFromMergeOutput`, the same mergeOut-derived technique as the ADR-1235 guard above, not `unmergedPaths`: once rerere's autoupdate has replayed and staged a path, `git status` no longer reports it as unmerged even though there is still a recorded resolution worth forgetting there). If the solo merge doesn't conflict at all, the batch's redness was a non-isolable interaction (ADR-059 D-e) unrelated to any conflict resolution, and nothing is forgotten — silently, with no warning logged. Entirely best-effort throughout: every git failure here is logged and none of them affect the eject that already happened. Skipped entirely under the `trainValidateFn` test seam, which never runs `assembleTrialBranch`'s real git assembly in the first place, so there is no real rr-cache entry from that episode to forget.

**Pruning: periodic `git rerere gc`.** Nothing in the codebase invoked `git rerere gc` before this issue, so `gc.rerereResolved`/`gc.rerereUnresolved`'s default expiries (60 days / 15 days) were moot and `rr-cache` would grow unbounded. `runMergeTrainWorker` now runs it once per worker invocation, immediately after `prepareTrainWorker` succeeds (mirroring that function's own base-SHA-pinning gate: skipped under the `trainValidateFn` test seam), best-effort. The defaults themselves are accepted as-is — rr-cache entries are small text blobs and the merge-train's own observed reuse window (minutes to weeks, per the motivating incident) comfortably fits inside 60 days.

**Accepted, not mitigated: concurrent rr-cache writers.** Two merge-train workers for the same repo but different `(repo, base)` partitions (ADR-1648) can run truly concurrently, each in its own trial worktree off the same shared bare clone, both writing into the same `rr-cache` with no mutex serializing the `git merge`/`git commit` sequence between them. Git's rr-cache writes are per-hash-directory, so two *different* conflicts recorded concurrently don't collide; the only real race is two workers resolving the *identical* conflict hash at the same moment — narrow, low-probability, and low-severity (worst case, one valid resolution's postimage is overwritten by another, still gated by the unchanged CI trust boundary above). No code mitigates this; it is a conscious trade-off, not an oversight.

**No configuration toggle.** rerere is unconditionally enabled whenever a bare clone is created or repaired; there is no `merge_train_rerere`-style opt-out, matching the issue's own requirements (which never mention one).

**Code path:** `enableRerere` (`engine/worktree.go`); `conflictedGeneratedSpecsFromMergeOutput`, `unionGeneratedSpecsByPath` (`engine/generated_files.go`); `commitRerereReplayedMerge`, `resolveTrainConflict`, `conflictedPathsFromMergeOutput`, `forgetPoisonerResolutions` (`engine/merge_train.go`, called from `handleRedBatch`), the `git rerere gc` call in `runMergeTrainWorker`.

**References:** [ADR-1834: Merge-Train Conflict Resolution Replay via git rerere](../adrs/1834-merge-train-rerere-conflict-replay.md), [ADR-1235: Generated-File Regeneration in Merge Train](../adrs/1235-generated-file-regeneration-in-merge-train.md) (the interaction this issue must preserve), [ADR-059: Internal Merge Train](../adrs/059-internal-merge-train.md) (D3 conflict resolution, D4 bisection, D-e non-isolable interaction), [ADR-1648: Merge-Train Per-Base Partitioning](../adrs/1648-merge-train-per-base-partitioning.md) (the accepted concurrent-writer surface), [ADR-1833: Deterministic Queued Batch Ordering](../adrs/1833-deterministic-queued-batch-ordering.md) (the prerequisite for rerere's hit rate), issue #1834
...
consults. A second structural shape is also a usage-limit exit (#1811, ADR-1811): the CLI reports
some session-limit hits as `terminal_reason == "api_error"` with `api_error_status == 429` (the HTTP
status behind the failure), which `classifyUsageLimitExit` claims ahead of §7.3a's `api_error`
classifier. `api_error_status` decodes tolerantly (`apiErrorStatus`): only a JSON integer counts;
absent, `null`, zero, or wrong-typed values read as "not 429" and never drop the rest of the result
...
**Scope (#1811, ADR-1811):** this section covers `api_error` exits whose `api_error_status` is **not**
429. An `api_error` carrying `api_error_status == 429` is a session/usage limit — account-wide, not
per-invocation — and is claimed by `classifyUsageLimitExit` first, taking §7.3's path (label,
comment, account-wide suspension). Every live `api_error` exit measured on 2026-09-19 (532 across three
daemons) was such a 429; the eight-exit sample below almost certainly was too. Non-429, absent, or
unparseable statuses keep the handling described here unchanged.
...
**Reinvoke cycle counters (#1812, ADR-1812).** The exemption extends to the reinvoke path's cycle budgets: an `api_error` (or usage-limit, suspension-gated, or `apiKeyHelper`) exit on a review, CI-fix or rebase reinvoke is refunded from `ReviewCycles`/`CIFixCycles`/`RebaseCycles` (and `ReviewBlockedCycles` when it was charged) rather than being permanently charged — see §6.2's "Did-not-run cycle refund". The comment circuit breaker (above) still counts the exit; only the cycle counters are compensated.
...
### 7.3c Tool-Permission Denial Coverage on Reinvoke Paths

§7.3b's bound (`ToolsDeniedRetries`/`MaxToolsDeniedRetries`, `pauseForToolsDeniedLimit`) originally
existed only on the stage-dispatch path (`finalizeStageOutcome`, `engine/item.go`) — every reference
lived in that one file. But `interpretClaudeResult` (`engine/claude.go`) is the single seam **both**
invocation paths share: `Invoke` (stage dispatch) and `InvokeForComments` (comment processing) each
receive an identical `*claudeToolsDeniedError` under the identical detection rule. `processComments`
(`engine/comments.go`) is the funnel every comment-driven invocation runs through — ordinary comment
review **and** all three reinvoke dispatchers (`dispatchCIFixReinvoke`, `dispatchReviewReinvoke`,
`dispatchRebaseReinvoke`) via the shared `dispatchReinvoke` scaffold — so before #1704 a mode denial
detected on any of those four invocation shapes fell into `processComments`'s generic `warn` log and
the comment circuit breaker (§4.6), which is calibrated for a different failure shape (a *runaway*
comment loop triggered by frequency) than a *slow, deterministic* denial recurring once per retry
cooldown. At a `poll: 120` cadence with a 20-minute stage-retry cooldown, a denial recurs roughly 1.5
times per 30-minute window — structurally unreachable against the breaker's default threshold of 10
per window — so the loop never converged: no terminal state, ~$0.30–0.40 burned per attempt,
indefinitely. The `ci-fix-reinvoke` case was the sharpest instance of this: CI had failed, so
`ci-fix-reinvoke` was the designed repair path, and it was the one disabled by the fault it exists to
recover from (#1657, #1704).

**Fix:** a new `errors.As(err, &toolsDeniedErr)` branch in `processComments`'s error handling
(alongside the pre-existing usage-limit exclusion, in the `if err != nil && !completed` block) —
covering all four invocation shapes in the single place they already share, rather than duplicating
the check inside each of the three `reinvokeOpts.after` callbacks. It calls the same
`recordToolsDeniedDetection` helper §7.3b's stage-dispatch branch calls (extracted from the previously
inline block so the two consumers cannot drift), and, once the shared counter reaches
`MaxToolsDeniedRetries`, the same unforked `pauseForToolsDeniedLimit`. The branch returns before either
`checkNoOpCommentCycle` or `checkCommentBreaker` runs, so a tools-denied cycle is bypassed from the
comment breaker entirely, exactly mirroring how the usage-limit exclusion immediately above it is
bypassed — the breaker's own thresholds are untouched.

**Consequences of sharing the counter:** `ToolsDeniedRetries` is keyed by `(repo, issue, stageName)`
only, not by which code path incremented it — so a denial on the stage path followed by a denial on
`ci-fix-reinvoke` for the same stage accumulate against the same counter and the same
`MaxToolsDeniedRetries` bound, and the outcome (pause at the bound, exemption from `max_retries`) is
identical regardless of which invocation type detected it (R3). No change was needed in `reinvoke.go`
...
`toolchain`/`go` directive, or `.tool-versions`) newer than what that inherited `PATH` resolves, and
nothing notices — workers keep running the stale toolchain silently, indefinitely; restarting `fabrik`
itself does not help, since it re-inherits the same `PATH` from the same parent shell. Reported in
#1658: a daemon whose hosting shell started 2026-08-11 kept resolving Node 20.20.2 after the repo
pinned Node 24 via `.nvmrc` on 2026-08-23, producing 36 `Unsupported engine` warnings in a single
issue's worker logs before the mismatch was noticed.

**Detection is structurally unlike every other exemption in this section (§7.3–§7.3c): it never skips
the invocation.** `checkToolchainDrift` (`engine/toolchain.go`) is a direct, best-effort side-effecting
call at the top of both invocation paths — `runInvocationWithExtension` (stage dispatch, immediately
after the `apiKeyHelper` check) and `processComments` (comment review, right after context files are
written) — never routed through `finalizeStageOutcome`, never returning an error, never touching
`StageAttempted`/`StageRetryIncremented`/`max_retries`. There is nothing for Fabrik to do about a
detected mismatch except tell someone: re-resolving `PATH` or driving a version manager (`nvm use`,
`asdf install`, …) is explicitly out of scope (#1786 R3).

1. `detectToolchainDrift` reads whichever declaration files exist in the worktree and, for each
   ...
   "cannot compare" — a distinct, pre-existing condition, never a mismatch. A successful resolution is
   memoized process-wide per tool for the daemon's lifetime (`resolveToolVersion`): since the bug's own
   premise is that this `PATH` cannot change without a restart, a second check for the same tool, on
   any issue, is a cache hit rather than a second shell-out. A resolution *failure* (subprocess timeout,
   transient exec error) is deliberately never cached — a failure carries no such invariance guarantee,
   so caching it would permanently and silently disable detection for that tool after a single bad
   moment; every invocation with a matching declaration retries an uncached failure instead.
2. Comparison is deliberately narrow and fails closed on ambiguity: `.nvmrc`, `package.json`
   `engines.node`, and `.tool-versions` compare at major-version granularity only (an `.nvmrc` alias
   like `lts/*`, or an `engines` range using `||`/`x`/multiple clauses, is "not comparable" — silence,
   never a false-positive warning); `go.mod`'s `toolchain`/`go` directive compares at major.minor
   granularity using minimum-version (`>=`) semantics, since a `go.mod` directive states a floor, not a
   pin — and `GOTOOLCHAIN=auto` (the Go 1.21+ default) already self-corrects in the common case, so this
   format mainly catches the `GOTOOLCHAIN=local`/auto-download-disabled edge case.
3. On any comparable mismatch, gated on `fabrik:toolchain-stale`'s own absence: an explanatory comment
   naming every mismatched declaration (source, declared version, resolved version) is posted and the
   label applied — once per episode (per issue), not once per invocation (#1786 R5).
4. If no comparable mismatch is found and the label is present, it is cleared — self-resolving once a
   worktree's declaration file is fixed, or the daemon is restarted with a corrected `PATH`.
...
small file reads and, at most once per tool per daemon lifetime, a version-check subprocess.

See §1.4's `fabrik:toolchain-stale` row, `docs/stage-lifecycle.md`'s "Toolchain Declaration Drift
Detection (Worktree)", and [ADR-1786](../adrs/1786-toolchain-declaration-drift-detection.md).
...
- **Exception — merge-train conflict resolution always gets a nonzero deadline.** `resolveConflictWithClaude` (`engine/merge_train.go`) never dispatches with the holding stage's raw `MaxWallTime`, even when it's absent or zero. `conflictResolutionStage` substitutes `mergeTrainConflictWallTimeFallback` (30 minutes) via a local, non-shared copy of the holding stage whenever its own `max_wall_time` is unset — `holdingStage(e.cfg)` returns the single process-wide `*stages.Stage` shared by every concurrently running (repo, base) merge-train worker (ADR-1648), so the substitution must never mutate that pointer in place. The fallback value flows into the ordinary `scaledWallTime` call above with no further wiring, so `fabrik:extend-turns` scaling applies to it exactly as it would to an explicit YAML value. This closes the gap where a stuck-but-still-emitting-output conflict-resolution invocation could otherwise run unbounded, relying solely on the inactivity timeout below (which never fires while output keeps arriving). See ADR-1500.
...
- Activity reset: only a poll cycle where `result.Active == true` (work was dispatched or a deep fetch ran) resets `idleStart` and the multiplier back to 1×. A `wakeCh` signal triggers an immediate poll but does **not** unconditionally reset the backoff — the multiplier is only reset if that poll finds active work. **Exception (#1716):** this assumes the wake reaches a poll at all — while rate-limit backoff is active (see the wake-path gate below), it does not; see §7.9.
- Rate-limit adjustment (`nextRateLimitLow` + `computeEffectiveInterval` in `engine/backoff.go`):
  ...
  - **Wake-path gate (#1716, ADR-1716):** the escalated interval computed above only ever reached the poll loop via the ticker (`ticker.Reset(result.NextInterval)` in `Run()`'s `doPollCycle`) — the separate `case <-e.wakeCh:` branch called `doPollCycle()` unconditionally, with no reference to backoff state at all. Under `--webhooks`, inbound GitHub events are not themselves rate-limited and kept arriving while every GraphQL call was being rejected, so a rate-limited daemon's own webhook traffic (including its self-feedback loop, §7.9) fed the loop at whatever rate events arrived — observed in production sustaining tens of polls per second and draining a fresh hourly GraphQL budget within seconds of every reset. `Engine.wakeBlockedByRateLimitBackoff()` (`engine/backoff.go`) closes this: `Run()`'s wake branch now checks it before calling `doPollCycle()`, returning true (drop the wake, no poll) whenever `backoffRateLimitLow` or `backoffRestPaused` is set. See §7.9 for the drop-vs-coalesce design and the ordering guarantee that this does not delay the legitimate rate-limit-recovery self-wake below.
  - **Poll-rate floor (R3, #1716):** independent of the gate above, `PollWithBackoff` itself refuses to reach `e.poll()` more than once per `minPollInterval` (a fixed 500ms, `engine/backoff.go`) regardless of what triggered the call — ticker, wake, or any future caller. This is defense-in-depth, not the fix for the wake-path bug itself: it bounds any future bypass of the gate above to a slow leak rather than an unbounded-rate loop. A floor-blocked call returns immediately (`PollBackoffResult{NextInterval: minPollInterval - elapsed}`) without touching `RateLimitStats()` or `e.poll()`.

**REST/core exhaustion is a hard pause, not a slowdown** (`shouldPauseForRESTRateLimit` in `engine/backoff.go`): the interval adjustment above conserves the **GraphQL** budget (spent by the poll read), but the **REST/core** budget is spent by per-item mutations (reactions, labels, comments, merges) and janitor fetches, which interval-stretching does not throttle. So when REST remaining drops to near zero (≤1% of limit, `isRateLimitNearZero`), `doPollCycle` skips the **entire** work phase — fetch, dispatch, and mutations — until GitHub's hourly reset (`reset + rateLimitResetBuffer`), emitting `RateLimitAlertEvent{Bucket: tui.RateLimitBucketREST, Exhausted: true}`. The independent worktree-janitor goroutine (`runWorktreeJanitor`) applies the same gate on entry, so it does not keep spending REST while the poll loop is paused. REST stats are refreshed from the headers of every REST response (including 403s), so the reset timestamp is authoritative even at zero budget. Introduced to stop a 403 retry storm after the REST budget was drained (#1118/#1121). `RateLimitAlertEvent.Bucket` (`tui.RateLimitBucketREST` | `tui.RateLimitBucketGraphQL`) identifies which of the two independent budgets an emit/clear event concerns, so `AlertBannerComponent` (`tui/alert.go`) tracks each bucket's active/reset state separately and never lets one bucket's `Exhausted: false` clear a banner the other bucket raised — see #1482. The GraphQL-only banner string reads "polling backed off," not "polling suspended" (#1716) — GraphQL exhaustion only slows polls via the interval/gate machinery above, it does not stop them the way REST exhaustion's hard gate does; the REST-only and combined banner strings are unchanged and remain accurate.

**Rate-limited probe logging is a per-attempt statement, not a suspension claim (R2, #1716).** `runProbeAndDeepFetch` (`engine/terminal.go`) runs *inside* `poll()`, before `PollWithBackoff`'s GraphQL hysteresis block (above) computes for that cycle — so when its own `ProbeProjectBoard` call fails on a low/exhausted GraphQL budget, it has no visibility into whether the real backoff this section describes is active, and no suspension mechanism of its own. Its log line describes only what happened at this call site (one probe attempt was rejected for rate limit) rather than claiming a stand-down it does not control; the `RateLimitAlertEvent{Bucket: GraphQL, Exhausted: true}` emission is unchanged (it remains the sole source of the GraphQL-exhausted TUI signal). **Log-rate protection (R4, #1716):** this log line is routed through `Engine.logfThrottled` (`engine/logthrottle.go`) — a shared dedup helper that suppresses a repeated *identical* message under the same key until either the message changes or a fixed interval elapses, appropriate here since this line's content (a minute-granular reset time, a mostly-stable error string) is genuinely static for the duration of one rate-limit episode. The three unconditional per-poll rate-limit stats/warning lines inside `poll()` (REST remaining, GraphQL remaining, GraphQL-low warning) instead use `Engine.logfThrottledByInterval` — a variant that throttles purely by elapsed time, ignoring message content entirely: their interpolated remaining-count changes on nearly every poll in ordinary (non-rate-limited) operation, so `logfThrottled`'s message-change-forces-log rule would have defeated the throttle for exactly the routine case it needed to cover (caught in review). Before this, a sustained rate-limited condition reproduced the same handful of lines every poll cycle; combined with the pre-#1716 wake-path bug this produced 39 log lines/second sustained in production (14 MB/hour). Both are thin wrappers over the same underlying dedup map — see `engine/logthrottle.go`'s doc comments for the exact semantics of each.
...
**Wake-path rate-limit gate (#1716, ADR-1716).** The idle-backoff interaction above assumes the wake reaches a poll cycle at all. It doesn't, while rate-limit backoff is active: `Run()`'s `case <-e.wakeCh:` branch checks `Engine.wakeBlockedByRateLimitBackoff()` (§7.8) before calling `doPollCycle()`, and drops the wake entirely when it returns true — no `doPollCycle()` call, no ticker drain (an already-pending tick is left to fire on the next loop iteration rather than being silently discarded along with the dropped wake). This closes the production incident this section's self-feedback loop made possible: under `--webhooks`, every self-feedback wake this section describes arrives while GraphQL or REST is rate-limited exactly when Fabrik's own mutations (label changes, comments) are what triggered the exhaustion in the first place, and pre-#1716 each one walked straight through the backed-off ticker interval to trigger an immediate, immediately-rejected poll.

- **Drop, not coalesce-with-re-arm.** A wake dropped during backoff is not queued for redelivery once backoff clears — no `pendingWakeDuringBackoff` flag exists. The already-backed-off ticker (reset to `PollBackoffResult.NextInterval` by the prior `PollWithBackoff` call) is what eventually re-polls; a board change that arrives via webhook mid-backoff and produces no further wake sits unprocessed for up to that interval, the same bound the ticker itself already had before this fix. The rejected alternative — an explicit re-arm flag that fires the instant backoff clears — was judged riskier: a dropped-with-no-re-arm bug in that design is silent, for a latency benefit already accepted as tolerable.
- **The legitimate recovery self-wake (§7.8's rate-limit-adjustment bullet, the `e.wakeCh <- struct{}{}` sent on the near-zero→recovered transition) is not blocked by this gate**, and needs no special-casing to achieve that: `PollWithBackoff` runs synchronously to completion — including the `backoffRateLimitLow` reassignment to its post-recovery value — before `doPollCycle` returns, and `doPollCycle` returns before `Run()`'s `select` can consume the queued wake. So by the time the gate actually evaluates a wake sent this way, `backoffRateLimitLow` has already settled to `false` and the gate passes it through. This is an ordering *property* of the current single-goroutine code (both `Run()`'s select loop and `PollWithBackoff` execute on `Run()`'s own goroutine), not an enforced invariant, and could be broken by a future change that makes either asynchronous — guarded by `TestPollWithBackoff_RecoverySelfWake_NotBlockedByBackoffGate` (`engine/poll_test.go`).
- **Acceptance/neutralization coverage.** `TestRun_WakeDuringRateLimitBackoff_DropsWake` (`engine/engine_test.go`) drives the real `Run()` goroutine with a sustained near-zero GraphQL budget and a burst of `wakeCh` sends, asserting the board-fetch count does not advance — confirmed (by hand, reverting the gate) to fail without it, exactly the "guards are deletable without failing a test" blind spot #1687 identified. `TestPollWithBackoff_MinPollIntervalFloor` covers §7.8's independent poll-rate floor.

### 7.10 Stall Detection and Corrective Re-Invocation

A stalled attempt — one where the worker backgrounded a long-running command (a dev server, build, or test run) and then idled waiting for a completion notification that never arrives in a headless environment — is distinguishable from a genuinely failed one by its turn-usage trend across retries within §7.1's cooldown loop: a stage that did real work and failed burns turns roughly consistently across attempts; a stalled one shows a clean incomplete attempt followed by a retry that completes **fewer** turns, still without completing. A genuinely-progressing retry does not shrink like that. This holds whether the first attempt burned its full turn budget before giving up, or — the cleanest and most common real-world shape — the worker correctly recognized it was waiting on a backgrounded command and ended its turn early, deliberately, well under budget: the latter is textually the `#1142` production incident (three clean, uncapped, declining Validate attempts — 45/100, 20/100, 7/100 — none of which armed the hint before the original turn-capped-predecessor precondition was widened). See issue #1146 and ADR-1146 for the original incident (#816) that motivated this, and #1767/ADR-1146's amended Consequences for the `#1142` gap and the widened signature below.
...
1. Reads the previous attempt's `TurnsUsed`/capped/clean status for this stage from the store (`LastTurnsUsed`, `LastTurnsCapped`, `LastTurnsClean` — zero-valued if this is the first attempt), and whether a hint has already armed this episode (`StallEpisodeArmed`).
2. Computes this attempt's own capped status: `clean && usage.MaxTurns > 0 && usage.TurnsUsed >= usage.MaxTurns`. Forcing `capped` to `false` whenever `!clean` matters independently of arming (step 4) — see the recording note below. Capped-ness is evaluated against *this attempt's own* `MaxTurns`, so it is unaffected by `fabrik:extend-turns` widening the budget between attempts. `LastTurnsCapped` is still recorded and read (step 4), but only to phrase the stall comment correctly — it dropped out of the arming precondition itself in #1767.
3. Records this attempt's `TurnsUsed`/capped/clean status via `itemstate.StageTurnUsageRecorded`, overwriting the previous values, **on every call — clean or not.** Recording a non-clean attempt with `Clean=false` (regardless of its actual turn count) keeps it from going invisible to the trend: it immediately invalidates the "previous clean" precondition (step 4) for the *next* comparison, so a generic error sitting between two clean attempts breaks the chain at the point of the error rather than being skipped over. An earlier version of this fix gated the whole function call (recording included) behind `clean`, which left `LastTurnsUsed`/`LastTurnsCapped` describing the last *clean* attempt across an intervening error — a later clean attempt would then compare against that non-consecutive predecessor and could still misfire. See ADR-1146's Rationale for the incident this reproduced and the regression test that guards it.
4. If `clean` is true, the *previous* attempt was itself clean (`LastTurnsClean` — capped or not; #1767 dropped the "previous attempt was turn-capped" requirement, since it made the cleanest form of the stall — a worker that deliberately ends its turn early, well under budget, having recognized it's waiting on a background job — invisible to the detector, exactly `#1142`'s shape), this attempt is **not** itself turn-capped, this attempt used more than zero turns but strictly fewer than the previous attempt, **and no hint has already armed this episode** (`!StallEpisodeArmed`), the pattern is a detected stall: applies `itemstate.StallHintArmed` (which also sets `StallEpisodeArmed`) for the stage and posts a one-time informational comment citing both turn counts, phrased according to whether the predecessor was actually turn-capped (`LastTurnsCapped`, from step 2) or stopped short on its own — never asserting a turn-limit hit that didn't happen. The "not itself capped" precondition guards against a false positive from the progress-based turn-extension loop (`runInvocationWithExtension`, see "Turn Limit Extension" above): that loop can widen the effective budget on one dispatch (e.g. to 2×/3× `stage.MaxTurns`) without the widening persisting to the next, separate dispatch, so two consecutive turn-capped attempts could otherwise show a smaller absolute `TurnsUsed` on the second purely because its own budget reset lower, not because it is a declining, stalled retry. The `clean` precondition guards against a different false positive: `claudeRan` is true for most invocation errors (network failure, git-push error, malformed CLI output), not only clean incomplete stops, so without it a generic error whose partial `usage.TurnsUsed` happened to be smaller than a prior clean attempt's would be misread as a declining, stalled retry and arm a specific, confident — but wrong — diagnosis. `err == nil` alone is not sufficient to compute `clean`, since a turn-cap exit is itself reported as a non-nil `*claudeTurnLimitError` (ADR-1178); `turnLimited` is reused from the classification already computed earlier in `finalizeStageOutcome` for the `InvocationRecorded` write, so both consult the same definition of "clean." See ADR-1146 (as amended by #1767) for the fuller rationale.
5. **One-shot per episode via an explicit guard, not an emergent property.** Before #1767, the attempt that triggered arming was (by construction, per the then-precondition) not itself capped, so step 3's overwrite cleared the "previous capped" precondition for arming again immediately — no separate one-shot guard was needed. Widening the precondition to "previous clean" removed that emergent property: the arming attempt is clean too (arming requires `clean` for the *current* attempt), so it no longer clears its own precondition by being uncapped. `StallEpisodeArmed` is the explicit replacement: set by `StallHintArmed` the first time a hint arms in a retry episode, checked before arming again, and cleared only by `StageRetryCleared` (see "Cross-stage isolation and cleanup" below) — so a continued multi-attempt decline (e.g. `#1142`'s real 45→20→7 sequence) still arms exactly once, not on every subsequent comparison.
...
**Cross-stage isolation and cleanup.** `StageRetryCleared` (applied by `clearFailedStage()` on unpause, and by a stage's completion) deletes `LastTurnsUsed`, `LastTurnsCapped`, `LastTurnsClean`, `StallHintPending`, and `StallEpisodeArmed` for that stage alongside the existing `Attempts` reset — so a later, unrelated incomplete run of the same stage (e.g. after `fabrik:revalidate`) never inherits a stale armed hint (or a stale one-shot guard) from a long-past episode.

**State:** In-memory only (`StageState.LastTurnsUsed`/`LastTurnsCapped`/`LastTurnsClean`/`StallHintPending`/`StallEpisodeArmed`, all `map[string]bool`/`map[string]int` keyed by stage name), mirroring every other per-stage `StageState` field — same restart-gap tradeoff already accepted by ADR-030's turn-extension baseline.

**Scope note:** this implements only the "cheaper to detect" trend signal from issue #1146 — a clean incomplete attempt (capped or not, #1767) followed by a declining, incomplete one. It does not add live NDJSON tool-use parsing to detect backgrounding-then-silence within a single invocation (the issue's other suggested signal); that remains a possible follow-up if the trend signal proves insufficient in practice.
...
| 5 | Awaiting input + comment | `isAwaitingInput()` + new **human** comments created at/after the pause (`resumeAuthorised()`) | Unblock → comment processing |
| 6 | Awaiting input, no comment | `isAwaitingInput()` | Skip with log |
| 7 | Paused + comment | `fabrik:paused` + new **human** comments created at/after the pause (`resumeAuthorised()`) | Unpause → fall through |
...
**`fabrik:reworking:<Stage>` without active comment processing (#1802):**
If `fabrik:reworking:<Stage>` is left orphaned by a prior crash mid-rework, the item's completion claim may be either still cleared or already durably resolved, depending on exactly where the crash landed — `finalizeComments` only removes this marker after `handleStageComplete` has made its own completion decision on a completing exit, not before (§4.2 step 8b/11). On restart, `runStartupCleanup()`'s third pass (mirroring the `fabrik:editing` pass immediately above it) reads the reworked stage's name directly out of the marker label's own suffix — `<Stage>` in `fabrik:reworking:<Stage>` — never from the item's current board `Status`. This is a deliberate correction (a Pruefer review finding, #1802): a completing rework that also auto-advances the board (yolo/cruise/`auto_advance: true`) moves `Status` to the *next* stage synchronously inside `handleStageComplete`, before `finalizeComments`'s deferred removal of this marker ever runs. A crash in that window would leave `Status` pointing at a stage that never actually ran; resolving the reworked stage from `Status` there would falsely stamp `stage:<NextStage>:complete` onto it, which `itemNeedsWork` then reads as "already done," silently and permanently skipping that stage's real dispatch. Reading the stage name from the label itself avoids this entirely, regardless of how far `Status` has since moved. Using that resolved stage name, the pass first checks whether `stage:<Stage>:complete` or `fabrik:awaiting-ci` is already present: if either is found, the rework already settled correctly before the crash and only the stale marker is removed; otherwise it restores `stage:<Stage>:complete` directly (the crash landed before any completion decision was recorded — indistinguishable from, and treated the same as, a non-completing exit) and then removes `fabrik:reworking:<Stage>`. Either way this only runs for an item with no active Worker. If the marker is malformed (no stage name encoded in the suffix), it is left in place (logged loudly) rather than silently dropping the only remaining signal that a restore is owed — see ADR-1802.
...
| `StageState.DidNotRunReinvokes` | `DidNotRunReinvokeRecorded` (increment); `DidNotRunReinvokesReset` (reset on a charged reinvoke); `EngineCyclesCleared` (reset) | `snap.DidNotRunReinvokes(stageName)` | — |
...
### 11.7 Process-Sweep Janitor (#1798 R3)

A fourth periodic janitor, `runProcessSweepJanitor`, runs alongside the worktree/log/session
janitors above — same schedule (§11.2: startup scan, then hourly), same
`JanitorIntervalHours != 0` gate, no separate config knob. Unlike the other three, it does
not scan `.fabrik/worktrees/` at all: it sweeps the durable session-scoped descendant
registry (`.fabrik/state/descendants.json`) for orphans that escaped both the PGID-scoped
grandchild cleanup and the per-invocation session-scoped reap — see
[stage-lifecycle.md § Session-Scoped Descendant Reaping](stage-lifecycle.md#session-scoped-descendant-reaping)
for the full mechanism, and [ADR 1798](../adrs/1798-session-scoped-descendant-reaper.md) for
...
mismatch — the process already exited, or the PID was reused for something unrelated since it
was recorded — means the entry is dropped from the registry and never signalled.

**Summary log**:

```
[proc-janitor] cycle complete: scanned N registry entries, reaped K, skipped M
```
...
by PID; an unparseable file is quarantined to `workers.json.corrupt` with a logged warning and
treated as empty, so a corrupt file cannot disable recording). The sweep takes one process-table scan (the shared cached one) and one `Getsid` pass, and
...
| **Label freshness** | Re-fetched via `refreshAutonomyLabels()` | Re-fetched via `refreshAutonomyLabels()` |
...
**Autonomy-label re-fetch, both paths (#1769, §6.23).** `handleStageComplete()` and `runCatchUpPhase2()` each call the shared `refreshAutonomyLabels()` helper (`engine/stages.go`) as the first step of their autonomy-decision logic, before either reads `hasYoloLabel`/`hasCruiseLabel`. This row previously read "Already fresh from deep fetch" for Path 2 — that was **false**: `Labels` is not part of the deep-fetch cache contract (`copyDeepFieldsFromState` excludes it), so an item that went through a "fresh" deep fetch earlier in the same poll pass could still carry a stale `fabrik:yolo`/`fabrik:cruise` from before an operator removed it. Both paths now perform the same live, item-scoped `FetchLabels()` call (never `e.cfg.Owner`/`e.cfg.Repo`, which is empty in multi-repo mode) immediately before the autonomy decision, so a mid-run add *or remove* of either label is observed by whichever path makes the next advance decision — see §6.23 for the full rationale and the D1/D2 defects this replaced.
...
| Repo write access | Repo not yet resolved by `resolveRepoAccess` (fail-open on "unknown"), OR resolved with `CanPush: true`. A repo cached with `CanPush: false` is never admitted, regardless of stage or status — see ADR-1347. Purely in-memory (no API call); the probe itself always runs earlier in the same poll cycle, in `poll()`'s seeding block. `resolveRepoAccess`'s probe branches on auth mode (#1750, ADR-1750): PAT mode is `FetchRepoAccess`'s `permissions.push` (unchanged, ADR-1347). App mode (`e.ghAppAuth != nil`) instead consults `resolveAppRepoAccess` against `Engine.appAccessibleRepos` — the pinned installation's own `GET /installation/repositories` list, fetched once at startup (`New()`), since `permissions.push` is structurally meaningless for an installation token (GitHub always returns it `false`, regardless of actual grants). Ambiguous App-mode answers (the startup fetch never succeeded, or the list was truncated and the repo wasn't found in it) fail open through the same `if err != nil` branch PAT mode's probe errors already use — never a silent `CanPush: false`. A confirmed exclusion (App or PAT mode) additionally gets a persistent `warnings.Record` entry (`Type: "repo_access"`), not just the one-line startup log. A confirmed **zero**-repo installation is a hard startup refusal from `New()` instead of a per-repo determination. |
...
| 5 | Awaiting input | `isAwaitingInput()` true AND new **human** comments created at/after the pause exist (`resumeAuthorised()`) |
| 6 | Paused | Not paused, OR paused with new **human** comments created at/after the pause (`resumeAuthorised()`) |
...
| `check_suite` | `completed`, `requested`, `rerequested` | No-op | — | Check suites are coarse aggregates; individual runs tracked via `check_run.completed`. The store holds no suite state, so the suite-aware CI gate (#1822, §6.4) reads suites live via `FetchCheckSuites` instead of relying on this webhook |
