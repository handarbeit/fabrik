...
**A known, narrow blind spot: a merge-train label write that fails outright.** If `fabrik:credited-pr:<N>` fails to write after its landing call site's `advanceToNextStage` already succeeded, the item still gets `fabrik:awaiting-landing-verification` (applied as a separate call, independent of the credited-PR label's own success), and the settle scan correctly falls back to `FetchLinkedPR` — which, for a merge-train item, finds the member's own (closed-not-merged) PR, not the integration/singleton PR that actually carried the work. This resolves as *inconclusive* (the linked PR is not the credited PR, so its `merged` state doesn't answer the actual question) rather than a silent pass or a false confirmed-failure — the item sits as `fabrik:awaiting-landing-verification` until it escalates to `fabrik:paused`, requiring manual clearing even though the landing itself was fine. Accepted per R5/AC4's "never reopen on ambiguity" framing.
