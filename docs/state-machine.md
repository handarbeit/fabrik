...
| 8b | Restore or re-derive `stage:<Stage>:complete` (#1802) | `endStageRework()` | No-op unless step 2b actually cleared the label. On a non-completing exit (this step, or any of the earlier setup-failure/invocation-failure early returns): restores `stage:<Stage>:complete` directly, then removes `fabrik:reworking` immediately — nothing later in the flow will restore it. On a completing exit: this step is deliberately skipped here — `stage:<Stage>:complete` is left to step 11's `handleStageComplete()` to re-derive from scratch (including its `wait_for_ci` deferral), and `fabrik:reworking` itself is *not* removed until after step 11 completes (not here at step 8b), so a crash anywhere between this point and step 11's completion-label write still leaves the marker as a durable signal that a restore is owed. See ADR-1802. |
...
| 11 | If FABRIK_STAGE_COMPLETE was detected: handle completion | `handleStageComplete()` | Same completion flow as a normal stage invocation (advance, PR ops, etc.). On a completing exit that began a rework (step 2b), `fabrik:reworking` is removed immediately after this step returns — not before — per step 8b's note (#1802, ADR-1802). |
...
**`fabrik:reworking` without active comment processing (#1802):**
If `fabrik:reworking` is left orphaned by a prior crash mid-rework, the item's completion claim may be either still cleared or already durably resolved, depending on exactly where the crash landed — `finalizeComments` only removes this marker after `handleStageComplete` has made its own completion decision on a completing exit, not before (§4.2 step 8b/11). On restart, `runStartupCleanup()`'s third pass (mirroring the `fabrik:editing` pass immediately above it) first checks whether `"stage:" + snap.Status() + ":complete"` or `fabrik:awaiting-ci` is already present: if either is found, the rework already settled correctly before the crash and only the stale marker is removed; otherwise it restores `"stage:" + snap.Status() + ":complete"` directly (the crash landed before any completion decision was recorded — indistinguishable from, and treated the same as, a non-completing exit) and then removes `fabrik:reworking`. Either way this only runs for an item with no active Worker. If `snap.Status()` cannot be resolved, the marker is left in place (logged loudly) rather than silently dropping the only remaining signal that a restore is owed — see ADR-1802.
