...
**Paused-item recovery loop (R4):** After the main catch-up loop, a separate lightweight scan iterates `deepFetchCandidates` looking for items that carry both `fabrik:paused` and `fabrik:awaiting-ci` on a `wait_for_ci: true` stage. For each such item, `e.client.FetchLinkedPR` is called directly (not via `e.readClient`/boardcache — the cache may have stale `Merged`/`State`). When `pr.Merged == true`, the loop calls `addCompleteLabelAndRemoveCI`, removes `fabrik:paused` + `fabrik:awaiting-input`, and calls `advanceToNextStage`. This advances the issue to Done without requiring a manual unpause. The loop must never dispatch workers or acquire `e.sem` (see ADR 053).
...
| Same column, Awaiting CI | Poll tick (catch-up) | `mergeableState ∉ {"", "unknown"}` (e.g. `behind`, `dirty`, `draft`, `has_hooks`) + no check runs + `hadChecks == false` | Same (blocked, no label) | | | New guard: `mergeable_state` is actively blocking but no check_runs visible — branch protection likely requires a Commit Status or other external signal Fabrik cannot see. Returns `(true, false, false)`; re-evaluates on next poll. `CIWaitTimeout` escalation applies via the generic timeout row below. |
...
| Same column, Awaiting CI + **`fabrik:paused`** (recovery loop) | Poll tick | Linked PR is **merged** (`pr.Merged == true`); item is paused with `fabrik:awaiting-ci` | Same column, Complete → advance | `stage:<X>:complete` | `fabrik:awaiting-ci`, `fabrik:paused`, `fabrik:awaiting-input` | R4: paused-item recovery loop (separate from main catch-up) detects merged PR via direct `e.client.FetchLinkedPR`. Clears gate + removes pause labels + advances to Done without manual unpause. Uses `e.client` (not `e.readClient`) to avoid stale boardcache state (see ADR 053). |
...
**After Phase 1 + Phase 2 — paused-item recovery loop (R4):** A separate lightweight scan iterates `deepFetchCandidates` for items with both `fabrik:paused` and `fabrik:awaiting-ci` on a `wait_for_ci: true` stage. For each, `e.client.FetchLinkedPR` is called directly (not `e.readClient` — boardcache may have stale state). When `pr.Merged == true`: `addCompleteLabelAndRemoveCI()`, remove `fabrik:paused` + `fabrik:awaiting-input`, `advanceToNextStage()`. This self-heals Case A (issue merged externally while paused) without requiring a manual unpause. The loop never dispatches workers or acquires `e.sem` — it is a read-only label-mutation path only (see ADR 053).
