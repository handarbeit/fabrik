...
**Code path:** `processItem()` → `handleNoWorkNeeded()` (writes the marker, then delegates) → `settleNoWorkNeeded()` (idempotent; does the rest of the work) — and, on retry, the no-work-needed settle scan in `poll.go` calls `settleNoWorkNeeded()` directly.
...
**Code path:** `preImplement()` → `spawnChildren()` (writes the marker on failure) — and, independently, the child board-placement settle scan in `poll.go` calls `settleChildPlacement()` every poll for any item carrying the marker.

**Settle-scan flow (`poll.go`, immediately after `cleanupClosedIssueTransientLabels`):**
...
**Code path:** `landOneAtATime` → `landSingleton` (writes the marker on close failure) — and, on retry, `settleMergeTrainMemberCloses` (`poll.go`) → `settleMergeTrainMemberClose` (`engine/merge_train_member_close_settle.go`) directly.

**Retry-owner: `settleMergeTrainMemberCloses` (`poll.go`).** Runs unconditionally once per poll, immediately after `handleMergeTrainBatch` — independent of `merge_train: on/off`, so a marker written while merge-train was enabled keeps draining even if the setting is later turned off. It iterates the **raw `board.Items`** (not `deepFetchCandidates` — this scan has no dependency on the deep-fetch/terminal-skip machinery at all). For every item carrying `fabrik:awaiting-member-close` and not `fabrik:paused` (mirroring the no-work-needed settle scan's own paused-item guard — an operator investigating a paused item must not be fought), it calls `settleMergeTrainMemberClose`.
...
3. For each item: resolve its current stage via `stages.FindStage(e.cfg.Stages, item.Status)`. Skip if `!item.IsClosed`, `stage == nil`, `stage.CleanupWorktree`, `stage.HoldingStage`, or `stageIsGateChecked(stage)` (excludes Validate — left to `runValidatePRTerminalAdvance`, preventing any double-advance/race between the two settle-owners).
...
**References:** [ADR-057: Single-Owner Validate PR Terminal Advance](../adrs/057-validate-pr-terminal-advance.md), [ADR-063: Closed-Item-At-Any-Stage Advance To Done](../adrs/063-closed-item-any-stage-advance-to-done.md)
