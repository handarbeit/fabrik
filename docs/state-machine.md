...
2. **Hook 2 (poll-side, `routeQueuedGroup`):** before `dispatchMergeTrainWorker` is called. If the counter is already tripped (from a prior worker run), pauses the current Queued snapshot (the uncapped `items` slice — every Queued member for the repo, not just the batch-capped subset that would have been dispatched) and skips dispatch. This handles beyond-cap members that Hook 1 could not reach. **Exception (#1647):** any member `filterNonDefaultBaseMembers` excluded from `trainItems` this same poll for targeting a non-default base is dropped from this pause set (`excludeNonDefaultBaseExclusions`) — such a member was just told, via `markNonDefaultBaseExcluded`'s own comment, that it "remains safely in `Queued`, not paused, not failed, not moved," and the runaway guard (an unrelated train-health signal that base-excluded member never contributes trials to) must not contradict that in the same poll cycle. Queue-enabled members and batch-cap overflow are unaffected by this exception — both are still paused as before.
