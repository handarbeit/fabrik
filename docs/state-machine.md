...
4. **The member's own CI is green *and complete*.** `classifyLandingCI` (ADR-1153/ADR-1441), reused unmodified — the same check-run-based classifier `pollForMergeable` uses for the ordinary landing path, never `mergeable_state` alone.
...
| Queued, single member, all conditions positively confirmed | `MergePR` succeeds; `recordAdvanceOutcome` succeeds | Done, awaiting landing verification | `fabrik:awaiting-landing-verification` | — |
| Queued, single member, all conditions positively confirmed | `MergePR` succeeds; `recordAdvanceOutcome` fails | Queued (unchanged); advance failure durably retried (§6.17) | — | — |
| Queued, single member, all conditions positively confirmed | `MergePR` fails (transient) | Queued (unchanged); retried next poll | — | — |
