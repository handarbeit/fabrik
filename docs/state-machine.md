...
1. `detectToolchainDrift` reads whichever declaration files exist in the worktree and, for each
   ...
   "cannot compare" — a distinct, pre-existing condition, never a mismatch. The resolved version is
   ...
   any issue, is a cache hit rather than a second shell-out.
