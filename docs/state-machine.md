...
**Paused-item recovery loop (R4) — `runPausedItemMergedPRRecovery`:** After the main catch-up loop, a separate lightweight scan iterates `deepFetchCandidates` looking for items that carry `fabrik:paused` AND either `fabrik:awaiting-ci` OR `fabrik:awaiting-review`. For each such item, `e.client.FetchLinkedPR` is called directly (not via `e.readClient`/boardcache — the cache may have stale `Merged`/`State`). When `pr.Merged == true`, the function iterates all pipeline stages in ascending `Order` from the stage after the highest already-complete stage up to (but not including) the cleanup-terminal stage. For each stage where `WaitForCI || WaitForReviews` is true and its `stage:<Name>:complete` label is absent, `AddLabelToIssue` is called with cache write-through. After all completion labels are added, `removeAwaitingCILabel` and/or `removeAwaitingReviewLabel` are called as applicable, then `fabrik:paused` and `fabrik:awaiting-input` are removed and `advanceToNextStage` is called. This advances the issue without requiring a manual unpause. The function must never dispatch workers or acquire `e.sem` (see ADR 053).

**Convergence-paused recovery loop (R4b):** A second lightweight scan runs immediately after `runPausedItemMergedPRRecovery`. It handles items with `fabrik:paused` + `stage:<X>:complete` but **without** `fabrik:awaiting-ci`. These items were paused by `pauseForConvergenceFailed()` (which removes `fabrik:auto-merge-enabled` but adds no `fabrik:awaiting-ci`); both the main catch-up loop (paused guard) and `runPausedItemMergedPRRecovery` (`fabrik:awaiting-ci` guard) skip them permanently. When `pr.Merged == true`, removes `fabrik:paused` + `fabrik:awaiting-input` and calls `advanceToNextStage` to advance to Done. Uses `e.client` directly (not `e.readClient`); must never dispatch workers or acquire `e.sem` (same constraints as ADR 053).
...
| Same column, Awaiting CI + **`fabrik:paused`** (recovery loop) | Poll tick | Linked PR is **merged** (`pr.Merged == true`); item is paused with `fabrik:awaiting-ci` | Same column, Complete → advance | `stage:<X>:complete` (+ any other missing gate-checked complete labels) | `fabrik:awaiting-ci`, `fabrik:paused`, `fabrik:awaiting-input` | R4: `runPausedItemMergedPRRecovery` detects merged PR via direct `e.client.FetchLinkedPR`. Iterates all pipeline stages in ascending Order from the highest already-complete stage onward, adding `stage:<N>:complete` for every `WaitForCI` or `WaitForReviews` gate-checked stage whose label is absent. Clears gate labels + removes pause labels + advances without manual unpause. Uses `e.client` (not `e.readClient`) to avoid stale boardcache state (see ADR 053). |
| Same column, Awaiting Review + **`fabrik:paused`** (recovery loop) | Poll tick | Linked PR is **merged** (`pr.Merged == true`); item is paused with `fabrik:awaiting-review` | Same column, Complete → advance | `stage:<X>:complete` (+ any other missing gate-checked complete labels) | `fabrik:awaiting-review`, `fabrik:bot-reprompted` (if present), `fabrik:paused`, `fabrik:awaiting-input` | R4 (symmetric case): `runPausedItemMergedPRRecovery` covers `fabrik:awaiting-review` as well as `fabrik:awaiting-ci`. Same multi-stage gap-fill logic applies. `removeAwaitingReviewLabel` clears the review gate (also removes `fabrik:bot-reprompted`). Advances without manual unpause (see ADR 053, amendment). |
| Validate column, Complete + **`fabrik:paused`** (convergence-paused recovery loop) | Poll tick | Linked PR is **merged** (`pr.Merged == true`); item has `stage:Validate:complete` but NO `fabrik:awaiting-ci` | Done | | `fabrik:paused`, `fabrik:awaiting-input` | R4b (convergence-paused path): items paused by `pauseForConvergenceFailed()` have `stage:Validate:complete` + no `fabrik:awaiting-ci` — skipped by both the main catch-up loop (paused guard) and `runPausedItemMergedPRRecovery`. Convergence-paused recovery loop detects merged PR via `e.client.FetchLinkedPR`, removes pause labels, and calls `advanceToNextStage` to advance to Done inline. |
...
**After Phase 1 + Phase 2 — paused-item recovery (`runPausedItemMergedPRRecovery`, R4):** A separate lightweight scan iterates `deepFetchCandidates` for items with `fabrik:paused` AND either `fabrik:awaiting-ci` OR `fabrik:awaiting-review`. For each, `e.client.FetchLinkedPR` is called directly (not `e.readClient` — boardcache may have stale state). When `pr.Merged == true`: the function iterates all pipeline stages in ascending Order from the stage after the highest already-complete stage up to (but not including) the cleanup-terminal stage, adding `stage:<N>:complete` for every `WaitForCI` or `WaitForReviews` gate-checked stage whose label is absent. After all labels are added, `removeAwaitingCILabel` and/or `removeAwaitingReviewLabel` are called as applicable, then `fabrik:paused` + `fabrik:awaiting-input` are removed and `advanceToNextStage()` is called. This self-heals both Case A (CI gate, issue merged externally while paused) and Case B (review gate, same scenario) without requiring a manual unpause. The function never dispatches workers or acquires `e.sem` — it is a label-mutation-only path (see ADR 053).
