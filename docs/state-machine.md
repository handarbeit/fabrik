...
5. On `ErrNotMergeable`: post comment, add `fabrik:paused`, return error (prevents completion label from being added, allowing retry)
6. On other API errors: return error (same retry behavior)
7. On success (including already-merged): log and return nil — advancement proceeds
