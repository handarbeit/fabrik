...
**Rate-limited probe logging is a per-attempt statement, not a suspension claim (R2, #1716).** `runProbeAndDeepFetch` (`engine/terminal.go`) runs *inside* `poll()`, before `PollWithBackoff`'s GraphQL hysteresis block (above) computes for that cycle — so when its own `ProbeProjectBoard` call fails on a low/exhausted GraphQL budget, it has no visibility into whether the real backoff this section describes is active, and no suspension mechanism of its own. Its log line describes only what happened at this call site (one probe attempt was rejected for rate limit) rather than claiming a stand-down it does not control; the `RateLimitAlertEvent{Bucket: GraphQL, Exhausted: true}` emission is unchanged (it remains the sole source of the GraphQL-exhausted TUI signal). **Log-rate protection (R4, #1716):** this log line, and the three unconditional per-poll rate-limit stats/warning lines inside `poll()` (REST remaining, GraphQL remaining, GraphQL-low warning), are all routed through `Engine.logfThrottled` (`engine/logthrottle.go`) — a shared dedup helper that suppresses a repeated identical message under the same key until either the message changes or a fixed interval elapses. Before this, a sustained rate-limited condition reproduced the same handful of lines every poll cycle; combined with the pre-#1716 wake-path bug this produced 39 log lines/second sustained in production (14 MB/hour). The throttle is a general-purpose helper, not specific to rate-limiting — see its own doc comment for the exact first-occurrence/message-change/elapsed-interval semantics.
