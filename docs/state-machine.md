...
**Known limitation, out of this issue's scope (carried forward from Research's Technical Questions), confirmed real rather than hypothetical:** `advanceValidateTerminalItem` — and by extension both settle-owners built on it — hardcodes `stage.Name == "Validate"` rather than `stageIsGateChecked(stage)`. Both shipped default stage templates, `stages/examples/review.yaml` and `stages/examples/validate.yaml`, set `wait_for_reviews: true` — so a deployment using the defaults genuinely has a second gate-checked stage (Review), not merely a hypothetical one. A closed item stranded at Review with no `stage:Review:complete` is, after this fix, never dispatched (R1 holds) but also never healed by any settle-owner — `settleClosedItemsToDone` (§6.11) already excludes `stageIsGateChecked` stages generally (pre-existing, unchanged by this issue), deferring to "whichever settle-owner owns gate-checked closed items," but that owner only ever answers for Validate. This is not a regression introduced by this fix — before ADR-1387 such an item was *admitted to dispatch* and exhibited the identical unbounded-loop symptom this issue fixes for Validate, just unreported; after ADR-1387 it is never dispatched but also never healed, trading a loud, token-burning symptom for a silent, stranding one. In practice reaching the stranded state itself is believed rare: an issue closes via `Closes #N` only on PR merge, and PR merge authority sits at Validate, not Review, in the standard pipeline shape — reaching this case requires an issue to be closed manually while still parked at Review with `fabrik:awaiting-review` outstanding. Generalizing `advanceValidateTerminalItem` to `stageIsGateChecked` would close this gap but is deliberately deferred (Research's Technical Question, resolved as "leave hardcoded" for this issue) since it changes behavior for a second gate-checked stage and needs its own test matrix, independent of this issue's Validate-scoped bug.
