...
**Logging (R4).** Every disposition — eligible-and-landed, or the specific unmet condition that forced a trial — is logged from `trySingletonFastPath`/`singletonFastPathEligible`, so "why didn't it fast-path?" is answerable from logs without inference.
