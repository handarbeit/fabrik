...
**One fetch covers every downstream read.** `attemptMergeOnValidate` (the single landing-decision function both paths call — ADR-1216) receives `item` by value and performs its own `hasCruiseLabel(item)` check with no live re-read of its own. Because both callers invoke `refreshAutonomyLabels` upstream of every `attemptMergeOnValidate` call site, that check is automatically fresh too, without a second REST call — bounding the cost to exactly one live `FetchLabels` per `handleStageComplete`/`runCatchUpPhase2` invocation (i.e. at most once per item per poll, since the two paths are mutually exclusive admission for the same item in the same pass).
